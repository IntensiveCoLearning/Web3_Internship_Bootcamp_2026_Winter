...
| 核心机制 | 恒定乘积 (CPMM) | 集中流动性 (CLMM) | 集中流动性 + Hooks |
| 合约架构 | Factory + Pair (多合约) | Factory + Pool (多合约) | Singleton (单巨型合约) |
| 资本效率 | 低 (0 -> $\infty$) | 极高 (最高可达 V2 的 4000x) | 极高 |
| LP Token | ERC-20 (同质化) | ERC-721 NFT (非同质化) | 无 (由 Hook 定义或内部记账) |
| 手续费级 | 固定 0.3% | 0.01%, 0.05%, 0.3%, 1% | 完全自定义 |
| 价格预言机 | TWAP (每块存储) | TWAP (存储在数组中，更容易调用) | 自定义 (通过 Hooks) |
| Gas 优化 | 一般 | 较好 (L2 上表现更佳) | 最佳 (Flash Accounting) |

* * *

### 5\. 专家视角的深度解析：安全与风险

作为一名系统安全专家，我必须提醒你注意各版本的特定风险：
