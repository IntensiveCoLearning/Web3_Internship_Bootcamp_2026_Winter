...
这份学习笔记基于 Austin Griffith 在 Speedrun Ethereum Workshop 中的分享整理而成。笔记涵盖了核心的学习方法转变、具体的挑战案例分析以及给初学者的建议。

### Web3 实习计划：Speedrun Ethereum Workshop 学习笔记

1\. 核心学习理念的转变：AI 辅助编程 ("Vibe Coding")

Austin 强调了目前 Solidity 学习方式的重大转变。

-   **不再死磕语法**：过去 Speedrun Ethereum 旨在让学生放慢速度，理解每一行代码。但现在，AI（如 Claude, Cursor, ChatGPT）已经足够强大，不需要再手写每一行代码,。
    
-   **关注高层概念**：现在的重点应放在理解“发生了什么”以及核心概念（如状态机、激励机制）上，而不是语法细节。建议将挑战说明复制到 AI 中，让 AI 解释核心课程，甚至生成代码,。
    
-   **工具推荐**：使用 **Scaffold-ETH 2**，配合 **Cursor** 和 **Claude** 进行开发,。
    

2\. 核心挑战与关键概念 (Key Concepts by Challenge)

**挑战 0: Simple NFT (Tokenization)**

-   **部署策略转变**：现在的“热点观点”是不再推荐使用测试网（如 Sepolia）。
    
-   **Test in Prod**：建议直接部署到 Layer 2 网络（如 Base, Arbitrum, Optimism）。因为 L2 的费用极其低廉（几美分），且部署在主网上的资产是永久存在的，可以发给朋友,。
    

**挑战 1: Crowdfunding (众筹)**

-   **智能合约即状态机 (State Machine)**：合约在不同状态（如“存款中”、“成功”、“失败”）之间流转。
    
-   **被动执行 (The Poke)**：以太坊上**没有任何事情是自动发生的**。即使到了截止时间，状态也不会自动改变，必须有人发起交易去“戳”（Poke）合约。
    
-   **激励机制 (Incentives)**：如果没有激励，没人会去触发状态变更。必须编写规则，例如“谁点击这个按钮触发结算，谁就能获得 100 美元奖励”。只要有经济激励，即使世界大乱，也会有人（或脚本）为了赚钱去执行操作,。
    
-   **对抗性思维**：必须假设参与者中有坏人试图偷钱。如果众筹失败，必须保证每个人都能取回资金，且不能被恶意锁死,。
    

**挑战 2: Token Vendor (代币贩卖机)**

-   **合约交互**：这是合约与合约之间的交互（代币合约 vs 贩卖机合约）。
    
-   **Approve Pattern (授权模式)**：这是以太坊的一个“怪异”且重要的模式。如果你想让贩卖机合约拿走你的代币并给你 ETH，你不能直接把代币发给它。
    
    1.  你必须先向代币合约发起交易，**Approve（授权）** 贩卖机花费你的代币。
        
    2.  然后再调用贩卖机合约，它会使用 `transferFrom` 把代币拿走。
        
-   **数学与小数 (Decimals)**：EVM 不处理浮点数。1 ETH 实际上在代码中是 $1 \\times 10^{18}$ Wei。在处理百分比时（如 5%），通常是乘以 100 再除以 105 等方式处理，而不是乘以 0.05,。
    

**挑战 3: Dice Game (随机数)**

-   **伪随机数的陷阱**：在公开的确定性区块链上做随机数很难。使用 `block.hash`（前一个区块的哈希）作为随机源是不安全的，因为矿工/验证者可以操纵它来获利,。
    
-   **解决方案：Commit-Reveal (提交-揭示)**：
    
    1.  用户生成一个随机数，将其**哈希 (Hash)** 后提交上链（Commit）。
        
    2.  等待新的区块生成。
        
    3.  用户**揭示 (Reveal)** 原始数字。
        
    4.  合约将用户的原始数字与之前的区块哈希结合，生成真正的随机数,。
        
-   **进阶方案**：使用 Chainlink VRF 获取可验证随机数。
    

**挑战 4 & 5: DeFi & Lending (去中心化金融与借贷)**

-   **DEX (去中心化交易所)**：通过费用激励流动性提供者 (LP)。LP 只要有人交易就能赚取费用，这是一种无需信任的协作机制,。
    
-   **超额抵押借贷 (Over-collateralized Lending)**：
    
    -   **核心逻辑**：借出 100 美元资产，必须抵押价值 150 美元的 ETH。
        
    -   **清算 (Liquidation)**：这是保证协议不破产的关键。当抵押品价值下跌（例如跌到 120%），必须允许任何人调用 `liquidate` 函数卖掉抵押品还债。
        
    -   **激励清算人**：为了保证有人调用清算函数，协议会给予清算人折扣或奖励（例如给他们 10% 的抵押品）。这是为了确保在抵押品价值跌穿借款额之前将其变现,。
...
-   **关于资金安全**：
    ...
    -   建议使用 **Gnosis Safe (多重签名钱包)** 来保管大额资金。通过 M-of-N (如 3/5) 的签名机制，防止因丢失单个私钥或设备导致资产丢失,。
        
    -   对于银行等机构，相比于 MPC 或 HSM，多重签名也是更好的管理方式。
        
-   **关于“代码即法律” (Code is Law)**：
    
    -   虽然在技术上黑客只是利用了合约逻辑（如闪电贷攻击），但在人类社会这通常被视为盗窃。Austin 认为即便代码允许，偷窃依然是不道德和非法的,。
        
-   **关于开发框架**：
    
    -   **Foundry** 目前是硬核开发者的首选；**Hardhat** 依然很好用，特别是在与前端 TS 测试结合时。但初学者现在可以直接用 Scaffold-ETH 配合 AI 生成代码,。
        

4\. 给新人的三条建议 (Three Advices)

1.  **完成 Speedrun Ethereum**：通关这些挑战，掌握上述的核心概念（状态机、随机数、清算等）。
    
2.  **阅读现有协议代码**：去阅读优秀的开源合约，如 Uniswap v2, OpenZeppelin, Solady 等，学习它们是如何运作的。
    
3.  **动手构建 (Build Things)**：使用 Scaffold-ETH 和 AI，尝试构建真正的应用。不要只停留在测试网，去 Base 或 Arbitrum 上发一个代币，组建流动性池，亲身体验“编程货币”的乐趣。
    

**未来展望**：Austin 认为未来是 **Agentic Economy**（智能体经济），AI Agent 将在链上进行支付和交互。以太坊将成为这些互不信任的 AI 之间的协调层,。
