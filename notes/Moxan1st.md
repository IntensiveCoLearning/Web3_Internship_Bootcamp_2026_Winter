...
完成汇报——Done，按照预期完成了，就得练！  
完成周总结——Done，手搓了总结，让 AI 帮忙润色了一下。

下周计划——尝试完成进阶类的任务，冲刺总榜。
...
分享多角度需求分解的重要性和方式。分享人**Moxan @Moxan1st**

第一页：

为什么要分解需求？因为项目落地需要团队的实施，落实到每个人身上的任务不清晰。

思考：如果不这样做，项目可以推动吗？

以DApp为例，一个Dapp最小团队需要有产品、前端和后端。

第二页：应用的定位：商业盈利、基础设施

第三页：开发：用户人物画像，市场大小——》使用场景——》功能——》子功能（前端、后端和合约）——》功能开发和验证

第四页：演示视频：使用场景——》视频脚本（镜头的移动方式，包含的拍摄主体）——视频制作（拍摄和生成）——》后期

第五页：演示PPT：背景——》内容（解决怎么样的痛点痒点问题、解决得怎么样）——》总结和期许

第六页：汇报过程：代入角色（你的目的是什么？）——》提前准备稿件——》提前反复练习（关键）

第七页：与 AI 合作：如果是人，依然会忘记自己的多重身份，忘记半小时前做事的细节，所以要盯紧 AI ，因为最后负责的活人一定是大家自己
...
动作：Kenji 在去学校的路上，发现一家新开的咖啡馆门口有完美的轮椅坡道。他拿出手机，打开 Help2Earn，拍了一张门口的照片并上传。3 秒后，AI 验证通过，Kenji 的钱包收到了 50 个 tokens 代币（够喝个奶茶）。

分镜1：镜头近景对准咖啡厅门口的轮椅坡道0.5s

分镜2：镜头中景对准咖啡厅和门口的轮椅坡道，小王从画外入镜，发现轮椅坡道

分镜3：镜头近景对准小王半身，小王拿出手机，

分镜4：镜头对准手机屏幕，小王打开 Help2Earn，拍了一张坡道的照片并上传，AI 验证通过，Kenji 的钱包收到了 50 个 tokens 代币（够喝个奶茶）。

### 场景 2：数据消费者 (The User)

人物： Sato 先生，资深咖啡爱好者，轮椅使用者动作： Sato 先生在街头遇到出行决策困难，通过 Help2Earn 获取了比普通地图更精准的深度数据，并成功抵达目的地。

分镜 1：镜头近景对准 Sato 先生的侧脸，他正坐在轮椅上，眉头微蹙地盯着手机上的 Google Maps。屏幕上显示着目的地咖啡馆，但设施标签模糊不清，只有一行简陋的“可能提供无障碍设施”。

分镜 2：镜头中景Sato 先生切换 App，手指轻点打开了 Help2Earn。界面瞬间加载出绿色的深度验证图标，比起普通地图，这里的标注显得格外醒目。

分镜 3：镜头特写（手机屏幕）屏幕上弹出 AI 深度分析报告，伴随着流畅的动效展示关键数据：AI 设施报告：坡度：

（小于标准，非常省力）路面： 磨砂防滑平整面提示： 自动门开启感应较快，持续时间约 5 秒，请注意进出节奏。

分镜 4：镜头全景Sato 先生露出放心的微笑，收起手机，熟练地操纵轮椅转向，朝着镜头远方的咖啡馆稳步前进。

### 场景 3：商家与链上声誉 (The Merchant)

人物： Lin（主理人）、小王（数据矿工/消费者）动作： 见证信用增长，铸造 SBT 荣誉，并完成代币支付闭环。分镜 1：镜头近景（平板屏幕）对准 Help2Earn 商家后台。屏幕中心是一个粗线条的向上箭头，随着背景中“验证人数”跳动，箭头由下至上逐渐变大、颜色由暗转金并发出强光。箭头下方标注着：On-chain Reputation Increasing...。

分镜 2：镜头特写Lin 的手指点下“铸造 SBT”按钮。强光汇聚成一枚无障碍友好 (Access-Friendly) 的盾形勋章。右下角伴随一行小字：“Verified by 50 Miners via SpoonOS”。

分镜 3：镜头中景（科技感交互）小王站在柜台前，举起手机对准平板扫描。平板屏幕闪过一个绿色的“Token Accepted”图标，显示：-50 Tokens。Lin 露出自信的微笑，两人之间仿佛有一种基于“共建社区”的默契。

分镜 4：镜头全景（完成闭环）Lin 将咖啡递给小王。小王接过咖啡转身走出店门。镜头拉远，定格在咖啡馆门口：完美的轮椅坡道、玻璃门上的 Help2Earn 金牌认证贴纸、以及小王离去的背影。

每个分镜要跟上个视频连贯的接在一起。这是Help2Earn的参考页面

第二个场景也要接到第一个场景，第三个场景也要接到第二个场景
...
头脑风暴黑客松项目——Done，选择赛道2，内容保密，制作readme.md
...
-   Market：具体事件下的交易市场，包含 NegativeRisk （多个互斥市场中对其中一个NO，相当于对一个集合取反，就是其它集合的并集）
    
-   Oracle 先知/预言机：“判官”。预言机 把结果（数据）“喂”给智能合约。
    
-   Condition：市场在链上的身份，要绑定预言机，具体问题（event），结果选项的数量。
    

![image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Moxan1st/images/2026-01-27-1769519549369-image.png)
