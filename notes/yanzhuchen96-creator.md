...
今天我看了 7k 老师关于Web3的分享，感觉脑子被塞了好多东西，但同时又有一种被轻轻戳醒的爽感。

最先打动我的是那张互联网演进的对比图。从Web 1.0的静态页面，到Web 2.0的社交爆炸，再到所谓“read-write-trust verifiable”的阶段，看着那些熟悉的旧图标渐渐被区块链相关的符号取代，心里其实有点复杂。以前我们都说Web 2.0让我们变成了内容的生产者，可现在回想，生产的内容其实一直在喂养平台的数据机器。这种“被看见却不被拥有”的感觉，分享里用enshittification（那个带💩的词）点得很准——平台先用甜头把我们哄进来，最后却把我们和创作者都榨干。我看的时候忍不住点头：对啊，这几年用得越多的App，反而越觉得它在变味。

然后是去中心化的网络拓扑那部分。中心化、去中心化、分布式三种结构的对比图画得特别清楚，从一个点辐射所有，到逐渐散开成网格，最后变成每个人都连着每个人的样子。突然就觉得，原来我们现在上网的体验，本质上还是在绕着一个或几个超级节点转。看到“不可能三角”的时候我甚至有点小激动——去中心化、安全、可扩展性三者真的不能同时满分？这不就是现实世界里所有治理难题的缩影吗？

经济和公共物品那几页让我停顿最久。公共物品 vs. 共有资源、搭便车问题、公地悲剧、国家供给 vs. 社区自治……这些概念本来就耳熟，但放到区块链语境里，突然变得很具体。尤其是那张Ostrom的四类物品表格，把知识、和平、国防放在“低可减性、低排除难度”的公共物品格子里，而渔场、森林放在共有池资源里，我盯着看了好一会儿。原来Web3想做的，不只是发币和炒币，而是试图用token、经济激励、二次方投票这些机制，来修复人类长期解决不好的“大家的东西没人好好管”的老大难。

价值金字塔那个倒三角真的戳到我了。从最底层的“use value”一层一层往上堆，到现金、黄金，最后顶端放了个问号的crypto。我盯着那个问号笑了半天——是啊，它到底算什么？是下一层文明的基石，还是又一次人类集体发疯的泡沫？没有答案，但这个问题本身就让人觉得兴奋又不安。

最后引用到从反主流文化到赛博文化的部分，虽然只有几句，但我读完后有种很奇妙的穿越感。原来今天区块链社区里那些“去中心化就是自由”“代码即法律”的口号，跟60年代公社主义者对共享家园的向往，其实是同一种情绪的不同版本。只是当年是回归土地，现在是回归代码。

整体看完，我没有被彻底说服Web3已经成功或即将成功，但被一种可能性打动了：人类好像又一次在尝试用新的工具，去重新回答“怎么让大家的东西被大家公平地拥有和维护”这个古老问题。这个问题没有终点，但每一次认真尝试，都值得被看见。

挺感谢这次分享的，它没给我标准答案，却让我对正在发生的事多了一点好奇和耐心。
...
黑客松里最容易翻车的不是bug，而是人没配对。理想状态是3人小队：一个懂产品和讲故事的人，加上两个能快速落地的开发。多一个闲人就多一份协调成本，少一个核心位就直接残血。真正高效的往往是多边形战士——会点设计的前端、能前后通吃的全栈，比纯专精但单一的人更吃香。人少好分钱，人多好吵架，这个反直觉的现实在每届黑客松都反复验证。

## 安全意识是底线中的底线

技术小白最容易踩的雷就是把私钥、API key直接推到GitHub。重复多少遍都不够：所有敏感信息必须塞进.env文件，并且立刻加到.gitignore。压秒提交、最后一刻狂推也是大忌，review延迟或意外暴露都能直接送走一个项目。安全不是加分项，是不被当场淘汰的入场券。

## 你的用户从来不是“所有人”

很多人一上来就想做“给所有人用”的东西，结果做成四不像。黑客松的真实用户排序永远是：评委 > 投资人 > 金主爸爸 > 普通用户。所以故事、商业闭环、视觉冲击、PPT质感远比代码优雅重要。dApp界面可以丑，但绝不能low；代码可以一坨，但必须让评委一眼看出“这个idea有戏”。先搞清楚痛点、用户画像，再谈功能。

## 别把MVP做成航母

最常见的死法是功能越堆越多，以为越多越完整，其实是在把时间浪费在“不会加分但极易翻车”的地方。黑客松评委要看的只是：你能不能用极短时间证明一个清晰的价值点，并且现场跑得通。只做一个完整闭环——问题→操作→结果→价值显现，其他全是奢侈品。功能多不代表强，稳定+聚焦才代表专业。

## 翻车现场的唯一正确反应

Demo崩了千万别当场debug，那一刻解释技术细节只会让评委更烦。第一反应是切到预录视频，平静地说一句：“现场网络/依赖不稳定，我们用预录Demo展示完整流程，同时这里是链上交易哈希和代码仓库关键结构。”然后立刻打开区块浏览器或repo证明东西确实做出来了。评委看到的是你的预案能力和表达冷静，而不是bug本身。提前录好多个版本的Demo视频，是黑客松铁律。

## 评委听不懂=你输了

技术再牛，没人记住也没用。Demo Day必须在3分钟内回答三个问题：你的创新点到底是什么？你想讲一个什么样的完整故事？为什么现在、为什么是你来做这件事？没有强有力的motivation和叙事弧光，项目再炫也只是背景板。讲故事的能力，往往比写代码的能力更稀缺。

## 邪门但管用的临场救场

实在来不及完善，就用前端障眼法：把loading、粒子特效、暗黑模式切换做到极致，视觉先把人抓住；或者直接上大监制剪片法，视频转场丝滑、BGM带感、字幕节奏感强，让人感觉“哇这个团队很懂呈现”。这些东西不解决核心问题，但能极大降低“看起来很low”的扣分。

黑客松的本质是短跑中的信息密度竞赛。技术大多已经过关，剩下的比拼是气场、预案、叙事和对评委心理的理解。把这些经验内化成肌肉记忆，上场就不会慌。
...
48小时启动阶段，开发团队专注写代码，影视制作团队同时着手准备 Demo 的拍摄和素材整理，保证技术与展示双线推进。

## 24小时：功能冻结（Feature Freeze）

进行到 24 小时时进入功能冻结阶段，严格执行减法原则：不再添加任何新功能和后端代码，测试覆盖率达不到 80% 的直接舍弃，并用 “Coming soon” 作为占位提示。异常处理方面，像余额不足、网络拥堵等情况一律先不处理，只确保主流程顺畅：链接钱包、领取测试币、购买 NFT、完成交易。此时要开始大量使用 Mock 数据，先坐下来画一条绝对不会出错的 Demo 路线，把链上数据索引结果预先写死到前端，保证演示过程完全可控。

## 12小时：魔术师阶段——构建幻觉

剩下 12 小时时，团队角色更像魔术师，核心目标是掩盖技术瑕疵、优化感官体验并确保整体逻辑自洽。首先在前端层面做“整容”，因为一个代码很强但界面丑陋的项目，往往打不过代码一般但 UI 精美的项目；在 Mint 的关键瞬间，可以让前端直接显示“成功”，把上链逻辑放到后台异步处理，即使失败也留到 Demo 之后再说。接着提前准备不超过三个已经在链上有过交互、具备资产和历史记录的钱包账号，以免现场临时领水失败或排队。最后要做好环境隔离，如果公共测试网（如 Sepolia）拥堵或 RPC 宕机，就预备一套本地节点（Localhost / Hardhat Network）作为备选，一旦公网出问题，立即切换到本地演示，让评委看不出差异。

## 12小时：导演阶段——Demo 视频与文档

进入最后 12 小时是导演阶段，重点产出决定生死的 Demo 视频和文档。视频录制是最重要的环节，因为很多评委只看视频不看代码，建议按照剧本公式来设计：前 0–30 秒讲清楚痛点，用讲故事的方式而不是念 PPT；30 秒到 2 分钟用录屏展示核心操作，加速处理签名等等待环节，并用高光或放大效果突出鼠标点击位置；2 分钟到 2 分 30 秒展示架构图，重点标出赞助商的 Logo；最后 2 分 30 秒到 3 分钟讲商业模式和后续开发计划。剪辑时即便代码不够稳定，也可以把多段成功运行的片段剪接在一起，营造流畅体验。同时要精心装修 GitHub README，把它当成项目门面：需要有醒目的项目 Logo、简洁有力的一句话 Slogan、演示视频链接、测试网部署地址（Demo URL）、测试账号和私钥，以及详细的赞助商技术栈列表，方便评委检索和对号入座。

## 4小时：后勤阶段——稳妥交付

最后 4 小时进入后勤保障阶段，核心是确保顺利提交，避免任何低级失误。首先要提前提交，考虑到 Devpost 之类的平台经常在截止前 10 分钟宕机，建议提前 2 小时创建 Project 页面，先填写 80% 的信息，最后再更新视频链接和 GitHub 仓库地址。其次要完成合约验证，保证智能合约在 Etherscan 上显示为 Verified（已验证）状态并公开源码，因为技术评委如果只看到一串 Bytecode 乱码，容易认为项目不够开源甚至有猫腻。最后做一次多端测试，把 Demo 链接发给朋友，让他们用手机或无痕浏览器打开，检查项目是否依赖本地插件或缓存才能跑通，确保任何设备上都能顺利访问。

## “防猝死”小贴士

在高压的 48 小时里，要特别注意团队健康和节奏管理。睡眠策略上，PM 或队长必须保持清醒负责整体统筹，而开发同学如果在同一个 BUG 上卡住超过一小时，就必须去睡 20 分钟换脑子。能量补给方面，单喝红牛效果有限，更要补充葡萄糖和巧克力这类能够快速提供多巴胺的零食。最关键的是心态建设：始终记住 Done is better than perfect，完成比完美更重要，一个有 BUG 但可以跑完完整流程的项目，远远好过一个只写了一半但自认为完美的项目。最后的 48 小时，本质是技术、产品和营销交织的心理战，评委看不到你熬夜写代码的辛苦，只能看到你呈现出来的故事，因此要把精力用在打磨整体叙事和观感上。
...
今天系统接触 Kite最大的认知是：它不是“一条普通公链”，而是专门给 AI / Agent 做支付和结算的基础设施。官方的定位是 AI payment blockchain，目标是让各种智能体可以带着身份、预算、权限，在链上用稳定币完成真实的支付和结算，而不只是做推理和聊天。 对我来说，一个简单的理解是：如果传统 EVM 链更像“通用操作系统”，那 Kite 更像是给 AI 经济体准备的“支付铁路”和规则引擎，专门解决“Agent 怎么安全花钱、怎么被追责、怎么被限制”的问题。

## 三层身份：User / Agent / Session 的脑回路

让我印象最深的是 Kite 的三层身份体系：User、Agent、Session。User 是真实拥有资产的人，比如一个普通用户的钱包；Agent 是被授权可以代用户做事、发起支付或调用合约的智能体身份；Session 则是每次具体操作时，用来执行某个任务的临时身份，有时间和额度限制。 这套设计直接对应了现实里“老板 → 授权员工 → 员工手上的工作卡”的关系：老板有钱，员工被授权可以在某些范围内花钱，而单次消费又可以通过限额卡来控制风险。在 Kite 里，这意味着我可以给某个 AI Agent 授权：它可以帮我支付，但必须通过 Session 这种一次性的“手套”执行，每次能花多少钱、能调哪些合约都可以在链上写死，既方便自动化，又不会把主账户完全暴露给模型。

## 可编程支付与 Escrow：资金先锁定，再按证明释放

Kite 的另一个核心概念是可编程支付约束和链上 Escrow。文档和白皮书里反复强调：资金不会直接打给对方，而是先进入 on-chain escrow，只有在有完成证明（proof of completion）、满足服务合约（SLA）条件时，才按规则释放部分或全部资金。 这种模式对 AI 场景非常重要，因为它把“是否完成任务”的判断，从单纯信任对方，变成由合约和验证机制共同控制。对我来说，这里的关键信息是：Kite 原生支持把资金锁定在可编程 Escrow 合约里，在那里可以实现授权、扣款、退款、冻结、撤销等完整生命周期；而 Agent 只是其中的一个参与方，通过被授权才能动这笔钱。

## 为什么 Kite 适合 AI 驱动的支付场景

站在应用设计的角度，我慢慢理解了为什么要选 Kite，而不是随便找一条 EVM 链来做支付。传统链也能做 Escrow 和多签，但 Kite 把“AI 作为一等公民”这件事做进了协议层：有 Agent Passport（身份）、可编程预算/权限控制、对 Agent-to-Agent 支付和退款/撤销流程的专门设计。 官方在用例和技术支柱里明确写了三块：Agent Passport、Programmable Escrow/SLA Contracts、Agent Payments，强调的是 AI 代理之间的支付轨道，而不是单纯人对人的转账。 对我个人的启发是：今后设计任何“AI 触发支付”的系统，都可以优先考虑 Kite，因为它天然提供了“谁花钱、能花多少、在什么条件下花”的链上保护层，少了很多自己造轮子的风控工作。

## 从文档到实现：我需要掌握的最低技术门槛

回顾这段学习，我对自己需要掌握的“最低技术门槛”也更清晰了。第一是理解 Kite 的网络层，知道它是 EVM 兼容链，有自己的 testnet、RPC、chainId，可以像在其他 EVM 上一样写 Solidity 合约和用工具部署。 第二是搞懂 Kite 的 SDK 和 Account Abstraction 思路：SDK 帮我封装了账户管理、交易构造和发送 UserOperation 等操作，我只要提供目标合约地址、调用数据，就可以用 Agent 或 Session 身份发起交易。 第三是学会把云端 LLM 服务接进后端：选一个有官方 API 的大模型（比如 OpenAI 或国内模型），用 API Key 调一次简单的文本判断，把结果封装成统一的 `{ passed: boolean, reason: string }` 格式，再用它来驱动链上合约里的放款逻辑。 只要这三块打通，我就能在 Kite 上实现一个由 AI 驱动、带有 Escrow 和可编程支付约束的完整 demo，而不必从零造一个“AI+支付”的基础设施。
...
-   黑客松的目标不是“做一个完整产品”，而是交付一个**能演示的闭环**。
    
-   选题时先想清楚：谁用、在什么场景用、具体痛点是什么、为什么一定是你。
    
-   聚焦一个明确、单点、可演示的需求，不要追求“大而全的应用级产品”。
    
-   如果一句话说不清楚“我为谁、在什么场景、解决什么痛点、用什么方案”，就要考虑换题。
    

## 二、MVP 与执行

-   时间极短（8–48 小时），核心目标：做出一个能一路点下去、跑通主流程的 MVP。
    
-   允许：
    
    -   用 mock/假数据、模拟结果、写死配置、单页面前端。
        
    -   合约/后端只实现最核心一两个功能。
        
-   不需要、甚至建议删掉：
    
    -   注册/登录、复杂权限系统、管理后台、多链/多模式、完整 UI。
        
-   只保留：一个用户、一个主路径、一个成功结果。
    
-   一句话：逻辑可以粗糙，但不能“没有产品/没有 Demo”。
...
-   对评委来说 Demo 和视频比代码重要得多，代码大概率不会跑，但 Demo 一定会看。
    
-   强烈建议：
    
    -   提前录好 2–3 分钟 Demo 视频（问题 → 操作 → 结果）。
        
    -   剪掉：loading、pending、卡顿、报错。
        
-   现场即使翻车，有视频也能“救命”。
    
-   没有 Demo = 白干；Demo 清楚 = 已经赢一半。
    

## 四、路演思路与 PPT 结构

**路演目标**：让评委在 2–3 分钟内搞懂“你在做什么 & 为什么值得加分”，而不是把所有细节讲完。

推荐结构（5 分钟以内）：

1.  一句话项目介绍（slogan）。
    
2.  问题/痛点：现实世界或 Web3 场景的具体痛点。
    
3.  解决方案：你的产品/核心思路。
    
4.  Demo：最重要，用视频或现场演示讲“人怎么用”。
    
5.  技术亮点：点到为止（突破点、性能/成本/安全改进、用了哪家 sponsor 技术）。
...
-   技术型团队（技术 > 产品）：
    ...
    -   适合做技术创新、性能/成本/安全性的突破，用好赞助商提供的 SDK / 链 / 协议。
        
    -   路演重点：技术突破点和技术价值，线下赛评委可能会看代码。
        
-   非技术 / 运营 / 产品型团队：
    
    -   80% 时间想清楚商业逻辑和使用场景，20% 时间用 AI/模板/低代码做 Demo。
        
    -   路演重点：痛点、场景、为什么这个产品有意义。
...
5.  不看 sponsor / 赛题方向，闭门造车。
    

**5 个核心能力：**

1.  闭环思维（比技术重要 10 倍）：能清楚回答谁在什么场景用、用前 vs 用后差别、为什么一定要你、为什么这个产品有意义。
    
2.  MVP 取舍能力：敢删功能，聚焦可演示闭环。
    
3.  “骗评委”的展示能力（褒义）：会用模拟结果、假数据、静态页面，把 POC 讲清楚。
    
4.  会用 AI 当“队友”：
    
    -   反推 idea、从评委视角提问、写 README 和路演脚本、设计 Demo 流程。
        
5.  Demo 优先思维：所有工作都围绕“如何在 1–3 分钟内演示清楚”。
...
在搭建节点阶段，我利用 Hardhat 自带的 Hardhat Network 启动了一条本地以太坊兼容链。我回到项目根目录，直接执行 npx hardhat node，Hardhat 会在 http://127.0.0.1:8545 上监听一个 JSON‑RPC 服务端口，同时在终端打印出一组测试账户和对应的私钥，这些账户默认拥有大量测试 ETH，方便我在开发过程中随意发送交易。 只要这个进程保持运行，本地链就一直存在，我后续在另一终端里使用 --network localhost 参数执行任何 Hardhat 脚本时，都会连到这条本地链，而不是创建新的临时网络。 对于我这次的练习来说，一个终端专门跑 npx hardhat node，另一个终端专门跑脚本，是最清晰也最不容易搞混的方式。

在部署阶段，我先在 contracts 目录下编写了一个最简单的 HelloWorld 合约。合约文件命名为 HelloWorld.sol，采用 Solidity 0.8 版本，内部定义了一个 string public message 状态变量，在构造函数中将其初始化为 "Hello, World!"，并提供 setMessage(string memory \_msg) 函数用来修改内容，以及 getMessage() public view returns (string memory) 函数用来读取当前消息。 由于 Hardhat 会在执行脚本前自动对 contracts 目录进行编译，我不需要单独运行编译命令。在 scripts 目录中，我新建了一个部署脚本 deploy-hello.js，采用 ES Module 写法并且依赖 Hardhat 的运行时环境。在脚本中，我通过 import hre from "hardhat"; 导入 Hardhat Runtime Environment，然后在异步的 main 函数里调用 const HelloWorld = await hre.ethers.getContractFactory("HelloWorld"); 获取合约工厂，接着执行 const hello = await HelloWorld.deploy(); 部署合约，使用 await hello.waitForDeployment(); 等待交易确认，最后通过 console.log("HelloWorld deployed to:", await hello.getAddress()); 打印出合约地址，作为部署完成的标志。 启动好本地节点后，我在另一个终端切换到项目根目录，执行 npx hardhat run scripts/deploy-hello.js --network localhost，脚本会自动编译合约并将它部署到刚才启动的 Hardhat Network 上，当终端输出合约地址时，我就完成了“配置环境、搭建本地节点、部署 HelloWorld 合约”这一整套本地链开发流程。

![屏幕截图 2026-01-24 145849.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/yanzhuchen96-creator/images/2026-01-24-1769262904406-_____2026-01-24_145849.png)
