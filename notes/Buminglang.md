...
-   Solidity by Example **Foundry、DeFi**
...
solidity三种抛出异常的方法：error、require、assert

1.  `error`**是**`solidity 0.8.4版本`**新加的内容，方便且高效（省**`gas`）地向用户解释操作失败的原因，同时还可以在抛出异常的同时携带参数，帮助开发者更好地调试。
    
    在执行当中，`error`**必须搭配**`revert`（回退）命令使用。
    
2.  `require`**命令是**`solidity 0.8版本`**之前抛出异常的常用方法，目前很多主流合约仍然还在使用它。它很好用，唯一的缺点就是**`gas`**随着描述异常的字符串长度增加，比**`error`**命令要高。使用方法：**`require(检查条件，"异常的描述")`，当检查条件不成立的时候，就会抛出异常。
    
3.  `assert`**命令一般用于程序员写程序**`debug`，因为它不能解释抛出异常的原因（比\*\*`require`**少个字符串）。它的用法很简单，**`assert(检查条件）`\*\*，当检查条件不成立的时候，就会抛出异常。
    
4.  `error`**方法**`gas`**最少，其次是**`assert`，`require`**方法消耗**`gas`**最多！因此，**`error`**既可以告知用户抛出异常的原因，又能省**`gas`，大家要多用！（注意，由于部署测试时间的不同，每个函数的\*\*`gas`\*\*消耗会有所不同，但是比较结果会是一致的。）
    

### 2\. payable关键字

在Solidity中，`payable`关键字用于标识可以接收以太币的**函数**或**地址**。只有带有`payable`关键字的函数才能接收以太币转账。默认情况下，地址类型是不可支付的，也就是说，你不能直接向一个普通的地址类型发送以太币。为了发送以太币，我们需要将地址转换为`payable`地址。这样可以防止意外转账到不支持接收以太币的函数，从而提高安全性。
...
Solidity 的变量类型划分非常清晰：

-   **数值类型 (Value Type)**：包括 `uint`, `int`, `bool`, `address`, `bytes32`, `enum` 等。它们在赋值时会进行**值拷贝**。
    
-   **引用类型 (Reference Type)**：包括 `struct`, `array`, `mapping` 等。它们处理的是数据的位置（如 `memory` 或 `storage`）。
    
-   **映射类型 (Mapping)** 和 **函数类型 (Function)**：也是 Solidity 的原生组成部分。
    

float不是数值类型，solidity不支持浮点数（float或double）。这是为了保证在所有区块链节点上计算结果的绝对确定性（浮点运算在不同硬件上可能会有微小差异，这会导致共识失败）。
...
-   **谁在转账？** `addr.transfer(1)` 是由**当前合约**发起的动作。
    
-   **转给谁？** 转给变量 `addr` 所代表的地址。
    
-   **转多少？** 在 Solidity 中，如果不带单位，默认数值的单位是 **wei**（以太坊最小单位）。1 ether = 10^{18} wei。
    
-   **为什么需要** `payable`**？** 只有标记为 `payable` 的地址类型才有 `.transfer()` 方法。
    

### 5\. msg

在 Solidity 中，`msg` 是一个非常特殊的**全局变量**，它是 **Message（消息）** 的缩写。每当有人调用合约函数时，以太坊虚拟机会自动填充这个变量的信息。

简单来说，`msg` 就像是每笔交易附带的“**身份证和货单**”。

你不需要在代码里定义 `msg`，它由以太坊系统**自动注入**。

-   每当一次“调用”发生，`msg` 就会根据当前这次通话的内容刷新。
    
-   **注意：** `msg` 的信息是**不可伪造**的。区块链节点会验证签名，确保 `msg.sender` 确实是发送这笔交易的人。
    

除了 `sender`，`msg` 还包含几个非常重要的属性：
...
今天稍微摆了一点~二刷了一些觉得很有价值的分享会以及web3 University的部分视频
...
在以太坊中，部署智能合约本质上是在区块链上“上传一段程序并初始化它的状态”，这个过程需要消耗资源，因此必须支付费用。我们可以从**消耗了多少资源**（Gas Used）和**每单位资源值多少钱**（Gas Price）两个维度来理解成本。

1.  **资源消耗量（Gas Used）—— 你在为哪些动作买单？**
    
    部署一个合约所需的 Gas 消耗，由以下几个“零件”组成：
    
    -   **基础交易费（Base transaction cost）**：这是所有以太坊交易的“起步价”，固定为 **21,000 Gas**。
        
    -   **合约创建开销（CREATE cost）**：这是为了在账本上新盖一座“自动售货机房子”而额外收取的费用，约为 **32,000 Gas**。
        
    -   **字节码存储费（Code deposit）**：你要把机器人的“大脑指令”（运行代码）永久存在链上，这需要按字数交钱，目前大约是**每 1 字节指令收 200 Gas**。
        
    -   **构造函数执行费（Constructor execution）**：机器人在“出生”那一刻执行的所有设置动作（比如贴标签、算初值），都会按指令消耗 Gas。
        
    -   **初始存储写入费（Storage initialization）**：这是最贵的一项，如果你在部署时第一次往机器人的“储物抽屉”里放东西，**每个抽屉大约要收 20,000 Gas**。
        
    
    **总结**：代码越长、初始逻辑越复杂、存入的数据越多，部署就越贵。
    
2.  **Gas 价格 —— 现在是什么收费机制？**
    
    自“伦敦升级”后，以太坊采用 **EIP-1559** 机制，Gas 的单价不再是一个死板的数字，而是由两部分组成的：Base Fee + Priority Fee
    
3.  **影响成本的关键因素**
    
    -   **合约复杂度**：指令越多、存储越多，成本越高。
        
    -   **网络拥堵**：在大家都在抢着交易的时候部署，基础费（Base Fee）会暴涨。
        
    -   **链的选择（L1 与 L2）**：
        
        -   在以太坊主网（L1）部署最贵。
            
        -   在 **Layer 2**（如 Arbitrum、Optimism）部署要便宜得多。
            
        -   特别是 **Dencun 升级（EIP-4844）** 之后，Layer 2 的数据成本下降了 50%–90%，现在的部署费用已经非常亲民。
            

**核心名词解释笔记：**

-   **Gas Used (资源使用量)**：机器人干活和存代码所消耗的“能量单位”。
    
-   **SSTORE (存储指令)**：把数据写进链上“抽屉”的动作，是整个过程里最烧钱的一步。
    
-   **Gwei**：Gas 价格的常用单位，1 ETH = 10^9 Gwei。
    

2.  **合约常见安全漏洞与防范措施**
    
    1.  **重入攻击 (Reentrancy) —— “无限分身的取款术”**
        
        这是以太坊历史上最著名的漏洞。
        
        -   **漏洞原理**：黑客在领钱时，趁着机器人（合约）还没来得及在账本上扣除他的余额，就利用一个“自动回拨”功能，让机器人不停地重复执行“领钱”这一步，直到把金库掏空。
            
        -   **名词解释——CEI 模式**：防范这种攻击的黄金准则。即：**先检查权限（Checks）**，**再修改账本（Effects）**，**最后才给钱（Interactions）**。如果先给钱再记账，黑客就有机会“重入”。
            
        -   **防范工具——防重入锁 (nonReentrant)**：给函数加一把锁，在第一次领钱还没结束前，禁止任何人再次进入这个函数。
            
    2.  **发送 ETH 的安全陷阱 —— “失效的旧邮票”**
        
        -   **背景**：过去程序员习惯用 `transfer()` 给别人转钱，它自带一个很小的“油费（2300 Gas）”补贴。
            
        -   **问题**：随着以太坊升级，干活的成本变高了，这 2300 Gas 像是一张过时的邮票，现在已经不够用了。如果对方是个复杂的机器人，转账就会失败，甚至导致资金被永久卡死。
            
        -   **防范方法**：现在推荐使用 `call` 这种更灵活的方式，并手动检查钱是否真的到账了。
            
    3.  **预言机操控 (Oracle Manipulation) —— “被蒙蔽的眼睛”**
        
        -   **名词解释——预言机 (Oracle)**：机器人（合约）无法直接看到外面的世界（比如现在 1 个 ETH 值多少美金），必须通过预言机这个“窗口”来获取数据。
            
        -   **漏洞原理**：如果窗口只有一家（单点预言机），黑客可以通过大额交易瞬间改变那一家的显示价格，骗机器人以极低的价格卖出宝贝。
            
        -   **防范方法**：使用 **Chainlink** 这种由多家机构共同提供的“去中心化预言机”，或者使用一段时间内的**平均价格 (TWAP)**，防止价格被瞬间操纵。
            
    4.  **权限与身份漏洞 —— “认错了主人”**
        
        -   **名词解释——tx.origin**：这是一个能看到“交易最初发起者”的变量。
            
        -   **漏洞原理**：黑客可以诱导你点一个诱饵合约，诱饵合约再去调你的金库。如果你用 `tx.origin` 来认主，金库会以为是你本人在操作，从而把钱给黑客。
            
        -   **防范方法**：永远使用 `msg.sender`（直接推门进来的人）来核对身份，并配合 **Ownable** 插件来管理管理员权限。
            
    5.  **随机数陷阱 —— “可以预测的骰子”**
        
        -   **漏洞原理**：如果你用“现在的区块高度”或“时间戳”来当骰子决定谁中奖，黑客可以算准那个时间点去投奖，因为这些数据对所有机器人都是透明的。
            
        -   **防范方法**：使用 **Chainlink VRF** 等专门生成的、不可预测的“真随机数”。
            
    6.  **逻辑检查的“自动检查员” (审计与测试)**
        
        为了防止人为疏忽，我们需要请这些“专业医生”来体检：
        
        -   **静态分析工具 (Slither)**：像扫描仪一样快速检查代码里有没有常见的安全隐患。
            
        -   **模糊测试 (Fuzzing/Foundry)**：通过给机器人投喂成千上万个乱七八糟的干扰指令（垃圾数据），看看机器人会不会在极端情况下出错。
            
        -   **专业审计 (Audit)**：聘请顶尖的安全专家团队进行人工逐行检查，这是高额资金协议上线前的必备步骤。
            
    
    **核心名词笔记：**
    
    -   **溢出检查**：Solidity 0.8.0 之后自动具备的本领，防止数字算错（比如 0 减 1 变成超级大的数）。
        
    -   **SafeERC20**：与代币打交道时的“防护服”，处理那些不听话、不按标准回话的旧代币。
        
3.  **合约部署后的公开性与可审计性**
    
    1.  **合约的公开性 —— “玻璃房子”效应**
        
        智能合约部署后，它的所有信息都是对外展示的，没有任何秘密：
        
        -   **字节码公开 (Bytecode Public)**
            
        -   **交易历史公开 (Transaction History Public)**
            
        -   **状态变量半公开 (State Variables Semi-Public)**
            
        
        以太坊上的“隐私”只是接口层面的限制，在数据层面，**链上没有真正的秘密**。
        
    2.  **合约的可审计性 —— “安全体检”机制**
        
        -   正因为合约是透明的，大家就可以给它做各种各样的安全检查，这种能力叫作**可审计性**。
            
            -   开发者会把合约的**源代码**（人类能读懂的 Solidity 代码）公开在区块浏览器或像 **Sourcify** 这样的去中心化验证服务平台上。
                
            -   系统会自动比对这份源代码编译出来的结果是否和链上的机器码完全一样。一旦验证通过，大家就能像看说明书一样看到这个合约的真实逻辑，从而建立信任。
                
        -   **人工审计 (Manual Review)**
            
        -   **自动化工具审计 (Automated Tools)**
    ...
    **核心名词总结：**
    ...
    -   **区块浏览器 (Block Explorer)**：专门用来查询区块链上合约、交易和余额的**搜索网站**。
        
    -   **存储位 (Storage Slot)**：合约用来存数据的“抽屉编号”，分析它就能读出里面的隐藏数据。
        
    -   **源码验证 (Source Verification)**：证明你公开的文字版代码和链上运行的机器代码**一模一样**的过程。
        
    -   **静态分析 (Static Analysis)**：不运行程序，仅通过扫描代码文字来找问题的**快速检查法**。
        
4.  **合约逻辑的修改与升级模式**
    
    在以太坊中，智能合约一旦部署到区块链上，它的代码就是**不可篡改**的。这虽然保证了安全，但也让修复 Bug 或增加新功能变得困难。为了解决这个问题，开发者们设计了几种“曲线救国”的升级方案：
    
    1.  **简单替换法 —— “搬家式升级”**
        
        这是最直接但最麻烦的方法。
        
        -   **操作方式**：如果你发现旧机器（旧合约）坏了，就重新在另一个门牌号（新地址）部署一台新机器，然后告诉所有人：“以后请去新地址办事”。
            
        -   **名词解释——状态迁移**：因为新机器里没有旧机器的数据，你需要写个脚本把旧机器里的余额、记录等一个个“搬”到新机器里。
            
        -   **优缺点**：实现很简单，但**地址变了**，所有人都要更新联系方式，搬数据也非常费钱费力。
            
    2.  **行业主流：代理模式 (Proxy Pattern) —— “不搬家的装修”**
        
        这是目前绝大多数 DeFi 或 NFT 项目使用的标准升级方法。它的核心思想是将“存放数据的办公室”和“执行任务的业务员”分开。
        
        -   **代理合约 (Proxy)**：这是永远不变的**办公室**。
            
            -   **固定的地址**：用户只需要记住这个地址，永远不搬家。
                
            -   **存放账本**：所有的余额、等级等数据都锁在办公室的保险柜（存储空间）里。
                
        -   **实现合约 (Implementation / Logic)**：这是被雇佣的**业务员**。
            
            -   **编写规则**：这里写着怎么算账、怎么执行逻辑。
                
            -   **可被替换**：如果规则需要变，办公室不搬家，只是**换一个新的业务员**。
                
        -   **名词解释——delegatecall（委托调用）**：代理合约通过这个“魔法电话”呼叫逻辑合约，让逻辑合约拿着它的规则到代理合约的办公室里干活，修改的是代理合约保险柜里的账本。
            
    3.  **模块化与钻石标准 (Diamond Standard) —— “万能拼装机”**
        
        当合约非常复杂时，一个业务员忙不过来，就需要把功能拆开。
        
        -   **EIP-2535 钻石标准**：就像一个巨大的机器人，它的手、脚、头都是**可拆卸的插件**（逻辑模块）。
            
        -   **作用**：你可以只升级“左手”的功能，而不影响其他部位。这适合像“城市级”规模的超大型协议。
            
    4.  **失效的黑魔法：销毁重建 —— “原地换壳”**
        
        过去曾有一种利用 `SELFDESTRUCT`（自毁指令）把合约删掉，再在原位盖新房的套路。
        
        -   **现状**：在 2024 年以太坊升级（EIP-6780）后，这种\*\*“原地换壳”的方法已经基本失效\*\*，且非常不安全，目前已不再被推荐使用。
            
    5.  **升级的“防盗门”：管理权限控制**
        
        既然逻辑可以改，那万一坏人偷偷改了逻辑把钱转走怎么办？因此升级必须配合这几个零件：
        
        -   **多签钱包 (Multisig)**：必须由 3 个或 5 个管理员同时签名同意，才能更换“业务员”。
            
        -   **时间锁 (Timelock)**：发出升级指令后，必须等待 24 或 48 小时才能生效，给用户检查代码和撤资逃跑的时间。
            
    
    关于“代理合约” 和 “实现合约”：
    
    -   “代理合约”和“实现合约”是分别部署在两个不同的区块链地址上的
        
        -   **代理合约（Proxy）的地址 —— “永远不变的门牌号”**： 这是用户唯一需要记住的地址。无论项目升级多少次，对外展示的永远是这个地址。它就像是政府办公大楼的**固定地址**，不管里面的工作人员怎么换，大家办事都得去这个地方。
            
        -   **实现合约（Implementation）的地址 —— “流动的工作证号”**： 它也有自己的地址，但它躲在代理合约后面，**不直接和用户打交道**。它只负责存放“逻辑代码”（即办事规则）。（所以实现合约有时也叫逻辑合约（logic contract））
            
    -   **为什么非要放在两个地址？**
        
        因为以太坊的合约代码一旦部署就**不可篡改**。
        
        -   如果你把逻辑和数据都写在一个地址里，发现 Bug 时你就没法改了。
            
        -   采用“双地址”模式后，代理合约里存着一个\*\*“指向实现合约地址”的变量（指针）\*\*。
            
    -   **升级时地址发生了什么变化？**
        
        当你想要给合约加新功能或修 Bug 时，流程如下：
        
        -   **部署新员工**：编写新的逻辑代码，部署到一个**全新的地址**上（实现合约 V2）。
            
        -   **修改指针**：管理员给代理合约发个指令，把里面存的那个“员工地址”从 V1 改成 V2。
            
        -   **结果**：用户访问的依然是原来那个**固定的代理合约地址**，但由于代理合约内部的指针变了，它现在会跑去 V2 的地址读取最新的规则来干活。
            
    -   **关键的技术魔法：**`delegatecall`
        
        虽然它们在不同地址，但代理合约会通过一个叫 **delegatecall（委托调用）** 的特殊指令来联络实现合约。
        
        -   这个指令的神奇之处在于：它让实现合约的规则跑到代理合约的地址空间里来运行。
            
        -   这意味着，虽然逻辑是在“实现合约”那学的，但**账本（余额、数据）永远记在“代理合约”自己的档案柜里**。
            
    
    **核心名词总结：**
    
    -   **不可篡改性 (Immutability)**：区块链代码部署后无法编辑，这是升级难题的根源。
        
    -   **代理模式 (Proxy Pattern)**：保持地址不变，通过切换后台逻辑合约来实现功能更新的主流方案。
        
    -   **实现合约 (Implementation)**：存放真正业务逻辑代码的那个合约，可以被随时替换。
        
    -   **delegatecall**：代理模式的核心技术，让外部逻辑在自己的存储环境下运行。
        
        **总结笔记：** 合约逻辑的“修改”本质上是**指向的切换**。**代理模式**是目前的标准答案：**地址和数据锁死在 Proxy 中，逻辑通过更换 Implementation 来升级**。为了安全，升级过程必须由**多签**和**时间锁**严密把关
