---
timezone: UTC+8
---

# dl4987638

**GitHub ID:** dl4987638

**Telegram:** @umu_n1

## Self-introduction

Web3 实习计划 2025 冬季实习生

## Notes

<!-- Content_START -->
# 2026-02-07
<!-- DAILY_CHECKIN_2026-02-07_START -->
今日計畫 uniswap v2 代碼審視
<!-- DAILY_CHECKIN_2026-02-07_END -->

# 2026-02-06
<!-- DAILY_CHECKIN_2026-02-06_START -->

今日計畫 繼續練習dapp開發流程
<!-- DAILY_CHECKIN_2026-02-06_END -->

# 2026-02-05
<!-- DAILY_CHECKIN_2026-02-05_START -->


今日計畫 進行foundry跟練
<!-- DAILY_CHECKIN_2026-02-05_END -->

# 2026-02-04
<!-- DAILY_CHECKIN_2026-02-04_START -->



今日計畫 熟悉hardhat
<!-- DAILY_CHECKIN_2026-02-04_END -->

# 2026-02-03
<!-- DAILY_CHECKIN_2026-02-03_START -->




今日計畫: 詳讀uniswap合約
<!-- DAILY_CHECKIN_2026-02-03_END -->

# 2026-02-02
<!-- DAILY_CHECKIN_2026-02-02_START -->





今日計畫 完成gas優化實例以及熟悉remix
<!-- DAILY_CHECKIN_2026-02-02_END -->

# 2026-02-01
<!-- DAILY_CHECKIN_2026-02-01_START -->






今日計畫:學習erc標準
<!-- DAILY_CHECKIN_2026-02-01_END -->

# 2026-01-31
<!-- DAILY_CHECKIN_2026-01-31_START -->







今日計畫完成任務 gas優化案例
<!-- DAILY_CHECKIN_2026-01-31_END -->

# 2026-01-30
<!-- DAILY_CHECKIN_2026-01-30_START -->








今日計畫 設定自己的mcp agent
<!-- DAILY_CHECKIN_2026-01-30_END -->

# 2026-01-29
<!-- DAILY_CHECKIN_2026-01-29_START -->









今日計畫 繼續研讀uniswap
<!-- DAILY_CHECKIN_2026-01-29_END -->

# 2026-01-28
<!-- DAILY_CHECKIN_2026-01-28_START -->










今日計畫 詳讀uniswap代碼
<!-- DAILY_CHECKIN_2026-01-28_END -->

# 2026-01-26
<!-- DAILY_CHECKIN_2026-01-26_START -->











今日計畫:完成 2 章 Unphishable 钓鱼攻防挑战

聆聽完投研分享會後自行整理筆記
<!-- DAILY_CHECKIN_2026-01-26_END -->

# 2026-01-25
<!-- DAILY_CHECKIN_2026-01-25_START -->













今日計畫:優化個人簡歷

詳讀良心殺

複習dapp workshop
<!-- DAILY_CHECKIN_2026-01-25_END -->

# 2026-01-24
<!-- DAILY_CHECKIN_2026-01-24_START -->














今日計畫:複習昨日的Workstation並繼續閱讀uniswap v2代碼
<!-- DAILY_CHECKIN_2026-01-24_END -->

# 2026-01-23
<!-- DAILY_CHECKIN_2026-01-23_START -->















今日計畫:

繼續閱讀Erc-7962相關內容
<!-- DAILY_CHECKIN_2026-01-23_END -->

# 2026-01-22
<!-- DAILY_CHECKIN_2026-01-22_START -->
















今日計畫:

複習solidity語法，複習完後進行uniswap代碼閱讀
<!-- DAILY_CHECKIN_2026-01-22_END -->

# 2026-01-21
<!-- DAILY_CHECKIN_2026-01-21_START -->

















今日計畫:詳細閱讀erc-7962相關文件

並複習solidity智能合約內容
<!-- DAILY_CHECKIN_2026-01-21_END -->

# 2026-01-20
<!-- DAILY_CHECKIN_2026-01-20_START -->


















今日目標:詳細閱讀zk 投票相關技術
<!-- DAILY_CHECKIN_2026-01-20_END -->

# 2026-01-19
<!-- DAILY_CHECKIN_2026-01-19_START -->



















今日於my first nft上了解了nft的機制和購買原則並最後透過自己實操來鍛造自己的第一個nft
<!-- DAILY_CHECKIN_2026-01-19_END -->

# 2026-01-18
<!-- DAILY_CHECKIN_2026-01-18_START -->




















今日繼續閱讀導章
<!-- DAILY_CHECKIN_2026-01-18_END -->

# 2026-01-16
<!-- DAILY_CHECKIN_2026-01-16_START -->





















-   **CodeHash**：EOA 为空哈希，CA 存储合约字节码的哈希值。
    

**CodeHash** 是账户关联代码的 **Keccak-256 哈希值**，用于标识该账户是否包含可执行代码

**EOA (Externally Owned Account) - 外部拥有账户**

-   **定义**：由私钥控制的普通用户账户（如 MetaMask 钱包地址）
    
-   **CodeHash 值**：
    

```
  keccak256("") = 0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470
```

这是**空字符串的哈希**，表示「没有代码」

-   **特点**：
    
    -   可以发起交易（签名后广播）
        
    -   **不能执行智能合约代码**
        
    -   没有存储槽位（storage）
        

**CA (Contract Account) - 合约账户**

-   **定义**：部署在链上的智能合约地址
    
-   **CodeHash 值**：
    

```
  keccak256(合约字节码) = 例如：0x1a2b3c4d...（唯一哈希值）
```

这个哈希对应合约的 **EVM 字节码**（bytecode）

-   **特点**：
    
    -   **不能主动发起交易**（没有私钥）
        
    -   只能被 EOA 或其他合约调用时执行
        
    -   有自己的存储空间（storage）
        

ipo上鏈一次約1.8M美金
<!-- DAILY_CHECKIN_2026-01-16_END -->

# 2026-01-15
<!-- DAILY_CHECKIN_2026-01-15_START -->






















今日目標

區塊鏈崗位以及智能合約開發部分 閱讀

今天演講中得知可以去試spoonOS(自主智能體)

![image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/dl4987638/images/2026-01-15-1768481330858-image.png)

有分前端後端

智能合約工程師

技術崗位以外的部分:

非技术岗的工作，适合没有计算机背景的同学们。

### [**1\. 产品与运营**](https://web3intern.xyz/zh/position-introduction/#_1-%E4%BA%A7%E5%93%81%E4%B8%8E%E8%BF%90%E8%90%A5)

1.  职位描述：
    
    -   在 Web3 产品生命周期中负责协调产品发布、用户反馈收集和持续改进流程，以提升用户体验和产品迭代效率。
        
    -   执行以用户获取、留存和参与度提升为目标的增长战略，并监控实施效果。
        
    -   与产品、技术、市场及合规团队紧密合作，确保产品上市（go-to-market）策略与各部门需求保持一致。
        
    -   持续分析运营数据，跟踪关键绩效指标（KPIs），并根据数据提出优化建议。
        
2.  职位要求：
    
    -   熟悉产品上线（Go-to-market）全流程，擅长跨部门资源协调与项目推进。
        
    -   具备扎实的数据分析能力，能熟练使用 SQL、Excel 或其他数据工具进行数据统计和洞察提炼。
        

### [**2\. 社区管理**](https://web3intern.xyz/zh/position-introduction/#_2-%E7%A4%BE%E5%8C%BA%E7%AE%A1%E7%90%86)

1.  职位描述
    
    -   构建并管理 Telegram、Twitter（X）、Discord 等社交平台的社区，实现持续的用户互动与增长；
        
    -   组织线上 AMA（问答）、活动、竞赛等形式多样的社区互动，以提升用户活跃度和品牌粘性；
        
    -   跟踪社区健康度指标和情感分析，定期向管理层汇报洞察与优化建议；
        
    -   与营销团队协作，制定并执行内容日历，发布社区公告和运营手册。
        
2.  职位要求
    
    -   Web3、DAO 或 NFT 社区管理经验，深刻理解去中心化应用生态；
        
    -   出色的文案撰写与沟通能力，能够有效引导社区讨论并快速响应用户反馈；
        
    -   熟练使用社区数据分析工具，能够监测并解读用户行为与舆情动态；
        
    -   具备活动策划与执行能力，能够独立组织线上及线下社区活动。
<!-- DAILY_CHECKIN_2026-01-15_END -->

# 2026-01-14
<!-- DAILY_CHECKIN_2026-01-14_START -->

























今日閱讀行業賽道總攬以及

Web3工作形式

okr範例:

1.  **目标设定原则**
    
    -   **聚焦关键目标**：每季度设定 3-5 个目标，避免分散精力。
        
    -   **野心与可行平衡**：目标应“令人不适但可达”，理想完成率在 60%-70%（Google 标准）。
        
    -   **结果导向**：聚焦成果而非产出（例：“提升客户留存率” 而非 “发送 10 份调研”）。
        
2.  **关键结果（KR）设计**
    
    -   **递进式量化**：避免二进制（完成/未完成），采用里程碑式量化：
        
        目标：提升产品国际化能力
        
        KR1：完成 3 个语言版本开发（40%）
        
        KR2：用户测试通过率 ≥90%（70%）
        
        KR3：新市场用户增长 25%（100%）
        
    -   **SMART 标准**：确保 KR 可衡量、有时限（例：“Q3 北美市场营收增长 15%”）。
        
3.  **评分与复盘**
    
    -   **评分规则**：5 分（卓越贡献）→ 1 分（完成度 ＜ 50%）；
        
    -   **复盘要点**：分析低分 KR 原因（目标过高/资源不足），高分则反思挑战性是否不足。
        
    
    _表：OKR 评分标准参考_
    
    | 得分 | 完成度 | 意义 |
    | --- | --- | --- |
    | 1.0 | 100%+ | 目标过于简单 |
    | 0.7 | 70% | 理想挑战水平 |
    | 0.3 | 30% | 需重新规划策略 |
    
4.  **智能合约团队 OKR 示例**
    
    **Objective** : 提升协议安全性至机构级标准
    
    **Key Results** :
    
    -   通过 Certora 形式化验证（主网部署前）
        
    -   漏洞赏金计划覆盖率 100%（Immunefi 平台）
        
    -   平均审计发现项 ≤2 个（对比 OpenZeppelin 基准）
        

行業術語

### [**1\. 基础类**](https://web3intern.xyz/zh/remote-work-guide/#_1-%E5%9F%BA%E7%A1%80%E7%B1%BB)

| 黑话 | 含义 |
| --- | --- |
| DYOR | Do Your Own Research，投资前请自行研究，项目方常用于免责 |
| FOMO | Fear of Missing Out，害怕错过，指因贪婪而追高的情绪 |
| FUD | Fear, Uncertainty, Doubt，恐慌、不确定、怀疑，指唱衰情绪或舆论攻击 |
| WAGMI | We're All Gonna Make It，大家都会发财，社区常用打气口号 |
| NGMI | Not Gonna Make It，讽刺某人/项目做法不行 |
| REKT | 被"爆锤"，损失惨重，如投资失败、合约被黑等 |
| Airdrop | 空投，免费发放代币以激励用户参与项目 |
| Whale | 加密大户、巨鲸，能影响市场价格的人 |
| Degen | "投机狗"，不问项目质量只冲高风险高回报机会的人 |
| Shill | 宣传、推销（常含贬义），如"shill 项目"指恶意安利 |
| Exit Scam | 项目方跑路，携款失联 |

### [**2\. 技术类**](https://web3intern.xyz/zh/remote-work-guide/#_2-%E6%8A%80%E6%9C%AF%E7%B1%BB)

| 黑话 | 含义 |
| --- | --- |
| L1 / L2 | Layer 1（主链，如以太坊）和 Layer 2（扩容方案，如 Arbitrum、zkSync） |
| EVM | Ethereum Virtual Machine，以太坊虚拟机，运行智能合约的核心 |
| Smart Contract | 智能合约，自动执行合约逻辑的链上程序 |
| Bridge | 跨链桥，用于在多条链之间转移资产 |
| Oracle | 预言机，用于向链上提供链下数据 |
| Fork | 分叉，指复制已有代码的项目，或区块链协议分裂 |
| Mint | 铸造，生成新的 NFT 或代币 |
| Burn | 销毁代币，减少总量，用于通缩机制 |
| Rugpull | 抽地毯，项目方卷款跑路（尤其在 DeFi 项目） |

### [**3\. 投资类**](https://web3intern.xyz/zh/remote-work-guide/#_3-%E6%8A%95%E8%B5%84%E7%B1%BB)

| 黑话 | 含义 |
| --- | --- |
| Pump | 拉盘，代币价格快速上涨 |
| Dump | 砸盘，代币价格快速下跌 |
| HODL | 原为"Hold"打错，后来变成文化，意为坚定持币不卖 |
| Bagholder | "接盘侠"，高位买入亏损后长期持币的人 |
| Alpha | 内部消息/潜在机会，表示未公开但价值潜力大的信息 |
| Tokenomics | 代币经济学，研究代币发行、分配、用途等机制 |

### [**4\. 社区与文化类**](https://web3intern.xyz/zh/remote-work-guide/#_4-%E7%A4%BE%E5%8C%BA%E4%B8%8E%E6%96%87%E5%8C%96%E7%B1%BB)

| 黑话 | 含义 |
| --- | --- |
| gm / gn | good morning / good night，Web3 社区日常打招呼方式 |
| Anon | 匿名者，社区中不透露真实身份的成员 |
| KOL | Key Opinion Leader，意见领袖，影响力人物 |
| CT | Crypto Twitter，加密行业活跃的信息与讨论来源 |
| DAO | 去中心化自治组织，社区驱动型的组织管理形式 |
| NFT PFP | NFT 头像项目，如 CryptoPunks、BAYC，PFP = Profile Picture |

### [**5\. 开发类**](https://web3intern.xyz/zh/remote-work-guide/#_5-%E5%BC%80%E5%8F%91%E7%B1%BB)

| 黑话 | 含义 |
| --- | --- |
| Hardhat / Foundry | 常用的智能合约开发框架 |
| RPC | Remote Procedure Call，链上节点访问接口 |
| Gas | 交易费用，以太坊中以 Gwei 计费 |
| Etherscan | 区块浏览器，用于查看合约、交易、地址信息 |
| Viem / Ethers.js / Wagmi | Web3 前端交互库 |
| ABI | Application Binary Interface，合约调用所需的接口信息 |
| Mainnet / Testnet | 主网 / 测试网，分别对应正式和测试环境 |
<!-- DAILY_CHECKIN_2026-01-14_END -->

# 2026-01-13
<!-- DAILY_CHECKIN_2026-01-13_START -->


























今日學習學習規劃 繼續閱讀區塊練入門導讀的以太訪概括

問:

the merge發生時 所謂插接是什麼?

只有乙太是有分共識層跟執行層嗎?還是其他鏈也有

詳細了解pos工作方式 ，懲罰機制中何謂作惡者

什麼是Blob 交易

答:

**“插接”(Docking)** 是指 The Merge 時，信標鏈(共識層)與原有的以太坊主網(執行層)進行**技術耦合**的過程。這是一個**無縫切換機制**，涉及多個關鍵技術組件：

### 1\. **Terminal Total Difficulty (TTD) 觸發機制**

-   設定一個預定的累積難度閾值：**58750000000000000000000**
    
-   當 PoW 鏈達到這個難度值時，自動觸發合併
    
-   這是最後一個 PoW 區塊，之後所有區塊由 PoS 產生
    
-   使用難度值而非區塊高度，避免時間預測偏差導致的協調問題
    

### 2\. **Engine API 接口層**

-   **JSON-RPC API**，實現共識層與執行層的雙向通信
    
-   關鍵方法：
    
    -   `engine_newPayloadV1`: 共識層發送執行負載給執行層
        
    -   `engine_forkchoiceUpdatedV1`: 通知執行層更新分叉選擇
        
    -   `engine_getPayloadV1`: 共識層請求執行層構建新區塊
        
-   這是**模塊化架構**的核心，允許共識客戶端和執行客戶端獨立開發
    

### 3\. **執行負載(Execution Payload)封裝**

-   原本由 PoW 礦工構建的區塊內容，現在被包裝成 **ExecutionPayload** 結構
    
-   包含：transactions, state\_root, receipts\_root, logs\_bloom, gas\_used 等
    
-   信標鏈的每個 BeaconBlock 現在嵌入一個 ExecutionPayload
    
-   **區塊結構雙層化**：BeaconBlock(共識數據) + ExecutionPayload(執行數據)
    

### 4\. **客戶端配對機制**

-   節點必須同時運行：
    
    -   **共識客戶端**(Prysm, Lighthouse, Teku, Nimbus, Lodestar)
        
    -   **執行客戶端**(Geth, Nethermind, Besu, Erigon)
        
-   通過 **JWT(JSON Web Token)** 認證確保兩者安全通信
    
-   本地通過 HTTP/IPC 連接，端口通常是 8551
    

### 5\. **狀態連續性保證**

-   執行層的完整歷史狀態保持不變(世界狀態 World State)
    
-   賬戶餘額、智能合約代碼、存儲槽位完全延續
    
-   **只有共識機制切換**，應用層無感知
    

### 6\. **難度炸彈(Difficulty Bomb)配合**

-   預先埋入的指數級難度增長機制
    
-   確保 PoW 礦工在 TTD 後無利可圖，無法維持舊鏈
    
-   經濟上強制遷移到 PoS
    

多條乙太以外的鏈如cosmos、Polkadot也有分

solana是單體
<!-- DAILY_CHECKIN_2026-01-13_END -->

# 2026-01-12
<!-- DAILY_CHECKIN_2026-01-12_START -->





























今日完成事項

聆聽以太訪周會

參加co-learning

完成入门导读部分学习

複習了區塊鏈相關概念

學到新詞聯盟鏈

首次得知了web3.0與web3之差

web3.0:

-   **语义标记**：使用 RDF、OWL 等标准描述数据关系
    
-   **结构化数据**：信息按照标准格式组织，便于机器理解
    
-   **知识图谱**：构建实体间的语义关系网络
    
-   **典型技术**：本体工程、语义查询语言（SPARQL）、链接数据
    
-   ❌ **不是区块链技术**，而是传统互联网的数据组织升级
    
-   ❌ **主要不依赖 AI**，而是通过标准化数据格式实现
    
-   ✅ 与 Web3 可结合（语义标记 + 区块链存储）
    

web3:

-   **数据主权归用户**：用区块链存储身份和资产
    
-   **无需信任中介**：智能合约自动执行规则
    
-   **真正拥有数字资产**
<!-- DAILY_CHECKIN_2026-01-12_END -->
<!-- Content_END -->
