...
> **“在当前区块高度 N 下，Order#1 的状态是 Filled”**

也就是说，**它不是在改数据，而是在声明状态**。

* * *

## 换一种你更容易理解的说法

你可以把 Subgraph 数据库想象成：
...
Subgraph 实际维护的是两类信息（内部概念）：

* * *

## 1️⃣ 当前状态（Latest State）
...
> **“在最新确认区块下，这个 Entity 的状态”**

* * *

## 2️⃣ 历史可回滚状态（Block-aware storage）

为了支持 Reorg，Indexer 内部会额外保存：
...
⚠️ 注意：  
**你查询不到 block 100 的状态**，但 Indexer **必须知道它存在过**。

* * *

# 四、为什么 Indexer 能“回滚数据库”？
...
👉 **这就是为什么不能用普通监听器做 Indexer**

* * *

## 3️⃣ Indexer 为什么不怕 Reorg？（关键）
...
-   回滚所有 **block > B** 的 Entity 修改
    
-   把数据库恢复到 **B 区块执行完之后的状态**
...
> **Indexer 不是监听器，也不是爬虫**
> 
> **Indexer 是一个“确定性区块重放引擎”**

这句话非常重要，你先记住，后面所有细节都是围绕它展开的。
...
## 2️⃣ Subgraph 必须满足一个硬条件

> **给定同一条链 + 同一份代码 → 得到完全一致的数据**
...
# 四、Indexer 是如何“顺序处理区块”的

## 1️⃣ Indexer 不关心交易，只关心 Event

在一个区块里：

-   有 N 笔交易
    
-   每笔交易可能 emit 多个 Event
...
-   读 event.block.timestamp
    
-   读 event.transaction.from
...
它不是 SQL 的 `INSERT` 或 `UPDATE`。

真实语义是：

> “在这个区块高度下，这个 Entity 的状态是这样”
...
这就引出一个**非常重要但很少人讲清楚的问题**。

* * *

# 七、区块重组（Reorg）时，Indexer 在干什么？
...
> **链上只做“不可替代的事”，其余全部下沉到链下**

* * *

# 二、第一层：合约判断 → storage（这是“权力中心”）

## 1️⃣ storage 在 DApp 中的唯一职责
...
**所有“if / require / revert”只依赖 storage**

* * *

## 2️⃣ storage 不应该干什么（重点）
...
> **storage 是“判断引擎”，不是数据库**

* * *

## 3️⃣ 正确示例（判断型 storage）
...
function fillOrder(uint256 id) external {
    ...
    require(msg.sender != o.owner, "self fill");
...
# 四、第三层：历史 / 列表 → Subgraph（这是“数据库”）

## 1️⃣ Subgraph 在整个系统中的定位

Subgraph 是：

> **“基于 event 的、可重建的、确定性的数据库视图”**
...
这其实就是 **CQRS 思想（读写分离）**，Web3 天然需要它。

* * *

# 五、第四层：UI 展示 → GraphQL（前端的“后端”）

## 1️⃣ GraphQL 在这里的真实角色
...
## 2️⃣ 为什么前端不直接读 Subgraph 数据库？

因为：

-   Subgraph 对外只暴露 GraphQL
...
> **前端不应该直接扫链，也不应该自己维护区块高度**

* * *

# 六、第五层：实时提示 → event.on（“用户体验层”）
...
```
contract.on("OrderFilled", (id, taker) => {
  toast("你的订单已成交");
});
```
...
# 七、把五层合在一起（真实 DApp 的“脑内运行图”）

你以后写 DApp，脑子里要自动运行这张图：
...
# 二、第二层：**钱包不是“登录工具”，而是“权力源头”**

这是整句话里**最容易被低估的一环**。

* * *

## 1️⃣ 钱包在 DApp 架构中的真实地位
...
钱包、协议、前端不会“读你的文章内容”，它们只检查：

> **你是不是在说“标准英语”**

* * *

## 3️⃣ 为什么实际开发几乎一定要 `import IERC`？
