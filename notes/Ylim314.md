...
今天是实习计划第三周的周三，晚上的分享会请来了 RootData 的 CPO Ye，专门讲《Web3 数据分析基础》。对于咱们马上要开始的黑客松比赛来说，这节课其实就是在教我们如何用数据给项目“撑腰”。讲师一上来就点破了核心逻辑：做项目不能只凭感觉，必须得懂链上数据的门道，特别是要把交易数据、区块数据和合约逻辑结合起来看，这样才能抓住痛点。

针对黑客松最关心的“如何证明项目价值”这个问题，笔记里重点提到了三个北极星指标。首先是 TVL（总锁仓量），这是衡量 DeFi 项目市场占有率的金标准；其次是活跃地址数，它能帮你剥离掉那些虚假的繁荣，证明你的项目有真实用户在使用；最后是协议收入，这是最硬核的商业造血能力证明。在比赛的路演环节，如果能拿出这几项数据的对比分析，或者清楚地展示你的项目未来如何优化这些指标，无疑会给评委留下深刻印象。

在实战工具的选择上，今天的核心建议就是“别重复造轮子”。黑客松时间紧迫，Ye 建议我们直接利用现成的“军火库”：做赛道调研和竞品分析时，直接去 RootData 查投融资背景；需要全网的宏观数据如 TVL 和收入时，DeFiLlama 是最权威的来源；如果需要更定制化的链上数据分析，就用 Dune 写 SQL 来实现。只有当这几个工具都满足不了需求，或者你要做一个专门的数据看板产品时，才需要去折腾底层的 ETL 流程，通过 ABI 去解码那些复杂的链上十六进制数据。简而言之，善用工具，把精力花在核心业务逻辑的创新上，才是黑客松的制胜之道。
...
第三周正式开启，今晚 0xhardman 的分享让我对“写代码”这件事有了全新的认知。如果说前两周是在学“怎么造砖（Solidity语法）”，那这一周的主题就是“怎么用 AI 指挥机器造楼”。

### 1\. 黑客松的本质：高压下的 MVP 交付

PPT 里有一句话很触动我：**“黑客松是组建一个微型互联网公司，在几天内爆肝一个产品。”**

-   **纠偏**：以前我觉得黑客松就是比谁代码写得难。但在 ETHGlobal 这种顶级赛场，**Idea (创意) + Completeness (完成度) > Tech Stack (技术栈)**。
    
-   **流程映射**：从 Stake（质押倒逼承诺）到 Check-in，再到 Pitch，这完全是一次创业的微缩演习。作为工程师，我不能只盯着合约，必须要有产品经理的视角。
    

### 2\. Vibe Coding：AI 时代的编程新范式

这是今天最大的收获。0xhardman 提出的 **Vibe Coding（氛围编程）** 概念，完全颠覆了传统的开发流。

-   **定义**：自己一行代码都不看，全靠自然语言操作 AI 写代码。
    
-   **工具演进**：
    
    -   **L1 (Chatbot)**: 复制粘贴代码给 GPT（效率低）。
        
    -   **L2 (AI IDE)**: Cursor / Windsurf（当前主流，感知上下文）。
        
    -   **L3 (TUI 编程)**: **这是未来。** 在 Terminal 里直接与 AI 交互，引入 MCP (Model Context Protocol) 和 Agent。
        
-   **我的思考**：对于区块链开发，环境配置（Hardhat/Foundry）和前端集成（Wagmi）往往最耗时。利用 Vibe Coding，我可以把 80% 的体力活（写 Boilerplate、写测试用例）交给 AI，自己只负责那 20% 的**核心逻辑设计**和**安全审计**。
    

### 3\. 工程策略调整

结合 PPT 的建议，我决定在接下来的黑客松中调整战术：

-   前端：不再手撸 CSS，尝试用 AI IDE 直接生成组件。
    
-   合约：用自然语言描述业务逻辑，让 AI 生成 Solidity 初稿，我主要负责 Gas 优化 和 漏洞审查。
    
-   工具栈：立刻去研究 PPT 推荐的 TUI 工具（如 Claude 的终端集成），争取做第一批吃螃蟹的人
...
今天是第二周的收官日，研读了同学分享的《Reactive 合约部署指南》，感觉打开了新世界的大门。如果说之前的 Solidity 学习是造“静态的自动贩卖机”，那么 Reactive Network 就是给贩卖机装上了“机械臂”和“传感器”。

-   以前我认为智能合约是万能的，但其实它们是\*\*被动\*\*的。没有外部 EOA（外部账户）发起交易，合约就是死的代码，无法自动执行。
    
-   **传统方案**：为了实现“定投”或“止损”，我们通常跑一个 Python 脚本（Web2 服务器）去轮询链上数据。但这引入了中心化风险——服务器断电，止损失败。
    
-   **Reactive 方案**：实现了 **Event-Driven Architecture**。通过监听链上 Event（如 `PriceUpdated`），去中心化地触发 Callback 合约。
    

2\. 工程实战：Uniswap V2 止损机器人

教程中的 Demo 非常硬核，模拟了一个去中心化的止损场景：

-   监听 ：Reactive 节点监听 Uniswap V2 Pair 的 `Sync` 或自定义价格事件。
    
-   触发 ：当 `token0/token1` 价格跌破阈值，Reactive 自动发起一笔交易调用 `callback`。
    
-   执行 ：Callback 合约执行 `swap` 操作，将波动资产换成稳定币。
    
-   工具链：不仅复习了 Solidity，还高频使用了 Foundry进行命令行交互，这才是工程师该有的开发姿势。
    

3\. 架构思考：AI + Reactive 的终极形态

结合我关注的 AI 方向，我看到了巨大的组合性

-   Brain (AI)：链下 AI 模型分析宏观舆情，预测市场趋势，动态调整“止损阈值”。
    
-   Hand ：Reactive 合约负责在链上无信任地执行 AI 下达的策略。
    
-   AI Agent 负责决策，Reactive Network 负责执行。这将彻底取代传统的中心化量化机器人。
...
1.  基础设施 ：
    ...
    会议提到的 Pectra 升级 是 DePIN 赛道的催化剂。
    
    -   逻辑：节点只需验证 1/128 的碎片数据即可确认完整性。
        
    -   应用：这意味着光伏逆变器等 IoT 设备 有机会直接运行轻节点，无需依赖中心化网关，真正实现边缘计算的可信上链。
        
2.  隐私合规 ：
    
    Tornado Cash 的案例表明，纯粹的“黑盒隐私”在工程上已是死路。未来的架构必须转向 ZK-Based Compliance，即在保护用户数据的同时，允许向监管方出示 View Key。
    
3.  工具论 ：
    
    知识库管理本质上是数据工程的 ETL 流程。利用 AI Agent 清洗“孤儿笔记”并建立索引，这不仅是整理资料，更是训练自己的“第二大脑”模型。
...
今天的分享会意外地硬核。本以为是讲怎么做笔记，结果讲的是\*\*“知识工程学 (Knowledge Engineering)”\*\*。作为正在研究 AI Agent 的开发者，我发现 Obsidian + AI 的组合简直就是为程序员量身定制的。

## 1\. 核心范式转移：From Storage to ETL

PPT 里提到的观点非常击中痛点：以前我们做笔记是在做 **Storage**，把资料囤积在 Notion/Evernote 里吃灰。

现在的逻辑应该是 **ETL (Extract, Transform, Load)**：

-   **Inbox**：数据湖 (Data Lake)，丢进去所有杂乱的 PDF、会议纪要。
    
-   **AI Agent**：ETL 管道。负责清洗数据、提取结构、打标签。
    
-   **Obsidian**：数据仓库 (Data Warehouse)。存放结构化、可调用的知识。
    

## 2\. 为什么是 Obsidian？

不是因为通过插件多，而是因为 **Markdown**。

-   **Markdown = Plain Text**：这意味着我的知识库本质上是一个**代码仓库**。
    
-   **AI Friendly**：AI 读取 Word/PDF 需要复杂的解析，但读取 Markdown 就像读代码一样顺畅。
    
-   **Git Version Control**：我可以像管理代码一样，用 Git 管理我的知识版本。
    

## 3\. 实战架构：给 AI 写“员工手册”

PPT 里最精彩的一点是提到了 `Rules.md`。

这本质上是在写 **System Prompt (系统提示词)**。

-   **我的操作**：在 Obsidian 根目录建一个 `Instruction.md`，告诉 AI：
    
    > “你是一个资深技术文档工程师。处理笔记时，请遵循以下规则：1. 所有的 Solidity 代码块必须标注版本号；2. 遇到未定义的术语，自动生成 \[\[Wiki Link\]\]；3. 总结必须包含 ‘Key Takeaways’ 模块。”
    
-   **效果**：每次跑脚本，AI 就会像一个训练有素的实习生，自动把我的乱码笔记重构成标准文档。
...
我们每天面对海量的链上交易，如果只是盯着 Etherscan 看，就像是在看乱码。

**未来的方向**：构建一个针对链上数据的 “Obsidian”。

-   写一个 AI Agent，自动抓取我关注的 Whale 地址。
    
-   自动解析交易行为（是套利？还是被清算？）。
    
-   生成一份 Markdown 日报推送到我的 Telegram。
