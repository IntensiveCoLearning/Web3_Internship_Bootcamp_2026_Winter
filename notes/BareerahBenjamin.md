...
好快啊，一个月就这样结束了。很充实的一个月，学到很多关于以太坊 / Web3 的东西，认识了许多有趣很酷的人，感谢ETHPanda，LXDAO 的组织和各位助教的努力。

虽然 Bootcamp 结束了，但是却是我 Web3 生涯的起点。吃水不忘挖井人，我会继续提升自己的能力，贡献自己的所学。
<!-- DAILY_CHECKIN_2026-02-08_END -->

# 2026-02-07
<!-- DAILY_CHECKIN_2026-02-07_START -->

今天做了 Crypto Talk 英语角的 host，然后在调 spoonos 的 agent
...
今天计划完成[**Challenge #4 -  Build a DEX**](https://speedrunethereum.com/challenge/dex)
...
今天把 uniswap v2 的笔记整理发布 然后尝试用 spoonos skill 搭建 agent，继续完善黑客松demo
...
昨天做了技术向的两个小任务，听了晚上的分享会，其他时候都在赶作业  

今天主要都在写学校的作业，然后完成了休闲黑客松的组队，学习 SpoonOS 的框架搭建
...
Chainlink 的 Wally 提到了他们与 SWIFT、瑞银的合作。最让我印象深刻的是，现在的重点不再是让 Web2 顺从 Web3，而是 Web3 基础设施在主动适配银行和资本市场。这种转变很务实：金融机构要的是效率和合规，而不是改变底层逻辑。

现场 Taiko 的 Adele 说得很直白，现在一键发链技术太成熟，未来 L2 会泛滥。但对于开发者和用户来说，大家只认“心智”。只有能真正沉淀资产、搞定实际支付或者承接 RWA 业务的链才能活下来。L2 赛道已经过了“讲技术故事”的阶段，现在拼的是谁能像金融路由器一样解决实际问题。

Cap.Money 和 Morpho 的合作模式很有趣。Cap.Money 已经是 Aave 的大金主，现在正通过 Morpho 这种更灵活的基础设施去提高资金效率。甚至现场还讨论了如何让 DeFi 用户参与香港的 IPO 基金产品，这种通过 RWA 把链上流动性引向本地金融红利的尝试，比单纯的空气币借贷有意义得多。
...
今天参与了LXDAO的周会，更深入的了解了DAO的运作机制和LXDAO项目的开展流程。

除此之外，我还梳理了这周所学到的内容，昨天的实习项目周会让我学到了一些运营的知识，还有朋友分享Uniwap和Solidity的知识，让我意识到自己需要了解和学习的知识还有许多。
...
![image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/BareerahBenjamin/images/2026-01-14-1768382815725-image.png)

\- **未来改进**：账户抽象（ERC-4337）让钱包更好用（可以批量操作、别人帮付 Gas），但底层还是需要签名，监管要求不变。

### 其他关键点

\- **地址**：都以 0x 开头，20 字节长，看起来像一串乱码，其实是公钥哈希。

\- **代币（如 ERC20、ERC721）**：其实不存钱包里，而是记录在合约的表格里，转代币 = 调用合约改表格。

\- **合约销毁**：现在很难彻底删（Dencun 升级后限制了），保护用户但也防止恶意操作。

\- **监管提醒**：代码一旦上线就改不了，所以大项目必须多轮审计。合规项目会主动加黑名单功能，防止脏钱流入。

### 智能合约到底是什么？

\- 就是一段**部署在链上的代码 + 数据**，写好规则后自动执行，不需要中间人。

\- 口诀：**Code is Law**（代码就是法律），谁都改不了。

\- 常见用途：发代币、自动借贷、NFT 买卖、DAO 投票。

### 写合约用什么语言？

\- **Solidity**：最主流，像 JavaScript，但专为以太坊设计。

\- 工具推荐：

\- Remix（浏览器里写，新手最友好）

\- Hardhat / Foundry（专业开发，测试更强大）

### 部署合约会产生什么？

1\. **字节码**：机器能读的代码（部署到链上）。

2\. **ABI**：接口说明书（告诉钱包怎么调用你的函数）。

3\. **地址**：部署后自动生成，公开透明。

\- 部署要花 Gas（钱），因为要存代码到链上。现在用 Layer2 或新升级（Dencun）能省很多。

![image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/BareerahBenjamin/images/2026-01-14-1768382918947-image.png)

\- **监管小贴士**：漏洞导致巨亏会引发系统风险，全球监管越来越严（MiCA 要求审计、投资者保护）。合规项目必须开源代码、多次审计，才能吸引机构和大资金。

### 合约上线后特点

\- **完全公开**：谁都能在 Etherscan 看代码、交易、日志（超级透明）。

\- **默认不可改**：想升级要用“代理模式”（前端地址不变，后端换实现）。

\- **监管好处**：公开 = 容易追踪资金流，帮助反洗钱。但匿名性仍需 KYC 补充。
