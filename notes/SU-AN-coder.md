...
-   运行流程：用户发起交易→广播全网→节点验证→共识机制打包成块→上链更新→矿工获奖励
...
| 联盟链 | 联盟邀请/审批 | 仅联盟成员可见 | 多中心联合决策 | 供应链、金融协作 |
...
-   Uniswap（DEX）：AMM自动做市商模式，x×y=k恒定乘积公式定价，用户存币成为LP赚手续费
    
-   Compound（借贷）：用户存币获cToken赚利息，借币需超额抵押，波动触发清算
    
-   Sky（原MakerDAO）：超额抵押生成稳定币（USDS），通过稳定费率调节供需
    

## 2\. NFT

核心：区块链实现数字资产唯一性与可验证所有权，智能合约支持自动版税分配

-   CryptoPunks：NFT先驱，10000个唯一像素头像，奠定NFT收藏文化
    
-   OpenSea：全球最大NFT交易平台，支持NFT买卖与创建
...
核心：基于网络文化的代币，价值依赖社区共识，投机性强、波动大（如DOGE、PEPE）

风险：缺乏实际价值，多为名人效应炒作和代币集中操控，缺乏明确的技术价值，但通过社区共识和文化认同获得价值
...
-   Web3+乡建：南塘DAO发行NT代币量化贡献，探索乡村社区货币与PGS区块链认证
