...
-   **任务要求：** 策划并举办一场线上活动（形式不限，如 Twitter Space 或会议分享）。
    
-   **组队规则：** 最多 3 人一组，共同完成策划、执行与复盘。
    
-   **时间节点：**
    
    -   **活动举办期：** 1月21日（周三）开始。
        
    -   **表单截止期：** 1月25日（周日）前提交申请表单，需审核排期。
        
-   **注意事项：**
    
    -   **账号使用：** 可申请使用官方号（需审核问题库），也可使用个人号（官方协助转推）。
        
    -   **学分发放：** 填写表单时，务必使用与 GitHub 绑定的邮箱，否则无法发放学分。
