...
| 协议 | 它的角色 | 生活中的类比 |
| ERC-20 | 同质化代币 | 钞票。你手里的一块钱和我手里的一块钱，价值完全一样，可以互换。 |
| ERC-721 | 非同质化代币 (NFT) | 身份证/房产证。每一张都是独一无二的，不能随便换。 |
| ERC-1155 | 多代币标准 | 超市收银台。既能处理成箱的饮料（ERC-20），也能处理独特的会员卡（ERC-721）。 |
| ERC-777 | 进阶版代币 | 带“自动扣费确认”的银行卡。比 ERC-20 更有礼貌，转账前会打招呼。 |
| ERC-4626 | 收益池标准 | 标准化存钱罐。不管你存什么，大家都用同样的口径计算利息。 |
| ERC-4337 | 账户抽象 | 智能管家。丢了钥匙也能找回，不用记复杂的助记词。 |
| ERC-1967 | 代理存储 | 可换芯的圆珠笔。外壳（地址）不变，里面的笔芯（逻辑）可以升级。 |
| ERC-2981 | NFT 版税 | 艺术家的提成合同。每次作品转手，创作者都能自动分到钱。 |
| ERC-6551 | 代币绑定账户 | 给你的玩偶（NFT）一个钱包。让你的 NFT 自己能收钱、买装备。 |
| ERC-3525 | 半同质化代币 | 电影票。同一个座位的票是唯一的，但同一排的票在某些属性上又是相似的。 |
...
├── [README.md](http://README.md) # 必须：项目说明文档

├── [DEMO.md](http://DEMO.md) # 必须：演示说明和截图
...
│ ├── [indexer.py](http://indexer.py) # 链上数据索引

│ ├── [decoder.py](http://decoder.py) # 交易解码（复用 Stage 1）

│ ├── [db.py](http://db.py) # 数据库操作

│ ├── [api.py](http://api.py) # Flask API

│ └── [main.py](http://main.py) # 入口
...
curl [http://localhost:8000/whales?limit=10](http://localhost:8000/whales?limit=10)
...
\- Polymarket 官方文档：[https://docs.polymarket.com/](https://docs.polymarket.com/)

\- Gamma API：[https://gamma-api.polymarket.com/](https://gamma-api.polymarket.com/)

\- Polygon RPC（免费）：[https://polygon-rpc.com/](https://polygon-rpc.com/)
...
Polymarket 就像一个自动化的“押注工厂”。

-   **ConditionId** 是工厂的**模具**（绑定了问题和裁判）。
    
-   **USDC** 是工厂的**原材料**。
    
-   **Split（拆分）** 是**生产线**：投入 1 USDC，吐出 1 个 YES 和 1 个 NO。无论结果如何，工厂永远不亏，因为两张票加起来永远只值 1 USDC。
    

**记住这个口诀**：

-   **Condition** 是“事”（判定结果的模具）。
    
-   **Collection** 是“位”（你是选 A 还是选 B）。
    
-   **TokenId** 是“钱”（事 + 位 + 抵押品）。
    
-   **AssetId 为 0** 就是拿着钱去买货（BUY）。
    

在实际写代码时，文档提到了一个非常重要的细节：**避免重复计数**。 同一笔撮合交易在链上可能会产生多条 `OrderFilled` 日志（比如一个大买家一次性撞上了好几个小卖家）。

-   **陷阱**：如果你把每一条日志都当成一笔独立的交易，你的总交易量统计就会翻倍。
    
-   **解决办法**：文档建议过滤掉那些 `taker` 地址等于交易所合约地址本身的日志，只保留 `maker` 侧的记录。
    

# 高效的索引器 Indexer ：用于海量交易数据

## 工程挑战：幂等性 (Idempotency)

**场景**：如果你的索引器在运行中途断网了，重启后它可能会重新扫描之前已经存过的区块。 **问题**：如果不做特殊处理，数据库里会不会出现两笔一模一样的交易？

### 1\. 什么是“唯一索引”？（比喻：超市小票）

**场景**：超市里每一笔交易都会出一张**小票**，而一张小票上可能买了**好几样东西**（比如：1. 苹果，2. 锅铲）。

-   **tx\_hash（交易哈希）**：就像是那张**小票的单号**。整笔交易共享一个单号。
    
-   **log\_index（日志索引）**：就像是小票上的**行号**（第 1 行、第 2 行）。一笔交易里可能包含多个动作（比如既有买入也有卖出），每个动作都有一个行号。
    

**回答第一个问题**：

为了防止存重了，我们会把 `tx_hash` **+** `log_index` 组合在一起作为“唯一索引”。

> 道理：就算小票单号（tx\_hash）一样，第 1 行和第 2 行（log\_index）也肯定不一样。只要这两个数凑在一起是唯一的，数据库就知道：“噢，这一行我已经记过了，不用再记一次。”

* * *

### 2\. 什么是“同步状态”？（比喻：看书的折角）

**场景**：你正在读一本 1000 页的《Polygon 区块历史》，今天读到了第 600 页，突然停电了。

**回答第二个问题**：

为了下次不从第 1 页重看，你需要一个“书签”。在开发中，我们会专门建一张 `sync_state`**（同步状态）表**。

-   这张表里只记一个关键数字：`last_block`**（最后处理的区块高度）**。
    
-   比如存入 `66,000,000`。下次程序启动，先查这张表，看到 6600 万，它就知道从 66,000,001 开始往后读就行了。
...
**场景**：有 5 匹马比赛，你觉得“1 号马绝对赢不了”。

**回答第三个问题**：

在 Polymarket 的“负风险”机制下，买入“A 不会赢（NO）”在数学上完全等于同时买入了“B 会赢、C 会赢、D 会赢、E 会赢”的 YES 代币。

-   **你会如何记录？** 虽然逻辑上很复杂，但作为索引器开发者，你只需要**如实记录**链上发生的动作。
    
    -   如果链上产生的日志是 `PositionsConverted`（头寸转换），你就记录这一次转换。
        
    -   在显示给用户看时，你可以标记为 **“多头寸组合”** 或者 **“反向对冲”**。
...
1.  **防重复**：靠“小票单号 + 行号”（`tx_hash` + `log_index`）双重校验。
    
2.  **断点续传**：靠“书签”（`sync_state` 表里的 `last_block`）记录进度。
    
3.  **多市场关联**：靠 `NegRiskAdapter` 合约把“买一个 NO”变成“买一堆 YES”的魔法转换。
...
现在，你已经理解了这些“记账”的规则。如果你要向别人解释为什么你的 Polymarket 索引器**数据很准且从来不重**，你会用哪个比喻？

**下一步建议**：

既然基础概念都通了，你想看看**阶段三**的“黑客松挑战”里，有哪些好玩的创意吗？比如做一个“鲸鱼大单追踪器”，看看有钱人都在赌什么？

# 可以打哪些标签？

在 Polymarket 的生态中，标签（Tags）不仅仅是一个名字，它是通过**链上历史数据**回溯出来的“用户画像”。根据你前面学到的解码逻辑（谁在买、买多少、买入价格是多少），我们可以把这些“特工”分成以下四大类：

* * *

### 1\. 规模类标签：看他“有多少钱” (Volume-based Tags)

这类标签只看交易金额，不看技术。利用你在 `trade_decoder` 中解析出的 `USDC_Amount`（记得除以 $10^6$）来判定。

-   **巨鲸 (Mega Whale)**：单笔交易金额经常超过 10,000 美元的用户。
    
-   **中坚力量 (Active Trader)**：虽然单笔不大，但每天频繁交易，累计交易额极高的用户。
    
-   **小试牛刀 (Shrimp)**：大多数单笔在 100 美元以下的小散户。
    

### 2\. 技术类标签：看他“有多准” (Performance-based Tags)

这类标签最值钱，需要结合阶段二数据库中的历史胜率来计算。

-   **聪明钱 (Smart Money)**：历史胜率极高（比如 > 80%），且总能提前在大行情到来前埋伏的用户。
    
-   **神算子 (Sharpshooter)**：专门买入那些赔率很高（单价 $0.1$ ~ $0.3$）但最后都赢了的订单。
    
-   **反向指标 (Inverse Signal)**：买什么亏什么，这种地址的动向也是极具参考价值的“反向明灯”。
    

### 3\. 策略类标签：看他“怎么玩” (Behavior-based Tags)

通过分析 `OrderFilled` 日志的频率和时机，可以识破他们的交易套路。

-   **搬砖套利者 (Arbitrageur)**：利用“负风险机制”，在不同的互斥市场之间倒手，赚取无风险价差的人。
    
-   **先行者 (Pioneer/Early Bird)**：在一个新市场刚刚创建（`ConditionPreparation`）就立刻低价杀入的人。
    
-   **机器人 (Trading Bot)**：成交极其规律，通常是毫秒级响应，这类通常是做市商（Market Maker）。
    

### 4\. 风险类标签：看他“有什么猫腻” (Risk-based Tags)

这类标签对监控市场异常非常有用。

-   **疑似内幕者 (Potential Insider)**：在没有任何新闻的情况下，突然重仓买入某个极低概率的选项，随后不久该选项价格暴涨。
    
-   **对冲者 (Hedger)**：同时买入 YES 和 NO，或者在相关市场进行相反的操作，通常是为了对冲风险。
...
现在你手里有这么多“标签”工具了，我们来玩一个**逻辑组合游戏**：

**场景**：如果你在你的“鲸鱼大单追踪器”后台看到一个新地址，他满足以下两个特征：

1.  **特征 A**：单笔投入了 20,000 美元（满足“巨鲸”标签）。
    
2.  **特征 B**：他在价格为 $0.05$（即市场认为只有 5% 概率发生）时买入（满足“神算子”或“内幕者”嫌疑）。
    

**引导问题：**

-   如果你要把这个地址推荐给你的用户关注，你会给这个通知起一个什么样**吸引人且准确**的标题？
    
-   在数据库层面，你要把这两个标签关联起来，你需要同时去 `trades` 表查 `usd_value` 和哪个字段？（提示：是那个决定他买入成本的字段）。
