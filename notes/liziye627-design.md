...
![image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/liziye627-design/images/2026-02-08-1770564539093-image.png)
...
pub state\_history: Vec<StateSnapshot>, // 复杂结构
...
\- \[x\] The Rust Book: [https://doc.rust-lang.org/book/](https://doc.rust-lang.org/book/)
...
\- 定义: 每个值只有一个所有者，离开作用域即释放

\- 特征: move 会转移所有权，clone 会复制数据

\- 理解程度: ✅

**概念2**: 借用与引用规则

\- 同一时刻：可有多个不可变引用，或一个可变引用
...
**问题描述**: 可变引用与不可变引用同时存在时的规则容易混淆

**解决方案**: 对照官方示例写 2-3 个小练习验证编译错误

**参考资料**: The Rust Book - 所有权章节
...
\- 定义: Rust 默认不可变，使用 `mut` 标记可变

\- 特征: 支持 shadowing（重新绑定）
...
**问题描述**: 表达式与语句的区别在复杂场景中还需要进一步体会

**解决方案**: 通过小练习（if/loop 返回值）强化理解

**参考资料**: The Rust Book - Chapter 3
...
1\. Rust 的语法较简洁，但类型与可变性非常明确

2\. `loop` 可返回值的设计很有特点

3\. 语句与表达式是 Rust 语法理解的关键
