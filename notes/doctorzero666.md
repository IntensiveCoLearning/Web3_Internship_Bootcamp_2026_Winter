...
<!-- Content_START -->
# 2026-02-09
<!-- DAILY_CHECKIN_2026-02-09_START -->
# **AI + Web3 落地现状（我今天的理解）**
...
-   **AI 用来服务 Web3**（更像“效率工具”：投研/风控/安全/交易辅助）
    
-   **Web3 用来补 AI 的短板**（更像“基础设施”：去中心化算力、可验证推理、数据/模型激励与协作）
    
    但整体还在“从叙事走向工程”的阶段，应用层爆款相对少。
...
### **A) AI for Web3：AI 给 Web3 提效率（目前最成熟）**

典型落地是 **链上数据 + LLM 的“投研/交易助手”**、安全助手、运营助手：

-   比如 Nansen 推出 AI 交易/分析聊天机器人，主打把链上数据和社媒信息整合成可问答的洞察（也在探索更自动化的 agent 交易）。
    

> 大白话：这类最像“把 Dune/Nansen/链上数据产品变得更好用”。

### **B) Web3 for AI：去中心化算力/基础设施（需求真实，但商业模式还在打磨）**

核心逻辑：AI 需要算力/数据/验证，Web3 用“市场+激励”把资源组织起来。

-   **去中心化算力（GPU/云市场）**：以 Akash 为代表，把算力做成“开放市场”，面向 AI/ML workload。
    
-   **DePIN 更大范畴**：把现实世界设备/基础设施接入链上，用 token 激励供给（其中就包含算力这条线）。
    

> 现阶段落地特点：
> 
> **有真实需求（GPU 稀缺），但供需质量、稳定性、SLA、合规与规模化仍在磨**

### **C) 可验证 AI（Verifiable Inference / zkML）：让“AI 输出可信”**

当 AI 要触发链上资金动作时，“你说模型输出是 X”不够，需要能证明。

-   zkML / 可验证推理被视为关键基础设施方向：让模型推理结果可证明、可验证（也常和隐私一起讨论）。
    

> 大白话：
> 
> **AI 很强，但不可信；区块链很可信，但不会算。zkML 是桥。**

### **D) Onchain / Web3 AI Agents：让 agent “持币 + 可执行”**

这条线的核心是：AI agent 不只是聊天，而是能 **拥有资产、调用合约、自动执行策略**。

-   市场观点普遍认为这是 2025–2026 的热门方向之一，但真正大规模普及还受限于安全、可验证性、权限管理。
...
1.  **信任与安全**：AI 容易幻觉；一旦自动操作链上资产，风险巨大（所以需要可验证推理/更强权限隔离）。
    
2.  **成本与性能**：把 AI 放到链上算不现实；链下算又要解决“怎么信任链下结果”。
    
3.  **数据问题**：训练/推理需要高质量数据，链上数据只是其中一部分，链下数据又牵涉授权与合规。
    
4.  **商业闭环不清**：很多项目还停留在“token 激励拉供给”，但需求侧付费能力/留存没跑出来。
    

* * *

## **4）如何判断一个 AI+Web3 项目是不是真落地（投研 checklist）**

你可以用这 6 个问题快速筛：

1.  **用户是谁？**（开发者/交易员/企业/普通用户）
    
2.  **AI 在哪一环？**（分析、决策、执行、验证）
    
3.  **链在解决什么？**（结算、所有权、激励、可验证、抗审查）
    
4.  _不用链行不行？_\*如果不用链也能更便宜更稳定做，那就要警惕“为了上链而上链”。
    
5.  **数据与成本模型**：数据来源、更新频率、推理成本谁付？
    
6.  **安全设计**：权限隔离、可撤销授权、多签/时间锁、是否引入可验证推理。
...
-   **短期（现在）**：最真实的落地是 **AI 帮 Web3 用户“读懂链上世界”**（投研/安全/操作辅助）。
    
-   **中期（1–2 年）**：会越来越重要的是 **去中心化算力 + 可验证推理（zkML）**，让 agent 能安全地“动钱”。
    
-   **长期（更大叙事）**：Web3 可能给 AI 时代提供新的“价值分配与数据主权”框架，但需要工程与制度一起成熟。
    

* * *
<!-- DAILY_CHECKIN_2026-02-09_END -->
...
-   **一句话**：Dune 是一个“链上数据分析平台”，让你用 **SQL** 直接查询区块链数据，并把结果做成可视化图表和 Dashboard。
    
-   你能用它做：
    
    -   查协议数据（TVL/交易量/手续费/用户数）
        
    -   查某个合约/代币的行为（mint、transfer、swap）
        
    -   查钱包行为（持仓变化、交互次数、活跃时间）
        
    -   做投研面板（增长、留存、资金流、鲸鱼动向）
        

* * *

## **2）Dune 的核心心智模型：它的数据从哪来？**

你可以这样理解：

-   链上原始数据：区块、交易、事件日志（logs）
    
-   Dune 把这些数据 **同步→解析→整理成表**（tables）
    
-   你写 SQL 查表，得到结果 → 做图 → 拼成 dashboard
...
## **3）最关键的概念：事件（Event）和 Logs**

很多协议的数据都藏在 **事件日志**里：

-   ERC20：Transfer
    
-   NFT：Transfer（ERC721/1155）
    
-   Uniswap：Swap/Mint/Burn 等事件
...
## **4）Dune 常见数据表（你学 Dune 必须会找表）**

不同链/版本表名会不同，但你要掌握“去哪找”：

-   **raw 交易类**：transactions、blocks
    
-   **事件 logs 类**：logs（原始）、decoded logs（解码后）
    
-   **token 类**：ERC20 transfers、balances（有时需要自己算）
    
-   **协议类**：很多热门协议有现成的“decoded/curated 表”（更好用）
    

你在 Dune 里做 SQL 的第一步通常是：

-   用 Data Explorer 搜关键字：协议名 / 合约地址 / event 名
...
-   地址字段（from/to/contract\_address）
    
-   数值字段（amount，注意 decimals）
    

### **Step 3：聚合统计**

-   按天：date\_trunc('day', block\_time)
    
-   计数：交易数、用户数（distinct address）
...
## **6）Dune 做投研时最常用的 6 个指标**

1.  **DAU / Active wallets**（活跃用户）
    
2.  **交易量 Volume**（真实需求）
    
3.  **手续费 Fees / Revenue**（商业模式）
    
4.  **留存 Retention**（是不是昙花一现）
    
5.  **新用户增长 New users**（增长曲线）
...
## **7）Dune 的优势 vs 局限（你要知道它不是万能）**

### **优势**

-   上手快：SQL + 图表，一天能做出看板
    
-   适合公开投研：Dashboard 分享方便
    
-   生态强：很多现成 query 可以 fork 改
    

### **局限**

-   链上数据 ≠ 全部真相：CEX 行为、链下订单、真实身份你看不到
    
-   有些协议数据需要复杂解析（比如多合约、跨链、隐藏逻辑）
    
-   token balance/成本价常需要“自己计算”，不是直接表里就有
...
2.  **它凭什么能赢？（护城河/网络效应/分发/产品）**
    
3.  **值不值得买？买了靠什么赚钱？（估值+催化剂+风险）**
...
### **Layer A：宏观与叙事（Narrative）**

-   当前市场处在什么阶段：风险偏好上升还是下降？
    
-   叙事是什么：L2、Restaking、AI+Crypto、RWA、DePIN、SocialFi……
    
-   你要找的是：**“这条赛道的钱往哪流”**
    

### **Layer B：链与基础设施（Chain / Infra）**

-   这条链有没有真实活跃？（用户、交易、费用、开发者）
    
-   是否便宜快？生态是否完整？（钱包、桥、DEX、稳定币）
    
-   你要找的是：**“这块土壤能不能长出应用”**
    

### **Layer C：协议/产品（Protocol / Product）**

-   产品到底怎么用？用户为什么留下？
    
-   PMF 信号：留存、复购、使用频率、TVL/Volume 的可持续性
    
-   你要找的是：**“是不是在真实解决需求，不是刷数据”**
    

### **Layer D：商业模式（Business Model）**

-   收入来自哪里？（手续费/利差/MEV/服务费）
    
-   成本是什么？（激励补贴、运营成本、安全成本）
...
### **Layer E：代币经济（Tokenomics）**

-   代币是“股权型（捕获价值）”还是“积分型（激励/治理）”？
    
-   代币价值捕获路径：费用分配？回购销毁？质押收益？
...
### **Layer F：风险与合规（Risk / Security / Compliance）**

-   合约安全：审计、权限、多签、升级、预言机
    
-   运行风险：桥、托管、前端劫持、治理攻击
    
-   合规风险：是否可能被认定为证券/违规发行/洗钱通道等
...
## **3）投研常用数据与指标（你可以当“检查仪表盘”）**

### **链级数据（判断生态活跃度）**

-   活跃地址、交易数、Gas/费用、合约部署数、稳定币规模
...
-   TVL、交易量、手续费收入、用户数、留存/复购
    
-   典型工具：DefiLlama、Token Terminal、Messari
    

### **资金流与行为（判断“谁在买/谁在卖”）**

-   巨鲸持仓变化、资金净流入、机构钱包行为、交易所净流入/流出
...
-   市值/FDV、流通量、解锁时间表、交易深度、波动率
    
-   典型工具：CoinGecko、CoinMarketCap
    

### **基本面与研发（判断团队执行力）**

-   GitHub 提交频率、核心贡献者、issue/PR 质量、版本迭代节奏
...
2.  核心指标：TVL/Volume/Fees 是涨是跌？
    
3.  代币结构：FDV 高不高？解锁近不近？
    
4.  安全红线：有没有审计？权限是不是单点？
...
-   读 docs、跑一遍产品、拆代币模型、算估值区间、找对手对比
    
-   找催化剂：新版本、上所、空投规则、合作、合规变化
...
1.  **相对估值**：同赛道对比（市值、FDV、收入倍数）
    
2.  **收入/费用倍数**：协议赚的手续费/收入是否能支撑市值（类似 P/S）
    
3.  **情景分析**：牛/熊/基准三种情况，估个区间，别追求精确
    

> 大白话：加密很多时候是“预期定价”，但你至少要知道
> 
> **贵不贵**
> 
> **靠什么变贵**
...
-   代币释放极不友好（近期开大锁 + 没有需求支撑）
    
-   数据明显刷量（交易量/TVL 与用户行为不匹配）
    

### **允许带风险但要写清楚的点**

-   监管不确定性（尤其面向散户“收益承诺”的）
...
-   **一句话**：Rust 是一门主打 **“高性能 + 高安全（内存安全）”** 的系统级语言，尽量在编译期帮你把很多低级 bug（空指针、数据竞争）挡掉。
    
-   常见用途：
    
    -   区块链/底层基础设施（很多链的客户端、节点、工具链）
        
    -   高性能后端、网络服务
        
    -   安全敏感的系统（加密、交易系统、数据库组件）
...
## **2）Rust 的核心价值：为什么大家说它“又难又香”？**

### **Rust 的优势**

-   **快**：接近 C/C++ 的性能
    
-   **安全**：不靠 GC（垃圾回收）也能做到内存安全
    
-   **并发强**：很多并发错误会在编译期被拒绝
    
-   **工程体验好**：Cargo（包管理+构建）很强，生态工具成熟
    

### **Rust 的门槛**

-   学起来比较“反直觉”：你得先接受它的规则（所有权/借用/生命周期）
...
## **3）Rust 最核心的概念：所有权（Ownership）**

Rust 最有代表性的点就是这套“内存管理哲学”。

### **3.1 所有权规则（记三条就行）**

1.  **每个值都有一个 owner（拥有者变量）**
    
2.  **同一时间只能有一个 owner**
    
3.  **owner 离开作用域，值就被释放（drop）**
    

> 大白话：Rust 像在管“物品归属”。一个东西同一时间只能明确属于一个人，没人管就自动丢掉。

### **3.2 Move vs Copy（为什么赋值会“把值搬走”？）**

-   某些类型（如 String、Vec）赋值时默认是 **move**：把所有权转移，旧变量不能再用
    
-   一些简单类型（如 i32、bool）是 **copy**：复制一份，旧变量还可以用
    

* * *

## **4）借用（Borrowing）和引用（& / &mut）**

Rust 不让你随便复制大对象，所以提供“借用”机制。

### **借用的关键规则（非常重要）**

-   可以有多个 **不可变引用**：&T（大家只读不改）
    
-   只能有一个 **可变引用**：&mut T（一个人改，避免冲突）
...
-   **一句话**：生命周期是 Rust 用来保证“引用不会指向已经被释放的东西”的标注系统。
    
-   你入门阶段不用硬背复杂写法，只要记住它要解决的问题：
...
## **6）错误处理：Result / Option（Rust 不爱抛异常）**

Rust 强调“把错误写进类型里”。

-   Option<T>：有值/没值（Some/None）
    
-   Result<T, E>：成功/失败（Ok/Err）
    

> 大白话：

> Rust 不喜欢“运行时突然炸”，更喜欢你“在编译期就把失败情况写清楚并处理掉”。

* * *

## **7）模式匹配 match（Rust 很常用的语法武器）**

-   match 就像“超级 switch”，配合 Option/Result 非常自然
...
## **8）工程化：Cargo（Rust 的强项之一）**

-   **一句话**：Cargo 是 Rust 的官方构建+依赖管理工具
    
-   典型能力：
    
    -   创建项目、管理依赖（类似 npm/pip，但更统一）
...
很多区块链生态（比如一些链的客户端/工具）大量使用 Rust，所以会 Rust 对你做 Web3 builder 非常加分。
...
**Web3 自治 = 规则写在链上（智能合约），决策靠投票/共识，执行尽量自动化，减少“某个人说了算”。**

传统组织：人定规则 + 人执行

Web3 自治：代码定规则 +（投票/多签）触发执行

* * *

## **2）自治想解决什么问题？**

1.  **信任问题**：不用相信某个老板/平台，靠透明规则
    
2.  **执行问题**：投票通过后能自动执行（比如发钱、改参数）
    
3.  **透明问题**：资金流向、投票记录、提案内容可公开验证
    
4.  **协作问题**：全球成员也能协作治理（社区化）
    

* * *

## **3）Web3 自治的三层结构（以后看 DAO 可以按这三层拆）**

### **A. 决策层（Governance）**

-   谁能提案？谁能投票？投票权怎么分配？
    
-   投票方式：Token 投票 / NFT 投票 / 信誉投票（soulbound/贡献）等
    
-   结果怎么算：简单多数 / 绝对多数 / 法定人数（Quorum）/ 双重门槛等
...
-   提案通过后怎么执行？
    ...
    -   **自动执行**：Timelock 合约到点自动执行
        
    -   **半自动**：多签签名后执行（更安全但更人治）
        
-   核心组件：
    
    -   **Timelock（时间锁）**：通过后延迟生效，给社区“反悔/应急”时间
        
    -   **Treasury（国库）**：资金在链上，按规则拨款
        

### **C. 运营层（Off-chain coordination）**

-   实际讨论、动员、信息同步常在链下
    
    -   论坛（Discourse）、Discord、Snapshot 等
...
### **方式 3：TimeLock + 合约自治（On-chain governance）**

**大白话：提案投票通过后，等一段时间自动执行，任何人都能验证。**

-   优点：透明、可强制执行、抗单点
    
-   问题：慢、链上投票成本高、容易被治理攻击
    

> 记忆法：TimeLock 是“冷静期”，防止突然改规则或被偷袭。

* * *

### **方式 4：Snapshot 链下投票（Off-chain voting）**

**大白话：先用链下投票“表态”，成本低；真正执行再走链上。**

-   优点：便宜、投票体验好、效率高
    
-   问题：本身不强制执行（要配合多签/链上执行）
    

* * *

### **方式 5：贡献/信誉自治（Reputation-based / Contribution-based）**

**大白话：投票权不是靠钱，而是靠贡献/身份。**

-   优点：更像“谁干活谁说话”，能缓解鲸鱼问题
    
-   问题：贡献怎么衡量？会不会被刷？如何公平？
    

* * *

## **5）一个 DAO 从 0 到 1 的典型自治流程**
...
-   **治理攻击**：大户借币/买币瞬间控票（尤其 token 投票）
    
-   **低投票率**：社区懒得投 → 被少数人决定
    
-   **信息不对称**：核心团队掌握更多信息，社区难判断
    
-   **执行权限风险**：升级权限/国库权限如果被盗或被滥用，后果严重
    
-   **治理过慢**：响应紧急情况不够快 → 所以需要多签应急机制
...
**Web3 的自治不是“完全没人管”，而是用“投票规则 + 多签 + 时间锁 + 国库管理”把权力拆开、把过程公开，让组织尽量不依赖某个单点。**
...
**一句话：DApp = 前端（界面） + 智能合约（规则） + 钱包交互（签名） +（可选）链下服务。**

-   **前端（Web/App）**：用户看到的页面、按钮、表单
    
-   **合约（Solidity）**：链上业务逻辑、状态存储、权限规则
    
-   **钱包（MetaMask 等）**：帮用户签名发交易，证明“是我本人操作”
    
-   **链下（可选）**：索引/数据库/文件存储/后端服务（为了更快、更好用）
...
### **Step A：需求与交互流程（先设计再写）**

你要先写清楚：

-   用户是谁？要解决什么问题？
    
-   **用户路径**：打开页面 → 连接钱包 → 读数据 → 写交易 → 等确认 → 展示结果
    
-   哪些逻辑必须上链？哪些放链下更合理（省 gas / 更快 / 更隐私）
...
-   **状态变量**：存数据（mapping/struct/array）
    
-   **函数**
    
    -   view/pure：读数据（免费，不用交易）
        
    -   写函数：改状态（要发交易、要付 gas）
        
-   **事件（events）**：记录“发生了什么”，给前端/索引用
    
-   **权限控制**：谁能改？谁能调用？（onlyOwner/roles）
    

### **Step C：本地测试（必须）**

-   用 Foundry / Hardhat 写测试
...
> 大白话：DApp 的“难点”不在写函数，而在把“链上确认”的过程做成用户能理解的体验。

* * *

## **5）链上数据展示：事件 + 索引（让体验更像 App）**

-   _事件（event）_\*是合约发出的“日志”
    
-   如果你要展示“历史列表/排行榜/行为记录”，只靠 storage 会很难查询
    
    常见做法：
    
-   小项目：前端直接读事件
    
-   正经产品：用 The Graph 或自己写监听服务 → 存数据库 → 提供查询 API
    

* * *

## **6）文件/图片/元数据：不要把大文件塞链上**

-   链上存储贵
    
-   常见方案：
    
    -   文件放 IPFS/Arweave/云存储
        
    -   链上存 hash/URI
        
-   NFT 就是典型例子：链上记录 ownership + tokenId，文件内容在链下链接里
...
-   对写操作要有明显提示：你将花费 gas、你将授权什么
    
-   避免盲签/恶意授权（approve）
    
-   合约权限要最小化：关键权限最好多签/延时
    
-   前端要防钓鱼：合约地址、域名、网络要校验
...
-   前端能处理：拒签/失败/pending/网络错误
    
-   事件日志完整（便于追踪）
    
-   合约关键权限明确（owner 是谁？可否升级？）
    
-   README 写清楚：如何部署、如何运行、如何在测试网验证
...
**Uniswap v4 = 把 AMM 变成“可插插件”的系统（Hooks），并用“单例合约 + 闪电记账”把交易/多跳成本压下来，让开发者能做出更多自定义玩法。**
...
1.  **更灵活**：池子的行为不再固定，允许开发者按需求改规则（Hooks）。
    
2.  **更省 gas**：多跳交易、复杂组合操作时减少中间转账，统一结算。
    
3.  **更像基础设施**：提供统一的核心（PoolManager），周边用 router/SDK/Hook 扩展功能。
...
**大白话：你可以在“建池、加/减流动性、swap”等关键节点插入自定义逻辑。** Hooks 是外部合约，挂在某个池子上，协议在流程中会调用它。
...
### **B. Dynamic Fees（动态手续费）**

-   _大白话：手续费可以上调/下调，而且 v4 不规定你怎么计算。_\*你可以每次 swap 调一次、每个区块调一次，或者按周/月调整都行。
    

这打开了：费率优化、价值再分配、更多机制设计空间。

* * *

### **C. Singleton Architecture（单例架构：PoolManager）**

**大白话：v3 是“一个池子一个合约”，v4 是“所有池子都在一个核心合约 PoolManager 里”。**
...
**大白话：先在 PoolManager 里把一串操作“记账”，最后一步再统一转账结算。**

所以多跳 swap / 多次修改流动性这种“组合动作”，中间不用每步都真实转 token。

关键概念：

-   每次会改变余额的操作，会更新一个内部的“净额 delta”
    
-   只有在最后结算时才发生真正的 token 转账
    

* * *

## **5）v4 的 Swap / 加减流动性的大概流程（心智模型）**
...
2.  执行 beforeSwap hook（如果有）
    
3.  执行 swap，更新池状态并收取费用
    
4.  执行 afterSwap hook（如果有）
    
5.  返回净额（BalanceDelta），最后统一结算
    

### **Modify Liquidity（加/减流动性）**

同样在关键节点会触发对应 hooks（beforeAdd/afterAdd、beforeRemove/afterRemove 等），然后返回 delta。

* * *

## **6）和 v3 最大的不同点（考点式总结）**

-   v3：多跳/复杂操作时，集成方需要在**每一步**负责转 token（更贵更繁琐）
    
-   v4：用 flash accounting，允许一串操作后**最后统一转账**（更省 gas、更适合组合）
    

* * *

## **7）开发者实践：怎么“用”v4（后面会用到）**

-   **建议走 Router/SDK** 而不是直接手撸底层（更顺、更不容易踩坑）
    
-   Uniswap 提供 v4 SDK，方便在 TS/JS 环境下交互（加减流动性、collect fees、对 hooks 的交互等）。
...
**Hooks 给了超级强的扩展能力，也带来更大的攻击面。**

比如 hook 逻辑写错、权限设计不当、重入/外部调用风险等，会影响 swap/LP 行为。

* * *

## **9）5 句速记（复习用）**

1.  v4 的核心是：**Hooks + Singleton（PoolManager）+ Flash Accounting**。
    
2.  Hooks 让池子行为可编程，可在 swap/加减流动性/建池前后插逻辑。
    
3.  Dynamic fees：费率怎么变由开发者决定，频率也可自定义。
    
4.  Flash accounting：中间先记 delta，最后统一结算转账。
    
5.  v4 比 v3 更适合复杂组合与多跳场景（gas 更友好）。
...
-   **一句话**：Vibe coding = 你先把“想要的感觉/效果”描述清楚，让 AI 帮你快速写出能跑的代码，然后你再用“迭代 + 测试 + 小修小补”把它打磨成可用产品。
    
-   它更像“导演+剪辑”的工作方式：你负责方向、边界、验收标准；AI 负责大量生成与搬砖。
...
Vibe coding：先出一个能跑的 demo → 边用边改 → 逐步加规则/边界 → 最后变成稳定版本
...
-   “把 API 抽成一个 service 层”
    
-   “加一个简单的日志/埋点”
    

### **Step 3：让 AI 帮你 Debug，但你要会“喂信息”**
...
当 demo 越来越大，必须开始做工程化：

-   目录结构清晰（components / pages / hooks / services / utils）
...
-   _【限制】_\*技术栈、不能用什么、要兼容什么
    
-   _【边界】_\*异常情况怎么处理
    
-   _【验收】_\*给我可运行代码 + 如何启动 + 我怎么验证
    

**例子（思路示范）**：

-   “用 Next.js + Tailwind 写一个页面，输入钱包地址，点击查询，显示余额；请求失败要提示；按钮要有 loading；代码要拆组件。”
    

* * *

## **5）Vibe Coding 的“坑”和解决办法**
...
-   及时抽象：组件化、service 层、类型、统一错误处理
    
-   把“规范”写进 prompt：目录结构、命名规则、注释要求
...
## **6）什么时候 vibe coding 特别适合？**

-   做 demo / hackathon / 快速验证 MVP
    
-   前端页面、数据面板、脚手架搭建
    
-   写重复代码：CRUD、表单、组件、API 封装
...
-   高安全合约代码直接全信 AI（必须审计/测试）
    
-   对性能/一致性要求很高的底层系统（需要更严谨设计）
