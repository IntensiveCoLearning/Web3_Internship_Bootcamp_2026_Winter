...
一、 区块链数据结构区块链数据主要分为三类基本类型，理解这些结构是进行链上分析的基础。1. 核心数据类型数据类型描述关键字段示例交易数据 (Transactions)记录状态改变的行为，包含资金转账、合约交互等。交易哈希 (Hash)、状态、收发地址、金额、Gas 费、Nonce。区块数据 (Blocks)记录交易的容器，包含元信息及交易列表。区块高度、哈希、时间戳、验证者/矿工、交易列表。合约数据 (Contracts)部署在链上的代码逻辑，定义了交互规则。合约地址、函数逻辑、事件 (Events)、状态变量。2. 链上交易生命周期一笔交易从产生到确认通常经历以下四个阶段：1.创建：用户通过钱包发起交易，指定参数。二、 核心数据指标在 Web3 分析中，以下指标常用于衡量协议或网络的健康度与价值。1. 价值与规模指标•总锁仓量 (TVL, Total Value Locked):•定义: 协议中锁定的加密资产总价值（如质押、借贷池）。•意义: 衡量 DeFi 协议市场占有率和吸引力的核心指标。•交易量 (Volume):•定义: 特定时间内涉及资产价值的总额。•意义: 反映市场活跃度及资产流动性。2. 用户与活跃指标•活跃地址数 (Active Addresses):•定义: 指定时间内执行过至少一次交易的唯一地址数。•意义: 类似于 Web2 的 DAU/MAU，反映用户基础和增长趋势。•交易数 (Transaction Count):•定义: 链上发生的交易总次数。•意义: 关注交易发生的频率。3. 成本与盈利指标•Gas 费用 (Gas Fees):•定义: 用户支付给矿工/验证者的费用（Gas 数量 × Gas 价格）。•意义: 反映网络拥堵程度及用户交互成本。•协议收入 (Revenue):•定义: 协议收取的服务费扣除运营成本后的净值。•意义: 衡量协议的盈利能力及对代币持有者的潜在价值。三、 数据分析工作流 (ETL)Web3 数据分析遵循经典的 ETL 流程，但具有区块链特有的解析步骤。1.提取 (Extract): 从 RPC 节点或浏览器（如 Etherscan）获取原始非结构化数据（通常是十六进制 HEX 格式）。2.转换 (Transform):•核心步骤: 使用 ABI (应用二进制接口) 解码 HEX 数据。•目标: 将原始日志转换为可读的函数参数、事件和数值。3.加载 (Load): 将清洗后的结构化数据存入数据仓库，进行 SQL 查询、聚合计算及可视化输出。四、 Web3 数据产品生态根据在 ETL 流程中所处的位置，数据产品可分为三类：五、 常用分析工具速查•Etherscan: 区块链浏览器，用于查询单笔交易详情、Gas 追踪及合约代码。•Dune: 强大的链上 SQL 查询平台，适合深度定制化分析和社区看板。•RootData: 综合性 Web3 项目库，涵盖投融资、团队背景、代币解锁等信息。•DefiLlama: 专注于 DeFi 协议的 TVL、收益率及跨链数据。•Token Terminal: 侧重于财务指标分析，如协议收入、市盈率 (P/S Ratio) 等。
