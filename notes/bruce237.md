...
实现一个 ReAct Agent 或者 Graph Agent

搭建本地区块链节点

完成 Reactive Contracts 构建跨链功能作业
...
完成三小节 Uniswap V2 实操内容

实践挑战：Challenge #2 - token vendor

学习 Uniswap V3 并产出一篇学习笔记

学习 Uniswap V4 并产出一篇学习笔记
...
在 remix 中运行 [Solidity by Example | 0.8.26](https://solidity-by-example.org/) Basic 部分的代码
...
提供区块的元信息并链接其他区块，确保区块链的顺序性及数据完整性，包含前一区块哈希值、时间戳、哈希值树 Merkle 根等。

区块体（Block Body）

储存区块的核心内容，及经过验证的交易数据或其他记录。

生成过程

交易收集

网络节点接收用户广播的交易，将其存入内存池（mempool）。

打包验证

矿工或验证节点按照共识算法，将合法交易打包成区块。

共识验证

其他节点验证区块的合法性，并将其加入区块链。

区块作用

记录交易

每个区块存储一组经过验证的交易，为网络的完整运行提供基础。

连接节点

区块之间的哈希链结构确保了整个网络的连续性和安全性。

防篡改证明

区块头中的哈希值和时间戳使数据的篡改难度极高。
...
我们需要对机制进行设计，从而将坏人作恶的可能性减弱到最小

二次方资助模型

多数人投票比单独的大户投票权更大  
（100 人投 1 票大于 1 人投 100 票）

更多真实的人发表真实的看法会让结果趋于最优解（群体智慧的最优解）
...
具有成熟 VC 投资以及商业化价值过高的项目，还可以分配公共物品资金吗？

我们面临 2 选 1 决策的时候，是均分还是投硬币？

我们如何能有效地防止治理攻击（从机制上，而不是对话判断）
