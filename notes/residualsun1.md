...
「零知识证明」来自密码学的知识，主要指一方（证明者）向另一方（检验者）证明某命题为真的的方法。在这个过程之中，除了证明「命题为真」以外，不泄露任何信息。最容易理解的例子，要向他者证明自己拥有某情报，直接公开情报即可，但这样也会将细节一起泄露出去，但「零知识证明」要做到的是证明自己**拥有该情报而不必透露情报内容**。

「My First ZKVote」是将「零知识证明」与 Web3 结合的链上投票展示。

这里引入三种投票方式的对比。

-   一种是传统的电子投票系统，但这种投票方式需要信任中心化服务器正确记录和统计选票，难以向公众证明统计过程没有被篡改，普通选民也很难独立验证审计过程。
    
-   第二种是传统的区块链投票系统，由于区块链的特性，这种投票方式公开透明，所有投票交易记录都在链上，任何人都可以查询，并且区块链一经确认便不可篡改，另外，智能合约会按照预先写好的逻辑完成计票与结果发布。由此，通过区块链进行投票，从选票生成、传输、计票到结果发布，每一步都可以被任何第三方独立地验证。但在传统的区块链投票中，每一次投票都是一次交易，而交易公开可查，可以复算，投票行为可以被与投票人的钱包地址永久关联，任何人都可以通过区块浏览器来查看钱包地址背后投票人的投票记录。
    
    具体来说，在一个链上投票中，一个地址对应一张选票，投票就是向投票合约发送一笔交易，这笔交易会公开记录三样信息：其一，发起交易的钱包地址；其二，调用的合约地址；其三，调用的数据——可以反推钱包地址背后的投票人投给了哪一个选项。所以，只要访问区块链浏览器，就可以查询到某钱包地址参与过多少次投票、每次投票在什么议题上、投给了哪一方，甚至如果该地址在其他场景已经与现实身份绑定，便可以进一步将真实身份与投票偏好绑定。
    
    当投票涉及政治立场、公司内部表决和薪酬评审等问题时，投票人希望自己的投票不会被关联到某个具体地址或身份，但仍然需要该投票过程可以被验证，不会因为匿名而牺牲正确性。这时便需要满足「让选票匿名，但又可以被验证」的条件，「零知识证明」便派上用场。
    
-   第三种即零知识证明投票，一般称为 ZK 投票。在对 ZK 投票进行理解前，需要理解「零知识证明」的三个基本特性：完备性、可靠性和零知识性。
    
    -   完备性指，如果语句为真，并且证明者与验证者都按照协议诚实执行，那么验证者最终一定会被说服；
        
    -   可靠性指，如果语句为假，那么任何试图伪造证明的「作弊」证明者，都几乎不可能骗过诚实的验证者
        
    -   零知识性指，验证者在整个证明过程中，除了「语句是真的」之外，不会获得任何额外信息，也就是不允许验证者从证明内容中还原出证明者掌握的秘密。
        
    
    对应到投票系统中，完备性保证合法选民的合法投票不会被无故拒绝；可靠性保证非选民或者试图一人多投的人无法通过验证；零知识性则保证验证者无法从证明中推断是谁，投了什么票。因而 ZK 投票的出现即对传统区块链投票几乎无隐私的回应。
    

在一次完整的 ZK 投票中，基本包含这样的流程：

生成身份秘密与承诺 → 获取最新的选民集合信息 → 在本地构造 ZK 证明 → 提交投票交易 → 链上验证与计算 → 查看结果与审计
...
工具（Tools）可以被理解为一种组件，它是 Agent 可以调用的函数。
...
在传统方式之中，每增加一次工具都需要修改一次代码，这意味着工具写死在代码之中，只要改工具就要改 Agent。后来 MCP 出现了，MCP 运行时动态加载，只需要连接 MCP Server，可以自动发现可用工具，而后根据用户需求选择工具。
...
可以说，MCP 带来的最大价值是生态价值。不管是 DeepWiki MCP（代码分析），Tavily MCP（网络搜索），还是 Slack MCP（消息），我们都可以通过 MCP 协议来用 Agent 调用任何人开发的 MCP Server。所以我们可以以 Server 独立升级的方式来代替改工具就要改 Agent 的方式。

MCP 与 Agent 的通信方式一共有三种，分别是 stdio、HTTP（可流式）、SSE（Serve-Sent Events）

如果在本地部署 MCP Server，首选 stdio 方式（无需网络），但这种方式不支持远程的调用，配置如下

```
config = {
    "command": "python",
    "arg":["my_mcp_server.py"],
    "transport": "stdio"
}
```

如果要使用他人开发的 MCP Server，便会使用 HTTP 的格式。

```
http_mcp_config = {
    "url": "https//mcp.deepwiki.com/mcp",
    "transport": "http",
    "header": {"Accpet": "application/json, text/event-stream"}
}
```

这里 `header` 的用处是声明 MCP Server 返回的格式是 `json` 或者 `event-stream`。当然 HTTP 格式显然支持远程调用，广泛使用于云端服务和公共 API 中。

SSE 的格式最大优势是对长任务友好，客户端通过 `HTTP GET` 来发送请求，服务器通过事件流来返回结果，并且是实时推送的方式。

```
sse_mcp_config = {
    "url": "http://mcpdeepwiki.com/sse",
    "transport": "sse",
    "headers": {"Accept": "text/event-stream"}
}
```

如果说 MCP 让 Agent 拥有了跟外部 MCP Server 去调用它们工具的能力，那么 Skill 就是让 Agent 拥有自身内部的专业能力。打一个比喻，如果 Agent 只是一个单独的员工，那么 Skill 可以被理解为员工的专业技能证书——不仅拥有知识，还可以「干活」。

创建 Skill 后，主要包含两个部分，其一是 `Skill.md`，里面存储了元数据和发给 Agent 的提示词，其二是 Scripts 文件，里面写好了可以执行的脚本。
...
`name` 是 Skill 的唯一标识，`triggers` 是触发器，决定什么时候触发 Skill，满足特定条件时，Agent 会拥有一项 Skill 的技能，`scripts` 定义了 Agent 可以执行哪些能力。分割线之后的内容就是 Markdown 内容，用于给 Agent 注入 Prompt。通过 `Skill.md` 文件，我们可以通过不写代码，只写配置文件（也就是 `Skill.md` 文件本身）就给 Agent 添加新的能力。
...
这需要结合 EIP 的概念来进行理解，EIP 是以太坊改进提案（Ethereum Improvement Proposal），涵盖协议层面的各种改进，ERC 则是 EIP 的一个子集，专注于应用层标准，主要定义智能合约的接口规范。以图示为例：
...
ERC-20 是最广泛使用的可替代代币标准，它定义了可代替「代币」的基本形态，也是 DeFi 的基石。在这里，可替代性意味着两点，其一是每个代币完全相同，可互换；其二是类似于现实中的货币，比如 1 美元等于 1 美元。

ERC-20 的典型应用场景包括稳定币（USDT、USDC）、治理代币（UNI、AAVE）、项目方代币，但面临以下问题，例如将代币误转到合约地址，合约没有处理机制，代币会永久丢失，合约再收到代币后无法自动执行逻辑。这些问题便催生 ERC-223、ERC-777、ERC-1363 等改进标准。

## **ERC-721**

ERC-721 是 NFT 标准，定义了非同质化代币（Non-Fungible Token, NFT）的标准。相对于 ERC-20 的可替代特性——这里包括 ERC-20 没有代币标识，只有数量，可进行分割的特点，ERC-721 的非同质化则强调每个代币都有唯一的 `tokenId`，让每个代币都变得独一无二，因而代币之间不可互换——打一个或许恰当的比方，这类似于艺术品梵高的《星空》不等于其他任何画作。

由此也不难理解， ERC-721 的典型应用包括数字艺术（CryptoPunks、Art Blocks）、游戏道具、会员、门票等等。

## **ERC-1155**

ERC-1155 是多代币标准，或者说是一个混合代币标准，允许在单个合约之中管理多种代币类型——同时包含可替代代币和非同质化代币，并且支持批量操作，从而降低 Gas 成本。

ERC-1155 的应用场景和 ERC-721 有相近之处，同样包括游戏道具（武器、装备、材料、金币）、票务系统、收藏品和 DeFi 头寸。

## **ERC-165**

ERC-165 定义了一种标准方法来发布和检测智能合约实现的接口，通俗来说是可以让合约「自报家门」，告知可以支持哪些接口，因而也可以称之为「接口检测标准」。

例如，如果需要调用一个合约的 `safeTransferFrom` 方法，但不知道该合约是否真的是 ERC-721，便可以先调用 `supportsInterface()` 检查合约是否支持某接口。

## **ERC-223**

ERC-20 的一个严重缺陷是，如果用户不小心将代币转给一个不支持代币的合约地址，代币便会永久卡在合约之中，无法取回，ERC-223 的作用就是解决 ERC-20 代币误转到合约后无法取回的问题，因为不再需要 `approve` + `transferFrom`，不过它并不向后兼容 ERC-20，因此生态采用率低。但这一概念被吸收到后来的 ERC-777 和 ERC-1363 等标准之中。

## **ERC-827**

ERC-827 扩展了 ERC-20，允许代币持有者在转账或授权的同时，调用接收方合约的特定函数。

但 ERC-827 存在安全风险，因为恶意的 `data` 参数可能会被用来执行任意调用，导致重入攻击等问题，所以 ERC-827 标准没有被广泛采用，ERC-1363 则成为其更安全的替代方案。

## **ERC-1046**

ERC-20 没有标准方式来存储代币的详细信息，包括代币图标（Logo）、项目描述、官方网站链接和社交媒体信息。ERC-1046 的出现可以解决该问题，它是可扩展元数据的代币标准，为 ERC-20 代币添加了标准化的元数据支持。

## **ERC-6093**

ERC-6093 是为 ERC-20、ERC-721 和 ERC-1155 代币定义标准化错误信息的规范。传统报错中使用字符串错误，会消耗更多的 Gas，各项目错误信息格式也不一，同时前端也难以程序化处理。但引入 ERC-6093 之后，Gas 可以降低 30%，前端处理可以进行程序化解析，提供的信息丰富度包含上下文参数，而非仅仅静态文本。

或许这可以用银行取钱举例子，去传统的老式银行取钱时，如果柜员反馈交易失败，余额不足，客户还无法得知有多少钱、还差多少，但是在现代银行 APP 里，交易失败后会清晰、具体地显示余额的具体数值，以及所差的余额。

## **ERC-1363**

ERC-1363 是 ERC-20 的扩展，实现了可支付代币（Payable Token），在转账完成后可以自动触发接收方的回调函数，一步完成「转账」和「操作」，类似于以太坊原生 ETH 的 `receive()` 函数。
...
ER-3643 是面向真实世界资产（RWA）的合规型代币标准，专为受监管资产设计，比如：
...
以上几种 ERC 标准的关系可以被归纳如下：

![屏幕截图 2026-02-04 163354.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/residualsun1/images/2026-02-04-1770194050987-_____2026-02-04_163354.png)
...
在我看来，一个最简单的 Polymarket 的框架可以被理解为由「事件」和「市场」来构成。其中，事件主要是指对现实世界事件进行预测的主题，例如「2028 年美国总统大选」，而市场则如同事件的子集，是预测主题下的具体预测问题，例如「候选人1是否可以当选」、「候选人2是否可以当选」、「候选人3是否可以当选」。

既然预测市场平台是基于区块链的，那么自然无法少了「上链」这一环节。具体来说，每个「市场」，也就是具体的预测问题都需要在区块链上「登记身份」，作为市场在链上的「出生证明」——这就是「条件」。

创建市场时，CTF（Conditional Token Framework，条件代币框架）中的 `prepareCondition` 会被调用以注册条件，得到相应的的 `conditionId`：

```
conditionId = keccak256(oracle, questionId, outcomeSlotCount)
```

`oracle` 是预言机的合约地址，`questionId` 是问题标识符，`outcomeSlotCount` 则是结果选项数量，如果是二元市场，值为 2。这里绑定了预言机和唯一的问题 ID，在市场需要结算时，预言机会通过该 `conditionId` 发布结果。

头寸是用户持有的某市场某结果的份额，但 Polymarket 会将每个头寸实现为一个 ERC-1155 标准的可交易代币——TokenID。一般来说市场结果是二元的，相应地产生两种对应的 TokenID，一个对应 yes 的份额，另一个对应 no 的份额，它们代表了对同一预测问题的两种相反结果的头寸。

![02-03-1.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/residualsun1/images/2026-02-03-1770106184488-02-03-1.png)

关于「抵押品」部分的解释不是很能理解：

\> Polymarket 市场的押注资金均以稳定币 USDC (Polygon 上为 USDC.e，地址 0x2791...Aa84174) 作为抵押品。每份 Outcome Token 背后对应 1 USDC 的抵押，当市场结算时兑现。

但是能理解紧随其后所说的价格含义，比如价格 0.60 USDC 意味着花 0.60 USDC 可购买该市场 1 份 YES 代币。如果该结果最终发生，持有者可赎回 1 USDC（获得净盈利 0.40 USDC）；如果未发生，则该代币价值归零，损失全部本金 0.60 USDC。

![02-03-2.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/residualsun1/images/2026-02-03-1770106293812-02-03-2.png)

Ploymarket 从创建到结算，步骤大致如下：

创建市场 → 生成初始头寸代币 → 交易，撮合买卖订单 → 结算

每个步骤都包含着关键事件，通过事件中的具体信息可以理解其原理。

「创建市场」对应 `ConditionPreparation` 事件，包含 `conditionId`、`oracle`、`questionId`、`outcomeSlotCount` 等信息。这个事件在链上确认了某预言机地址与问题 ID 的绑定关系，相当于市场的建立。一旦发布，预言机（UMA OptimisticOracle）稍后将根据这个 conditionId 报告结果。

「生成初始头寸代币」对应 `PositionSplit` 事件，包含 `conditionId`、`collateralToken`（应为 USDC 地址）、`parentCollectionId`（一般为 0）、`partition`（拆分出的 indexSets 列表，如 \[1,2\]）、以及 `amount`（拆分抵押品数量）。该事件证明抵押品被锁定，并生成了对应数量的 YES 和 NO 代币。对于二元市场，拆分 1 USDC 通常会得到面值各 1 USDC 的 YES 和 NO 代币各一枚。最初的流动性提供者可能是做市商，他们将 USDC 拆分为两种头寸代币，并可以在订单簿上挂单提供买卖报价。(市场创建后，需要流动性提供者拆分出初始的 YES/NO 代币)

「交易」对应 `OrderFilled` 事件，每当买卖双方的订单在链上部分或全部成交时，都会触发该事件，记录交易的详情，包括：`maker` 和 `taker` 地址：做市（挂单）方和吃单方地址；`makerAssetId` 和 `takerAssetId`：成交时双方各自支付的资产 ID（Polymarket 将资产用一个 ID 表示：0 表示 USDC，非零则表示特定市场的头寸 TokenId）；`makerAmountFilled` 和 `takerAmountFilled`：各方成交的数量（对应各自资产的数量，整数形式）；`fee`：maker 方支付的手续费数量。

「结算」对应 `reportPayouts` 事件。当事件结果揭晓且到达市场设定的关闭时间后，预言机合约（例如 UMA OptimisticOracle）将把结果提交回 CTF 合约，公布各结果的兑付率。调用 `reportPayouts`，将相应 `conditionId` 下的头寸标记为可赎回：胜出的头寸代币每份价值 1 USDC，失败的头寸代币价值 0。用户随后可以调用 `ConditionalTokens.redeemPositions` 来赎回胜出代币的抵押品。Polymarket 当前使用 UMA 的乐观预言机机制，这意味着通常在预言机确认结果后，通过 Polymarket 前端或合约即可触发结算。结算完成后，对应的 YES/NO 代币可以兑换回 USDC，市场生命周期结束。总体可以概括为：

`ConditionPreparation` 证明市场的存在和参数 → `PositionSplit` 证明资金注入和代币铸造 → `OrderFilled` 记录交易交换细节 → `reportPayouts` 确认结果以供赎回。
...
对「应用」的强调似乎也反映了 Vitalik 对 Crypto 未来的构想，具体而言，如果整个 Crypto 行业堕落成 100% 炒币的地方，只有投机而没有应用，那等大家玩腻了，整个行业便会在无聊中走向死亡，因此 Vitalik 十分主张构建出真正的应用、价值，例如更好的 DAO、真正渗透进各行各业的去中心化应用、更开放的 DeFi。  

LXDAO 和 ETHPanda 应该正在进行这样的实践吧！恰好 Antonia 老师今晚在分享就业经验时也说到 Web3 行业里还是投机为多，在做 build 的人却是很少，这也是她认为这一行不大好的一面：面向的用户多是「赌狗」，加密经历不容易被币圈以外的市场认可，行业混乱且不稳定，链上收入和社会身份也是灰色的。当然这是可以看到的「坏」的一面，要说也有「好」的一面，远程工作为多，而且不是兼职性质，且对 KPI 要求不高，不会像国内的一些公司那样对实习生或刚入职的员工提出过于超出业务范围的要求，以及……如果祖坟位置比较好的话可能会有概率直接发财，这辈子都不用工作了（笑）。

Antonia 老师给我们的感觉十分通透，用 Swen 的话来说就是「有一种拉几把倒的通透感」。有同学问起怎么为求职 Web3 学习，特别是提升技能，老师说可以直接将一些知名仓库源码扒下来然后和 AI 交流，增强自己对行业认识的情绪和浓度，或者多点自己进行开发，抄 GitHub 上的一些代码也可以，在此基础上进行修改——不要有太重的做事情包袱，特别自己还不是一个成名的人——Web3 虽然现在也会追求现插现用，但是对经验、经历没有卡得太死，把个人能力数据、成果摆好更重要，哪里缺乏了就补足哪里，有时候确认我们做了什么事情比我们做得怎样更重要。

Antonia 老师还说到自己刚毕业那会儿是一个什么都会一点，但又不深的状态，因而决定和聪明的人待在一起，看看别人在做什么，自己可以培养什么能力，寻找自己的长板，调侃就算自己是翻译专业出身，但进入 Web3 也还是好好活到了现在。可以说人生其实就不是线性的，只是我们很多人许多时候带着线性的思维去看待。同样，很多事情也不是我们个体可以去轻易左右的，而是那些手握上千亿资产的少数人，作为普通人能好好活着，今天能有收入，明天能有收入，每天问问当下的事情有没有做好，其他则都是运气和机缘了。
...
今天就是休闲黑客松的 Demo Day 了，说不紧张是假的！我感觉我们的项目产品在技术上基本无法再进行改动了，唯一能做的就是在宣讲中尽可能把叙事做好。花了半天的时间把要宣讲的稿子写好，然后是制作完善 PPT，下午再次主动把队友们召集起来开会，我主动过了一遍宣讲的流程，小白帮我计时，给予反馈，然后重新录制 Demo 视频，最后我们晚上也在九点多顺利完成了宣讲。

我认为自己在此次黑客松中很是幸运。小白是一个执行力很强的人，我提出任何产品可以改进的地方，经过讨论确认以后，他转头哐哐就是一顿修改，修改完代码便马上反馈——至少在一定期限的技术层面要求上，小白比我执行力要高很多，我反倒是更喜欢提出建议，而后为产品进行宣讲设计。我不知道可以不可以说，我在此期间体验到了产品经理的感觉（？但不是说自己就不适合技术，而是强调终于体验到了一种新的角色，甚至是一种可能可以参考的规划路径。当然，我还是对技术抱有好奇心，在此次实习计划中，我虽然没能学好 Web3 的技术，但也涉猎智能合约，学了一些基础语法，用过 foundry，更重要的是可以开始进行 Vibe Coding 了，知道有哪些 IDE 可以用，又可以如何在终端内置 Agent，知晓怎么通过更经济的方式获取大模型 Token。我觉得不一定非得明确什么路径，学习技术的过程也可以体验其他的角色，这本身就是一个整体的收获。  
  
另外，此次黑客松见识到了很多大佬们的产品项目，有些我听不懂但感觉很厉害，有些我能听懂也同样感到很厉害，「大逃杀」、Moxan 的 Web3 无障碍、Web3 与自动驾驶，排在后面等待宣讲时我很紧张，因为前面的大佬真真实实太厉害，都说休闲黑客松，其实应该是「修仙」黑客松——孩子误闯天家。必须承认，我感受到挺大的心理压力，特别是看到与我们产品理念相近的「大逃杀」项目做得很有趣味性，UI 也很抓眼球，叙事性也很不错，之后是 Moxan 的路演结束后，赞助商评委更是青睐有加，我渐渐感觉自己不好意思上场。但很快就调整好心态，怎么说都是完成一件事物比退缩更好，我的能力摆在这里，把我能做的做好就可以了，因而最后我也坦坦荡荡（虽然很紧张好像不小心开了摄像头，还漏了一些内容没讲）、大大方方地宣讲完了我们的内容。我在产品的 idea、PPT 设计上花了一点小心思，在产品理念中夹带了私货，事后似乎发现 Sue 老师挺认可我们的项目，也有一些小伙伴觉得我做的 PPT 页面很好，以及项目能满足一些需求，这些都给了我鼓励。

不过呢，宣讲结束以后，我还是想起前段时间在群聊里看到学员发的一个表情包，上面写着：一路走来没有敌人，全是老师——这段话构成了我对此次黑客松的想象和理解，我慢慢地减少了许多焦虑和自我埋汰，反而觉得自己能坚持参与，以及参与此次冬季实习计划是一件无比幸运的事情。
...
小白给本地测试网站录制了视频，我们之前讨论的功能基本能够得到实现，但 AI 验证部分却还存在漏洞。我将源代码下载到了本地，从我这里进行调试，但是频频出问题——先是 AI 无法调用，后是钱包无法显示代币余额，再有列表无法显示已经创建的任务。

逐一排查问题，AI 调用的问题出在没有真正创建 .evn 文件，并在其中正确调用 Deepseek 的 API，而是将 API 放在 .env.example 文件中，关键在于正确创建 .evn 文件。完成这一步后，在任务创建一栏输入相关文本，即可调用 AI 进行对任务进行拆解和分析。关于钱包代币的余额显示，则在于小白还在本地链上对 Demo 进行测试，我需要网络切换到 Anvil，并给自己 MetaMask 中调入 Anvil 提供的账户的代币。在 Claude Code 的协助下，前两个问题都得到了有效解决，但到最后一个问题时今日限额已用完……智能切换为 Codex，但目前还没有解决。  
  
另外，今天 Co-building 上小海老师分享了一些休闲黑客松的 Tips：  
...
-   流程
    ...
    -   0-30s：痛点是什么（不念 PPT，要讲故事）
        
    -   30s-2min：录屏操作，加速处理签名等环节
        
    -   2mmin-2min30s：架构图（着重标出赞助商 logo）
        
    -   2min30s-3min：商业模式和后续开发计划 哪怕代码跑不通，也可以通过视频剪辑将几段跑得通的代码拼接起来  
...
-   Devopst 平台，经常在截止前 10m 崩溃
    
-   提前 2 小时创建 project 界面，填写好 80% 的信息
    
-   最后更新：视频链接和 GitHub 链接
...
我们四个人都是第一次打黑客松，也不清楚在比赛中一个团队具体的协作流程，我能做的就是积极发言，分享自己能想到的点子，围绕小白做出的 Demo 提供建议，在会议的最后「自作主张」地给大家分配了任务。
...
> 我们团队需要在五天时间里做出一个以 AI Agent 为核心的休闲黑客松产品，要求基于 SpoonOS Framework，结合 React Agent / Graph Agent 调用 LLM，构建以 Agent 为核心的可执行 AI 应用或工作流系统。我的团队成员已经构想出上述方案，但是我们还不知如何分工。按照一场黑客松来说，似乎要先形成产品理念、方案，在理念方案的指导下展开开发工作，所以我希望你能基于上面的构想和方案要求，为我提供一个基本的 FocusFlow 产品的设计方案，可以供我们团队成员共享，并且便于我们的分工。需要指出的是，我们都还是开发入门的新人，所以需要尽量详细一些、通俗一些。

据闻 Opus 是几个大模型里代码性能最好的，它也确实是很详细地拆解出了不同的分工，大概分为前端、智能合约、后端、AI Agent 和产品经理五个部分。我选择了 AI Agent 的部分，在 Opus 的指导下我进行了试验，完成了本地调取 API 的部分，才发现在 API 的调用上，可能 DeekSeek 方案的总体性别比要比其他模型更好。

我才做完 AI Agent 的第一环，小白就做好了 Demo，但是 UI 界面还有很多可以优化的地方，我打算明天在 UI 界面上进行优化设计的尝试。

晚上是几位助教对黑客松的经验分享，主要从选题、如何保证做完、如何展示三个方面进行展开。

## **1\. 黑客松选题**

选题总体而言是三个关键词：创新 + 前端展示 + 商业价值。

对于应用的 Dapp，如果缺乏**商业逻辑**的闭环是不行的，需要在商业层面上能自圆其说。因为多数黑客松的背后都有赞助商，不能太偏离「金主爸爸」的需求（业内人的评判）。

Echo 老师就举了一个例子，在一次黑客松上，有一个小组在链上做了一个关于电影的产品，从大学生的角度来看还可以，但是业内人士却看不到商业的价值，没落到点子上——这些比很大、很高难度的选题更重要。

## **2\. 黑客松如何保证做完？**

Draken：

-   人员配置重要，如果实在只有一个人，那就展示项目最小的一个功能，演示完整的流程。
    
-   Demo 视频可以剪辑，剪去匮乏的部分，突出 idea 的创新性、独特性。
...
-   评委看重概念验证——评委甚至不看代码，但是一定会看 Demo 视频。**Demo 视频很重要**。
...
-   团队成员如果大部分是技术，可以选比较老的、出现过的选题。
    
-   技术为主的可以突出技术实现，80% 的时间要弄清楚逻辑闭环，20% 的时间用于生成 Demo。
    
-   产品讲的是如何用，评委在意的是**痛点是什么**。
    

## **3\. 黑客松怎么展示**

-   Demo 只展示高光，标题、logo、一句话说清楚做什么、核心交互、操作面板、展示区。
    
-   简要说清楚技术栈，核心演示结束后要展示技术架构，比如用到什么协议、功能。
    
-   技术闭环要展示到什么程度，比如节省了多少 gas。
    
-   最后要说愿景——让 Web3 更简单？
    
-   评委一定会看 readme，要有结构：解决什么痛点、方案，赞助方的协议。
    
-   边讲边演示，展示核心功能。
    
-   PPT 简洁，可以不需要章节目录，展示做什么，有什么痛点即可，演讲者补充细节即可，核心是看见功能。
    
-   PPT 丑一些也没关系，关键是吸引眼球。
    
-   录演时间大概 10 分钟，最后有 3 分钟被评委提问。
    
-   在黑客松中多 socilize，多了解一下别人的想法。
    
-   虽然不是必须，但是强烈推荐录制 Demo 视频——因为现场演示有可能会出现问题，比如一直 block pending。而录制好视频可以规避一些问题，而且剪辑可以节省时间。
    
-   马铃薯的补充： 线上＋时间长的黑客松，在沟通上一来一回很浪费时间（作息不一样、慢回等等），所以想了一下方法：
    
    -   开始做的时候要说自己在做啥：让大家知道谁正在做啥，防止做的部分冲突，也提供有人陪伴在做事的安全感，也让别人知道这些是谁在做的。
        
    -   维护已完成的部分：确保大家了解目前进度。
        
    -   维护待开发（未完成）事项：让大家知道有哪些需要做以及还没做，有空的人可以解决，防止状况外。在一开始就可以先列出有哪些需要开发。正在做的人有想到的都可以去添加。
        
    -   核心：减少沟通成本，确保大家状况在线。
        
    -   最重要：大家要知道自己要做啥，不懂、有问题要马上沟通。
...
晚上 Hardman 老师和我们分享了黑客松的经验，有好几点让我感到很是受用，在此记下：

-   对待黑客松：学习新技术的过程就是边学边打。
    
-   黑客松是一个验证新想法很好的平台。
    
-   推荐的黑客松比赛：ETHGlobal。
    
-   ETHGlobal 往届的作品：[https://ethglobal.com/showcase](https://ethglobal.com/showcase)
    
-   其实许多黑客松的背后都有着赞助商的资助，因而评奖标准也与赞助商的倾向紧密结合，项目有时要与赞助商的想法结合在一起。
    
-   在 AI 时代，人人都是产品经理，人人也都是全栈开发。
    
-   在黑客松中找怎样的队友？从身边下手，找熟悉的、互补的、靠谱的，通常最大人数 5 人——两个开发者，一个pitch，一个产品？
    
-   UI、前端在项目中比较重要，因为评委最直观和首先看到的就是界面（说起来我对计算机产生兴趣也正是从一个可见的网站界面开始）。
    
-   但更重要的是，黑客松主要还是要**完整地呈现自己的想法，哪怕是交一个视频**，**流程完整比功能实现更重要**。
    
-   Hardman 老师推荐的工具：TTmaker（[https://ttsmaker.com）（https://www.openai.fm）。](https://ttsmaker.com）（https://www.openai.fm）。)
    
-   在开发组件和框架上，如果项目不大的话，next.js 一把梭。
    
-   区块链行业很看重声誉（想起 wachi 老师说每一次小任务、每一次分享，都是在积累自己的 Credit）。
    
-   黑客松基本的时间节奏：第一天睡一觉，第二天爆肝，做完一个产品，然后留下时间做ppt、视频。
    
-   关于不同大模型的比较：Claude 代码能力很强（技能点都梭哈在了代码方面），Codex 的推理能力很强，Gemini 则可能是前端的页面设计会好一些。
    
-   Vibe Coding 关键是上下文管理能力。
    
-   Vibe Coding 需要拆解问题，将复杂问题简单化，小问题丢给 AI，最后再执行——要合理应用 plan mode。context 也很重要，要勤用。
    
-   Vibe Coding 要多跟 AI 请教（比如我有什么没想全的吗，你有什么不清楚的可以问我）。
    

最后，今晚和小白老师、ChiliChili 还有功夫小马组好了黑客松队伍。
