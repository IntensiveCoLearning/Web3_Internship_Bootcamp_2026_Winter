...
今天是web3冬季训练营第二周的第一天，以下是今天的具体学习计划：

-   学习[**Day 3: Intro to Ethereum Clients & Hardhat**](https://www.youtube.com/watch?v=7EwDOV4nEcY&list=PLJz1HruEnenAf80uOfDwBPqaliJkjKg69&index=8)。
    
-   学习[**Day 4 - Solidity & Testing with Hardhat**](https://www.youtube.com/watch?v=KjzH5EPCA-A&list=PLJz1HruEnenAf80uOfDwBPqaliJkjKg69&index=9)。
    
-   学习[**Day 5 - scaffold-eth Introduction and Solidity Recap**](https://www.youtube.com/watch?v=7pIzfm7hxQk&list=PLJz1HruEnenAf80uOfDwBPqaliJkjKg69&index=10)
    
-   完成挑战 Challenge #1 - Decentralized Staking App
    

## **观看 Intro to Ethereum Clients & Hardhat**

这个视频该视频核心讲解了如何使用 **Hardhat** 框架在本地环境和测试网上开发、测试及部署智能合约。

以下是内容的详细总结：

### **1\. 以太坊节点与客户端 (Clients)**

-   **运行节点的意义**：强调开发者应该尝试运行自己的以太坊客户端（如 Geth），而不是完全依赖第三方服务。这不仅能增强去中心化，还能提高数据读取速度和安全性。
    
-   **节点类型**：
    
    -   **全节点 (Full Node)**：下载并验证所有区块，存储当前所有状态。
        
    -   **轻节点 (Light Node)**：只下载区块头，通过向全节点请求证明来验证数据，适合资源有限的设备。
        
    -   **归档节点 (Archive Node)**：保存自创世区块以来的所有中间状态，体积巨大。
        

### **2\. Hardhat 开发框架**

-   **核心功能**：Hardhat 被描述为以太坊开发的“瑞士军刀”，它提供了一个本地以太坊网络环境，并支持编译、部署和自动化测试。
    
-   **本地环境**：演示了如何启动 `npx hardhat node`。该命令会生成 10 个带有 10,000 ETH 的测试账户。视频提醒：千万不要在主网使用这些公开私钥的账户。
    
-   **任务系统**：介绍如何通过 Hardhat Tasks 编写自定义脚本，简化日常开发流程。
    

### **3\. 合约开发与编译**

-   **合约编写**：以 `Greeter.sol` 为例，讲解了 Solidity 的基础结构，包括构造函数（Constructor）和状态变量（State Variables）。
    
-   **编译产物 (Artifacts)**：
    
    -   **ABI**：定义了如何与合约进行通信的接口。
        
    -   **Bytecode**：编译后的机器码，实际部署到区块链上的内容。
        
-   **Console.log**：Hardhat 允许在 Solidity 代码中使用 `console.log`，这对习惯 Web2 开发的程序员来说是非常强大的调试工具。
    

### **4\. 自动化测试**

-   **Waffle 与 Chai**：讲解了如何使用 Mocha 和 Chai 编写 JavaScript 测试脚本。
    
-   **测试逻辑**：演示了如何编写测试来验证合约的初始状态、修改状态后的反馈，以及使用 `expect` 关键字来断言交易是否成功或报错（Revert）。
    

### **5\. 部署到测试网**

-   **流程演示**：展示了将合约部署到 Rinkeby 测试网的步骤。
    
-   **环境配置**：强调使用 `.env` 文件保护敏感信息（如私钥和 Alchemy API Key），避免代码上传到 GitHub 时泄露私钥。
    
-   **Etherscan 验证**：演示了如何在区块链浏览器上验证合约源代码。验证后，用户可以直接在网页上调用合约函数，增加了项目的透明度和可信度。
    

### **6\. 前端集成建议**

-   **Ethers.js**：简要提到了前端如何通过 Ethers.js 库连接到 Hardhat 网络或测试网，从而实现 UI 与智能合约的实时交互。
    

## **观看 Solidity & Testing with Hardhat**

这一该视频向开发者介绍了如何通过 **Hardhat** 框架进行 **Solidity** 智能合约的测试。

以下是视频核心内容的总结：

### **1\. 开发理念：实验性修补 (Tinkering)**

-   **核心思想**：学习 Solidity 最好的方式是“写一点、部署、测试、查看效果、重复迭代”。
    
-   **工具推荐**：
    
    -   **Scaffold-ETH**：提供即时的前端界面，方便直观观察合约状态变化。
        
    -   **Remix**：基于浏览器的 IDE，适合快速原型开发。
        
    -   **Hardhat**：专业的本地开发框架，用于编译、部署和编写自动化测试。
        

### **2\. Solidity 核心语法基础**

-   **版本声明 (Pragma)**：解释了如何通过 `pragma solidity` 指定编译器版本。提到 0.8.0 版本后，Solidity 内置了数学溢出检查。
    
-   **状态变量 (Storage)**：这些变量永久存储在区块链上，修改它们的成本（Gas）非常高。
    
-   **可见性 (Visibility)**：
    
    -   **public**：会自动生成 Getter 函数。
        
    -   **private**：仅限合约内部访问，但注意在链上数据依然是透明可见的。
        
    -   **external/internal**：分别用于外部调用和合约继承内部调用。
...
-   **Require 语句与修饰器 (Modifiers)**：
    
    -   演示了如何使用 `require` 进行条件检查（如：只有管理员能操作）。
        
    -   介绍了 `modifier` 的用法，通过 `onlyOwner` 等修饰器实现代码复用。
        
-   **映射 (Mapping)**：类似于键值对字典（如 `address => uint256`），是存储余额和权限的核心结构，但无法被遍历。
    
-   **数组与结构体 (Arrays & Structs)**：介绍了如何定义复杂的数据对象并存储在数组中。
    

### **5\. 资金管理与转账**

-   **双向锚定与转账**：演示了如何编写 `buy` 和 `withdraw` 函数。
    
-   **发送 ETH 的陷阱**：视频特别提醒，在合约内发送 ETH 时，推荐使用 `call` 而不是 `transfer` 或 `send`，以避免 Gas 限制带来的问题。
    
-   **Receive 函数**：如果合约想要直接接收从钱包转来的 ETH，必须显式定义 `receive() external payable` 函数。
    

### **6\. 事件 (Events)**

-   **作用**：事件是存储在链上的非状态数据，其他智能合约无法读取，但前端可以非常方便地监听并获取历史记录。它是实现低成本上链存储的一种方式。
    

### **7\. Hardhat 自动化测试**

-   视频详细演示了如何使用 **Mocha** 和 **Chai** 编写测试脚本。
    
-   介绍了如何模拟多个账户（Signers）、部署合约、发送带金额的交易，并验证交易是否按预期成功或回滚（Revert）。
    

## **观看 scaffold-eth Introduction and Solidity Recap**

这段视频的核心目标是让开发者通过 **Remix**、**Hardhat** 和 **Scaffold-ETH** 等工具，对 **Solidity** 智能合约进行“实验性修补”（Tinkering），从而深度理解合约的底层逻辑。

以下是视频内容的详细总结：

### **1\. 开发哲学：快速迭代**

-   **“边修边学”**：Austin 强调不要只是死记硬背语法，而是应该“修改一行代码、部署、在前端看结果”。
    
-   **Scaffold-ETH 的作用**：展示了如何利用该工具自动生成的 UI 界面，实时观察合约状态变量的变化，这比在终端看日志要直观得多。
    

### **2\. Solidity 核心基础**

-   **版本与溢出**：提到了 Solidity 0.8.0 之后的重大变化，即编译器现在会自动处理数学溢出问题，开发者不再需要像以前那样必须导入 SafeMath 库。
    
-   **状态变量 (State Variables)**：
    
    -   解释了存储在“Storage”中的变量是永久上链的，且修改这些变量需要消耗大量的 Gas。
        
    -   强调了变量可见性（public, private 等）的区别：即使标记为 `private`，数据在链上依然是透明可见的，只是其他合约无法直接调用。
...
-   **View 与 Pure**：
    ...
    -   **Pure**：既不读也不改，仅进行纯数学运算。
        
    -   这两类函数在“外部调用”时是不消耗 Gas 的，这对降低用户成本至关重要。
        
-   **Payable 函数**：演示了如何让合约具备接收 ETH 的能力，以及如何通过 `msg.value` 获取用户转账的金额。
    

### **4\. 关键全局变量**

-   **msg.sender**：这是 Web3 开发中最常用的变量，代表当前调用合约的钱包地址，是权限控制（如 `onlyOwner`）的核心。
    
-   **block.timestamp**：当前区块的时间戳。视频提醒开发者，这个时间是由矿工/验证者提供的，虽然有小范围波动，但在大多数业务场景下足够精确。
    

### **5\. 常见数据结构实战**

-   **Mapping（映射）**：通过代码演示了如何将地址映射到余额或权限。Mapping 像是一个无限大的字典，但无法直接遍历。
    
-   **Struct（结构体）**：讲解了如何定义复杂的数据对象（例如一个“待办事项”包含内容和完成状态）。
    
-   **Array（数组）**：对比了固定长度数组和动态数组，并警告在循环中使用动态数组时要极其小心 Gas 耗尽的问题。
    

### **6\. 合约交互与转账**

-   **转账逻辑**：演示了如何使用 `transfer`、`send` 或更推荐的 `call` 方法将合约内的资金转给特定地址。
    
-   **Constructor（构造函数）**：讲解了构造函数仅在部署那一刻运行一次，通常用于初始化管理员地址或设置初始参数。
...
-   学习[**Day 1: Scripting and Smart Contracts! Providers, Signers, and Wallets**](https://www.youtube.com/watch?v=m2AJAcWk394&list=PLJz1HruEnenAf80uOfDwBPqaliJkjKg69&index=6)。
    
-   学习[**Day 2: Reading and Writing to Contracts with Ethers.js**](https://www.youtube.com/watch?v=9qt35swYSUg&list=PLJz1HruEnenAf80uOfDwBPqaliJkjKg69&index=7)。
...
## **观看 Scripting and Smart Contracts! Providers, Signers, and Wallets**

这个视频重点在于掌握 ethers.js 库的使用以及理解 Provider 与 Signer 的核心差异。在环境搭建上，应通过 nvm 管理 Node.js 版本以确保兼容性，并利用 `.env` 文件安全管理私钥与 API 密钥。核心交互逻辑围绕 Provider（只读连接）和 Signer（身份签名）展开：Provider 负责查询区块高度、账户余额及解析 ENS 域名等不消耗 Gas 的操作；而 Signer 则用于签署消息和发送交易。实战中，必须通过格式化工具（如 `formatEther` 和 `parseEther`）处理以太坊特有的 18 位小数大数字（BigNumber），避免 JavaScript 标准数值类型的精度丢失。

在实际发送交易时，开发者需要先构建包含目标地址和转账金额的交易对象，随后由连接了特定网络 Provider 的 Signer 进行签名并推送到内存池。通过 `tx.wait()` 异步等待矿工确认，是确保链上状态更新成功的关键步骤。整个学习过程强调了从只读查询到安全签名的转变，尤其是理解助记词推导路径与私钥导入的原理。

## **观看 Reading and Writing to Contracts with Ethers.js**

这个视频主题围绕如何使用 **ethers.js** 与智能合约进行读写交互。视频深入探讨了区块链交互的本质——即所有的合约调用实际上都是通过特定的“调用数据（Call Data）”发送的交易。

以下是核心内容总结：

### **1\. 脚本工具类与环境安全**

-   **工具类封装 (Utils)**：为了提高开发效率，视频演示了如何创建一个 `utils.js` 文件，将获取 Provider（连接区块链）、Signer（签名者）以及生成随机钱包的逻辑封装成可复用的函数。
    
-   **安全实践**：强调严禁将私钥或助记词直接写在代码中，必须使用 `.env` 环境变量文件，并确保该文件被加入 `.gitignore`。开发者应为每个项目生成独立的临时账户，并仅充入必要的测试币或少量资金以隔离风险。
    

### **2\. 智能合约的读取与 ABI**

-   **ABI（应用二进制接口）**：ABI 是与合约交互的“说明书”，定义了函数名、输入输出参数及其类型。视频展示了如何从以太坊浏览器（Etherscan）获取已验证合约的 ABI，并将其导入脚本。
    
-   **只读调用**：使用 `Provider` 实例化的合约对象只能执行“读”操作（如查询余额、获取代币价格），这种操作不消耗 Gas 且不需要签名。
    

### **3\. 智能合约的写入与交易解构**

-   **Signer 交互**：执行合约的“写”操作（如 `mint` 或 `transfer`）必须使用连接了 Provider 的 `Signer`。写入操作会产生交易哈希，并需要等待矿工打包确认（`tx.wait()`）。
    
-   **调用数据 (Call Data)**：视频展示了合约交互的底层原理——交易对象中的 `data` 字段。它由“函数选择器（函数签名的哈希前 4 字节）”和编码后的参数组成。
    
-   **原生交易模拟**：开发者演示了不使用 ethers 简洁的合约对象，而是手动构造包含 `to`（合约地址）、`value`（发送金额）和 `data`（调用指令）的原始交易来实现 NFT 铸造和代币转账。
    

### **4\. 实战演示：发送稳定币 (DAI)**

-   **跨合约联动**：通过主网 Provider 解析 ENS 域名（如 `sanford.eth`）获取目标地址，再通过 Signer 调用 DAI 合约的 `transfer` 函数。
    
-   **高级控制**：演示了如何手动设置交易的 `nonce`（随机数）和 `gasPrice`（油价）。通过发送相同 Nonce 但更高 Gas 的交易，可以实现对链上排队交易的“手动加速”或“覆盖”。
    

总体而言，本课强调了**“一切皆交易”**的理念：无论是转账、部署合约还是调用函数，本质上都是向以太坊发送一个带有特定字段的数据包。通过 ethers.js，开发者可以灵活地在“高层抽象接口”和“底层原始交易”之间切换，从而实现复杂的自动化脚本。

## **学习《021 学习以太坊》第五章**

这一章节是全书的技术硬核部分，它解释了以太坊如何从一个简单的“分布式账本”演变成一台“全球通用计算机”。

### **一、 EVM：以太坊的“大脑”**

EVM（Ethereum Virtual Machine）是智能合约的运行环境。你可以把它想象成一个安装在成千上万台计算机上的同一个“模拟器”。

-   **准图灵完备性**：EVM 可以执行任何逻辑（循环、条件判断等）。之所以说“准”，是因为它受到 Gas 的限制，不能无限运行。
    
-   **基于栈的架构**：不同于我们电脑 CPU 的寄存器结构，EVM 采用“栈”（Stack）结构（后进先出）。这种设计简单且易于在不同硬件上保持一致性。
    
-   **存储结构**：
    
    -   **Stack（栈）**：存放临时变量，存取极快但容量有限。
        
    -   **Memory（内存）**：合约执行期间的临时存储，执行完即销毁。
        
    -   **Storage（存储）**：数据永久保存在区块链上，成本极高。
        

### **二、 Gas 机制：资源的度量衡**

Gas 是以太坊防范风险和分配资源的核心手段。

-   **为什么要设置 Gas？**
    
    1.  **防止死循环**：如果没有 Gas，一段恶意代码可以让全球节点陷入死循环。
        
    2.  **资源定价**：计算需要 CPU，存储需要硬盘。Gas 确保用户为占用的物理资源付费。
        
-   **Gas Limit（Gas 限制）**：
    
    -   **交易 Gas Limit**：用户愿意为这笔交易支付的最大 Gas 数量。
        
    -   **区块 Gas Limit**：单个区块能容纳的所有交易 Gas 总和（目前通常为 30M），决定了以太坊的吞吐量。
        

### **三、 费用计算：EIP-1559 协议**

书中重点讲解了目前以太坊执行的费用模型，交易手续费由三部分决定：

1.  **Base Fee（基础费）**：
    
    -   由网络根据拥堵程度自动设定。
        
    -   **关键点**：这部分 Gas 会被直接**销毁**，不再给矿工/验证者，从而实现 ETH 的通缩。
        
2.  **Priority Fee（优先费/小费）**：
    
    -   用户主动支付给验证者的费用，用于诱导验证者优先打包你的交易。
        
3.  **Gas Used（实际消耗）**：
    
    -   由执行的具体操作码（Opcode）决定。例如：加法运算便宜，写入新数据昂贵。
        

**最终公式：**`Total Fee = Gas Used \* (Base Fee + Priority Fee)`

### **四、 开发者视角：Gas 优化意识**

由于链上资源极度昂贵，第五章强调了编写“省钱”代码的必要性：

-   **慎用 Storage**：每一次写操作（SSTORE）都会消耗大量 Gas，应尽量在 Memory 中处理逻辑。
    
-   **短路原理**：在逻辑判断中，把最可能跳出循环或返回结果的条件写在前面。
    
-   **批量处理**：在一次交易中完成多个动作比分多次交易更省钱（因为每笔交易有 21,000 Gas 的固定基础成本）。
    

### **五、 核心启发**

-   **确定性**：EVM 必须保证在任何机器上运行的结果完全一致。
    
-   **经济驱动安全**：Gas 不仅仅是手续费，它是一套精妙的防御机制，将技术上的“计算资源”转化成了市场上的“经济成本”。
    

## **学习中文排版规范**

以下是针对该指南核心内容的整理与学习总结：

### **一、 核心原则：空格的使用**

该指南最著名的规则就是关于“中英文空格”的处理。

NaN.  **中英文之间加空格**：
      
      -   **正确**：在 LeanCloud 上，我们可以轻松地实现。
          
      -   **错误**：在LeanCloud上，我们可以轻松地实现。
...
NaN.  **数字与单位之间加空格**：
      ...
      -   **正确**：这块固态硬盘的容量是 512 GB。
          
      -   **例外**：角度单位（如 `15°`）或百分比（如 `50%`）通常不加空格。
...
NaN.  **尊重品牌名（大小写一致）**：
      ...
      -   **正确**：iPhone, Google, GitHub, Android。
          
      -   **错误**：iphone, GOOGLE, github, android。
...
NaN.  **链接前后加空格**：
      ...
      -   为了防止链接与前后的中文粘连，建议在链接前后空一格。
          
NaN.  **避免过度强调**：
      
      -   不要在一段话中大量使用**粗体**、_斜体_或感叹号（！），这会造成读者的视觉疲劳。
