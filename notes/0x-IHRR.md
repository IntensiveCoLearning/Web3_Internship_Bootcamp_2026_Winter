...
为期一个月的 Web3 冬季实习计划可以告一段落了。

这一个月以来过得挺充实的。但怎么讲呢，跟我之前的状态也没什么太大区别，只是我之前周一到周五工作，周六周日休息，而这段时间周六周日可能还要学习和打卡。除此之外，倒没什么不适应的。

一个月下来，确实感触良多。虽然接触这个行业有两三年时间了，相对于一部分人来说有一些提前的经验，但大部分经验都停留在应用层面。这一次的计划确实提升了我对建设行业这一块的认知角度，包括链上协议的规范、链上的准则，以及一些来自真正在行业里工作的嘉宾分享的技术和运营视角。这为我扩展了一个相对较新的视角。

在行业里待着，很多应用自然而然就会使用了，但如果不对这些深层角度进行刻意学习和了解，其实很难接触到。像我虽然接触行业两三年，也是第一次在这次实习计划里学到这些东西。如果没有这样的契机，一般人是不会主动去了解的。这也是我觉得这次实习计划对我帮助很大的一个点。

另外，我也认识了一些非常积极、优秀的小伙伴，让我看到了外面的世界。

今天是打卡的最后一天，从明天开始大家就要回归到正常生活了。但对我来说，这可能是下一周学习的开始。目前接下来的任务不是很多，我还是希望能有机会在行业里找一份入门工作。

我确实觉得，如果要深耕一个行业，单凭自己摸索是很困难的。我不是那种天纵之才，没那个资本去硬碰硬，还是需要借助好的平台去深耕行业、深造自己。我觉得人是可以不断进步、不断成长和学习的，但确实需要一些外在力量的帮助。

今天我把任务完成后，没有再去学习新知识。我个人比较倾向于在周一到周五学习。

这次“残酷共学”确实让我打开了新的视角。我觉得这种活动真的挺好，意义重大。我很喜欢 ETHPanda 和 LXDAO 组织的这次活动，也非常感谢他们给了我参加这次活动的机会。
<!-- DAILY_CHECKIN_2026-02-08_END -->

# 2026-02-07
<!-- DAILY_CHECKIN_2026-02-07_START -->

**gas** 是 **EVM** 虚拟机处理各种复杂计算机操作时所做的工作量，**gas price** 是矿工的小时工钱，两个加起来就是 **gas fee**，如果在要执行操作前，给这个操作允许的 **gas** 低于实际操作所需的 **gas**，这笔操作就会失败，但消耗的 **gas** 已经被消耗了，**gas fee** 依然会被矿工拿取，因为矿工确实执行了 **EVM** 虚拟机的操作，矿工干了活，产生了工作量，所以 **gas fee** 他们还是拿走了，只是因为我允许的 **gas** 不足以支持完成这步操作，打个比方，我要走100步，但是在走到95步的时候，我没力气，走不动了，就算还剩最后5步，也不行，而且在 **EVM** 虚拟机中，只要没完成操作所有步骤，交易失败，所以已有操作就会回滚，就是钱给了，事没办成。（2022年以太坊更新过一次，刚刚跟人交流了一下，好像已执行的操作不会进行回滚操作）

**gas** 是一种计量单位，单纯为了计算操作的复杂程度，所以不能跟链上用于 **gas** 计量的 **TOKEN** 绑定价格，消耗了多少工作量，就计算多少 **gas**，再乘上我给出的 **gas price**，再等价换算成这条链上用于充当 **gas** 的 **TOKEN** ，就是我为这笔操作所最终需要付出的钱数。

so，我知道我这笔操作在链上最低需要多少工作量（**gas**），然后 **price** 是我自己定的，我就可以这样精准控制成本（实际情况肯定更加复杂，因为链上的 **price** 是跟**链上拥堵量**（就是同一时间在链上的交易量多与寡，就跟堵车一样，如果上下班高峰期，肯定是走的慢的，但上下班没有几个人有优先特权，链上你加钱就有特权）来决定的，如果给的 **price** 太低，矿工是不会处理这笔操作并打包上链的（不想搭理穷逼哈哈)（如果是富哥的话，把 price 给高点，那矿工肯定优先处理你这笔交易的，这也是夹子机器人和一些打新、**BTC** 链上抢区块的基本概念）

so，控制成本的简单总结就是，前提是没有特殊要求的（打新、防夹等），根据 **gas** 历史数据，找出一天或一周（看个人需求）的 **gas fee** 最低点，比如东八时区下午4点-6点（北京时间）**gas fee** 是相对最低的，就可以把一天所需要完成的交易集中在这个时间点来操作，要知道的必要知识就是我的操作是什么类型，不同类型的操作所消耗的 **gas** 是不同的，比如 **ETH 转账**消耗的 **gas** 数量固定是 **21000** （不添加备注的情况下，额外消耗的 **gas** 数量与备注内容长度有关，当然一般也没人会添加备注），**USDT** 转账的 **gas** 固定消耗就是 **54128** ，还有其它各种类型，如 **uniswap** 的 **swap**（**gas** 数量 **184523**），**zksync** 的跨链 （**gas** 数量 **143430**）等等，知道了类型所需的工作量要消耗多少 **gas**，就可以控制 **gas** 的数量，**price** 的话，有一些专门的网站可以看到链上的打包过程以及每笔未执行的交易情况，把 **price** 控制在中下或中等，要把成本降低，又要确保矿工会注意到这笔交易（毕竟没那么多人会插队，也要知道在等待的过程中，也还会有新的交易产生，如果把 **price** 拉到最低，就会被人一直插队，这样子就是时间成本会大一些而已）这样子操作下来，整体下来的 **gas fee** 成本就可以控制了，就算最后没省下几个三瓜两枣，也不至于钱花去哪了都不知道。
...
\- 链下程序（比如 DApp前端）可以 **监听事件** ，当事件被触发时执行对应的逻辑（比如更新页面）

**event 事件名(参数类型1 参数名1， 参数类型2 参数名2， .....);**

事件可以加一个特殊的修饰符 **indexed** ，被这个修饰符修饰过的参数可以被当作索引，存在日志的【索引区】，用于在链下用这个参数去进行快速过滤；没有被修饰的参数就只能显示内容，不能充当筛选参数，会存放在日志的【数据区】,但这里有个要注意的点：\*\*一个事件里面最多只能有 3个 indexed\*\* （因为区块链日志的索引字段最多就存 3个）

**emit 事件名(参数1， 参数2， ...);**

用 emit 关键字去触发事件（相当于在链上”记录这条日志“），反正就是在调用那个函数的时候，里面有个 emit ，就跟触发机关一样，家里来贼了，从窗户外打开了窗户，就自动在我手机上报警，日志触发，留存记录啥玩意的。

这个 emit 的参数需要跟 event 的参数匹配，event 里面有什么参数，emit 里面就要有什么参数，用 emit 去触发 event ，把调用函数传入的参数扔到 emit 里面，emit 再传给 event，然后生成日志

\`\`\`

contract Event {

event Log(string message, uint val); // 创建一个叫 Log 的事件，传了两个参数

event IndexedLog(address indexed sender, uint val); // 给地址类型的参数 sender 用 indexed 修饰了一下，后面就可以用 sender 这个参数去当索引了

function example() external {

emit Log('off', 1234); // 触发 Log 事件

emit IndexedLog(msg.sender, 789); // 在触发上面没多大区别，event 里面是什么类型，这边就传什么类型

}

event Message(address indexed _from, address indexed_ to, string message);

function sendMessage(address \_to, string calldata message) external {

emit Message(msg.sender, \_to, message);

// 触发日志，from当前调用者，to地址\_to,message发送了啥消息
...
这个市场确实有很多对长期主义者来说是噪音的东西，而且远比传统行业要多得多。要在这种环境下扎根生存，就要把眼光放得更加长远：从短期的波动，转向叙事，再到宏观层面的共识。

他在文章里提到一个观点我觉得很有意思：共识不是大家一起讲故事，而是人们的行为真实地发生了改变。很多人在寻找百倍币、千倍币，但在黄瓜猫老师看来，这可能有点舍本逐末，没有关注到真正核心的东西。他建议去找那些现在看起来很粗糙、但真正有人在用的产品，它们才有可能带来一波新的共识。

他还提了几个问题方便我们自省：如果没激励、没有吸引你的手段、币价也不再有明显的涨幅，你还愿不愿意在这个生态或这件事情中一直待下去？如果你愿意，这才有可能是共识，而且是那种能改变人们行为的共识。

看到这里，我立马去找 AI 探讨了很久，选出了两个我觉得比较可能的方向，今天还专门写了一篇文章。这两个方向分别是：

1\. 数据确权

2\. AI Agent 的劳动协作

我喜欢从大趋势或宏观往微观去看，但因为不是金融背景，一直缺乏这方面的知识储备，以前思考起来总是东一榔头西一榔头。黄瓜猫老师的这篇文章确实给了我一个梳理逻辑的机会，挺有收获的，起码让自己浮躁的内心慢慢平稳了下来。
...
1.  **节省 Gas：批量转账 / 批量 mint 时更省**  
    例如：给 100 个玩家发不同道具，用 ERC-721 可能要 100 次 `mint`，  
    用 ERC-1155 可以一次 `safeBatchTransferFrom` 或 `mintBatch` 处理。
    
2.  **一个合约管理多种资产，部署和维护更简单**  
    代币逻辑统一，资产用 `id` 区分，不需要每个资产都部署一个新合约。
    
3.  **FT + NFT 混合支持**  
    同一个合约里，部分 id 当 FT，用数量表达；  
    部分 id 当 NFT，用 `amount == 1` 来表示唯一性。
    
4.  **更适合游戏、元宇宙、道具市场、礼包等复杂场景**
...
-   ERC-20：`mapping(address => uint256) balances;`
    
-   ERC-721：`mapping(uint256 => address) ownerOf;`
    
-   ERC-1155：`mapping(uint256 => mapping(address => uint256)) balances;`
    

其中：

-   第一维 `uint256 id` 表示“资产类型”；
...
我现在会把 ERC-1155 想象成“游戏道具仓库标准”，几个典型场景：
...
1.  **链游道具系统**
    ...
    -   `id = 1001 ~ 2000`：不同编号的武器（NFT或半同质）
        
    -   全部放在同一个 ERC-1155 合约里，运营和交易所、市场合约都很好集成。
        
2.  **NFT 项目里的“道具 + 票券”**
    
    -   头像 NFT 本身可能是 721；
        
    -   但项目的“门票”、“签名券”、“空投券”等可以整合到一个 1155 里统一发放和回收。
        
3.  **礼包 / 批量空投**
    
    -   用 `safeBatchTransferFrom` 一次发多个 id 给一个用户；
        
    -   或者多个用户批量领取不同的资产组合。
        

这一类场景如果都用 ERC-20 / 721，会显得非常笨重。
...
-   ERC-20：同质化代币（FT），比如治理币、积分、LP 代币；
    
-   ERC-721：非同质化代币（NFT），比如头像、收藏品、独一无二的道具。
    

问题是：**很多游戏 / NFT 应用里，同一个合约里会有大量不同“种类”的资产**，比如：
...
如果只用 ERC-20 + ERC-721 来做：

-   每一种代币要一个合约（多种 ERC-20 就多份合约）；
    
-   每一类 NFT 也可能拆不同合约；
    
-   部署多、操作多、Gas 成本也高，管理复杂。
    

ERC-1155 就是为了解决这个问题的：**在一个合约里，同时管理多种 FT 和 NFT**，并且支持批量转账，降低 Gas。

一句话理解：

ERC-1155 = 一个“资产仓库合约”，里面可以发很多不同类型的 FT/NFT，还能一次性批量操作。

**2\. 核心概念：id 表示“资产种类”**

在 ERC-20 里，代币是“一个合约 = 一种代币”。  
在 ERC-721 里，`tokenId` 通常表示“某个具体 NFT”。

而在 ERC-1155 里：

-   每个 `id` 代表一种资产类型（比如“金币”“药水”“某个系列的 NFT”）；
    
-   每个地址对某个 `id` 有一个余额（`balanceOf(address, id)`）；
    
-   如果这个资产是 FT，那 balance 可以是 100、1000；  
    如果是 NFT，通常余额只会是 0 或 1。
    

也就是说：**“一份 ERC-1155 合约 = 多种资产的集合”**。
...
在这次黑客松（Hackathon）的过程中，我成长了很多。通过与队友开会、聊天以及讨论细节，我对项目有了更深的了解，也取得了长足的进步。

这就是今天做的一些事情。希望今晚的 Demo Day 能顺利进行，把我们的项目成功介绍出去。
...
今天还是在 Vibe Coding。因为今天是周六，所以跟团队里的小伙伴开了一个简单的会，做最后的一轮优化。

为了贯彻极简主义，我们已经在前端删减掉了很多功能，目的是让用户只专注于一件事情，不要让那些复杂的东西给用户添加杂念。

晚上的进展如下：

1\. 举行了一场 Space：

(a) 因为是第一次举行，业务不熟悉，加上还在写代码，准备得不是很充分。

(b) 过程中确实出现了一些临时情况，非常感谢各位老师的积极互动和临时救场。

(c) 总体来说虽然不那么顺利，但这种失败经验比一次成功的活动学到的东西更多。

2\. 提交代码：刚刚把认为还可以的一版做出来了，已经提交到 GitHub，准备迎接明天的 Demo Day。

在这个过程里确实学到了很多东西，这就是今天的一些进展。
...
此外，整个业务闭环也做得不够完善。确实做得有点累了，Web3 开发（Web3 code）有时候挺耗费心力的。不过今天也还好，起码做出了第一版 Demo，完善了一些自己之前没想到的想法和点，整体还行。
...
今天主要是在 Web3 Internship Bootcamp 课程里做 demo。但在设计过程中碰到了一些问题，主要体现在以下几个方面：

1\. 板块适配问题：

设计了三个板块，但板块之间的适配做得不够好，目前还在不断出现 bug。
...
目前还有一些技术难关没有克服，暂时也没想到最合适的解决办法。

虽然过程比较波折，但确实学到了一个项目从 0 到 1 的开发流程，这点我觉得很有意义和帮助。
