---
timezone: UTC+8
---

# Nahwordiss

**GitHub ID:** Nahwordiss

**Telegram:** @nahwordiss

## Self-introduction

Web3 实习计划 2025 冬季实习生

## Notes

<!-- Content_START -->
# 2026-02-08
<!-- DAILY_CHECKIN_2026-02-08_START -->
代币经济学（Tokenomics）核心笔记

1\. 定义：研究加密代币的发行、分配、流通、销毁及激励机制，决定代币价值与项目生命力。

2\. 核心要素

• 总供应量：固定/通缩/通胀，影响稀缺性（如BTC固定2100万枚）。

• 分配机制：团队、投资人、社区、国库的占比，决定抛压风险。

• 流通机制：解锁周期、质押锁仓，影响市场流通量。

• 效用价值：支付、治理、质押、生态赋能，是价值支撑核心。

• 销毁/通缩：回购销毁、手续费销毁，减少供应抬升稀缺性。

3\. 关键逻辑：代币价值=稀缺性+效用+共识，机制设计越合理，长期价值越稳。
<!-- DAILY_CHECKIN_2026-02-08_END -->

# 2026-02-07
<!-- DAILY_CHECKIN_2026-02-07_START -->

OpenCode+oh-my-opencode，核心总结如下： 1. **核心价值与优势**：OpenCode为开源工具，支持75+AI模型、拥有TUI可视化界面，搭配oh-my-opencode插件后，可实现7个专家智能体的自动协同工作，且能接入GLM-4.7、MiniMax等多款模型的免费额度，同时完全兼容Claude Code的所有配置，可无缝切换，工具本身免费开源，对比Claude Code更具成本和功能优势。 2. **7大智能体分工**：由Sisyphus统筹需求拆解与任务编排，搭配Oracle（架构）、Librarian（文档）、Explore（代码分析）、Frontend Engineer（前端开发）、Document Writer（技术文档）、Multimodal Looker（视觉分析）六大智能体，可并行执行任务并汇总结果，还能根据用户的模型订阅情况自动分配最优/免费模型。 3. **安装与配置**：需先检查Node.js（18+）、Git（2.23+）环境并安装Bun，通过官方脚本安装OpenCode；oh-my-opencode分两种安装场景，有官方订阅可一键安装并完成Claude/ChatGPT/Gemini认证，无订阅则需配置中转API，安装后可通过指定命令测试智能体协同效果。 4. **Agent自定义与调用**：支持OpenCode原生、Claude Code命令两种方式创建全局/项目级自定义Agent，且能直接共用Claude Code已创建的Subagents；调用方式分@符号直接指定智能体，或通过ulw指令让Sisyphus自动拆解委派任务，后者效率更高。 5. **日常使用与优化技巧**：基础可通过命令启动工具、Tab键切换模型，ultrathink关键词可开启深度推理模式；针对Token消耗快的问题，可利用内置LSP自动优化、配置免费模型、降低Sisyphus的模型规格、提供详细任务计划等方式节省；同时可通过~/.claude/目录实现与Claude Code的配置共享，简单任务用Claude Code，复杂任务用该组合。 6. **进阶功能**：预装Playwright（自动化测试）、Git Master（智能Git操作）两大Skills，自动启用Context7（文档查询）、[grep.app](http://grep.app)（开源代码搜索）、Exa（开发者搜索引擎）三大MCP，还有22个默认启用的Hooks系统保障使用体验，均可通过配置文件禁用不需要的功能。 7. **配套资源**：提供B站小白教程、中文详细教程、GitHub项目链接及官方文档，方便不同阶段用户学习和查阅。
<!-- DAILY_CHECKIN_2026-02-07_END -->

# 2026-02-06
<!-- DAILY_CHECKIN_2026-02-06_START -->


![截屏2026-02-06 16.58.25.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-02-06-1770368308099-__2026-02-06_16.58.25.png)
<!-- DAILY_CHECKIN_2026-02-06_END -->

# 2026-02-05
<!-- DAILY_CHECKIN_2026-02-05_START -->



![exported_image (1).png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-02-05-1770264208827-exported_image__1_.png)
<!-- DAILY_CHECKIN_2026-02-05_END -->

# 2026-02-04
<!-- DAILY_CHECKIN_2026-02-04_START -->




![exported_image (1).png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-02-04-1770193861952-exported_image__1_.png)
<!-- DAILY_CHECKIN_2026-02-04_END -->

# 2026-02-03
<!-- DAILY_CHECKIN_2026-02-03_START -->





Web3 简历与面试要点总结

一、简历核心要点

1\. 避雷指南

• 不要一套简历投所有岗位：Web3 细分领域差异极大，需要针对岗位和公司修改简历，突出匹配度。

• 避免信息堆砌：不要把所有经历都放进去，要围绕岗位需求筛选重点。

2\. 加分要素

• 基础要求：排版清晰、逻辑分明，让招聘方一眼抓住重点。

• 核心硬实力

◦ 链上经验：比如参与过的链上项目、合约交互经历，是 Web3 行业的关键凭证。

◦ 项目经验：学生会、社团独立负责的项目，或个人 Side Project，体现项目管理和推进能力。

• 技术岗额外加分：GitHub 提交记录、开源项目贡献、对口实习经历。

• 非技术岗额外加分：运营过小红书/B站/抖音等社交账号、组织过线下活动、对口实习经历。

• 进阶亮点

◦ 对行业热点、资金情绪的感知能力（比如快速跟进热点做了相关项目并获得好评）。

◦ 有链上赚钱的实际成绩（比如 NFT、DeFi 收益等）。

二、面试与求职核心建议

1\. 求职方向选择

• 交易所：可通过 CoinMarketCap CEX 排名找头部平台，关注其官网和公开招聘渠道。

• 媒体：国际头部有 Coindesk、Cointelegraph、Blockworks；国内头部有深潮、吴说、律动、PANews 等，是合规且适合新人的切入点。

• 避坑提示：谨慎选择小社区类岗位，这类岗位往往什么都要做，甚至可能出现拖欠工资的情况。

2\. 公司可靠性判断

• 学习基础技术后，可通过询问公司“在哪个链上做事”，来初步判断项目是正规 Web3 项目还是资金盘。

• 警惕“元宇宙”“RWA”等热门叙事的套壳公司。

3\. 新人职业路径参考

• 技术成长优先级：安全领域 > Layer1/Layer2 项目 > 交易所 > 节点商/矿池 > 其他方向。

• 运营/商务成长重点：更看重行业周期和遇到的老板，选对平台和引路人比短期薪资更重要。
<!-- DAILY_CHECKIN_2026-02-03_END -->

# 2026-02-02
<!-- DAILY_CHECKIN_2026-02-02_START -->






2026年2月区块链&DeFi全球经济动向

2026年开年二月，全球区块链与DeFi行业迎来剧烈震荡，宏观环境、二级市场、生态运行与全球监管同步发生关键变化，整体呈现去泡沫、强合规、重基建、严风控的核心发展基调，行业进入结构性调整与价值重塑阶段。

二级市场在本月出现大幅回调，比特币接连跌破关键价位，最低触及7.5万美元区间，创下2025年4月以来的新低；以太坊、SOL等主流公链代币同步深度下挫，全市场24小时内市值蒸发规模超1110亿美元。高杠杆合约出现集中踩踏式爆仓，涉及用户数量超40万，总爆仓金额约25.6亿美元，极端行情下的杠杆风险集中释放，市场投机泡沫快速出清。

DeFi生态在行情波动中暴露出阶段性风控短板，头部借贷协议Aave V3因极端行情下清算引擎响应延迟，出现约3.2亿美元抵押资产异常清算事件，引发行业对链上风控机制、极端行情适配能力的集中讨论与升级需求。与此同时，RWA现实资产代币化、链上贵金属衍生品交易展现出较强韧性，Hyperliquid等平台相关合约成交量逆势走高，成为波动行情中少数具备资金吸引力的细分赛道，也印证了真实资产锚定、落地场景支撑的DeFi模块具备更强抗风险能力。

全球加密资产与DeFi监管进入密集落地期，合规化成为不可逆转的行业趋势。美国CLARITY法案推进遇阻，稳定币监管相关条款争议持续发酵，立法进程不及市场预期；中国香港地区上调无担保加密资产风险权重，进一步抬高机构参与门槛，强化风险隔离；巴西于本月正式落地拉美地区最严格的加密资产监管规则，明确交易限额、持牌经营要求与反洗钱合规标准，对虚拟资产服务商实施全流程监管。全球主要经济体均从“探索包容”转向“规则落地”，野蛮生长的行业阶段正式结束。

宏观层面，美联储降息预期下调，全球美元流动性收紧，高风险资产估值承压，机构资金从比特币现货ETF等产品持续流出，市场驱动逻辑从投机情绪转向基本面与合规资质双重筛选。中小无落地项目、低效矿企与不合规交易平台加速出清，行业资源持续向头部合规平台、技术基建方与具备真实应用场景的项目集中，行业集中度稳步提升。

从长期发展视角来看，2026年2月的行业调整属于去杠杆、去泡沫的结构性洗牌，并非行业基本面的根本性逆转。跨链互操作、ZK隐私计算、RWA规模化落地等核心技术方向仍在稳步推进，DeFi行业的竞争重心从流量博弈、代币炒作，转向应用价值落地、风控体系完善与合规资质搭建。随着全球监管框架逐步完善、市场杠杆风险充分释放，区块链与DeFi行业将告别非理性繁荣，进入更稳健、更可持续、更贴近实体经济的长期发展周期。
<!-- DAILY_CHECKIN_2026-02-02_END -->

# 2026-02-01
<!-- DAILY_CHECKIN_2026-02-01_START -->







![截屏2026-02-01 09.05.17.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-02-01-1769907929413-__2026-02-01_09.05.17.png)
<!-- DAILY_CHECKIN_2026-02-01_END -->

# 2026-01-31
<!-- DAILY_CHECKIN_2026-01-31_START -->








Web3黑客松全流程学习笔记

一、基础定义

Web3黑客松是围绕区块链、DeFi、NFT、ZK、AI+Web3、跨链、预言机等赛道，在限定时间内完成项目构思、组队、开发、原型交付、路演、评审的密集型创作与竞技活动，是Web3行业最核心的Builder实战场景、人才筛选与早期项目孵化载体。

二、完整标准流程（通用线上/线下通用）

阶段1：赛前筹备（Pre-event）

• 报名与资质核验：填写个人/团队信息，部分赛事要求提前提交项目提案、赛道选择

• 赛前宣讲与工作坊：主办方/公链/协议方讲解主题、赛道规则、SDK/API使用、开发文档、评审标准

• 自由组队：线上社群/线下会场匹配产品、开发、设计、运营角色，补全团队能力缺口

• 资源对接：领取测试网代币、开发工具包、节点权限、导师预约通道

阶段2：正式开赛（Build Period，通常24–72小时）

1\. 开幕式：主题解读、赛道说明、赞助商与评审介绍、时间节点重申

2\. 项目立项与方案定稿：确定赛道、痛点、解决方案、技术架构、核心功能范围

3\. 密集开发：智能合约、前端交互、后端逻辑、链上数据对接、Demo交互闭环

4\. 导师问诊：中期进度检查、技术问题答疑、产品逻辑修正、路演结构建议

5\. 内部联调：代码合并、Bug修复、流程跑通、演示视频录制、文档撰写

6\. 截止提交：按要求上传GitHub仓库、产品文档、Demo视频、项目介绍页、团队信息

阶段3：路演与评审（Demo Day）

• 初筛：评审团按创新度、技术难度、实用性、完整性、合规性打分筛选入围团队

• 现场/线上路演：3–5分钟项目演示+2–3分钟问答，核心讲清痛点、方案、技术、价值、下一步

• 终审打分：综合技术实现、产品完整性、商业潜力、用户价值、赛道契合度评定名次

• 颁奖与公示：公布获奖名单、奖金/权益说明、孵化对接、投资/合作意向沟通

阶段4：赛后跟进

• 项目反馈收集：评审意见、用户建议、导师优化方向整理

• 原型迭代：从Demo打磨为MVP，完善合约安全、交互体验、文档与测试用例

• 资源对接：入驻加速器、申请 grants、对接VC、加入生态共建、社区运营启动

• 经验沉淀：流程复盘、问题总结、能力短板梳理、下次参赛优化方案

三、核心目的（个人/团队/主办方三维）

个人视角

• 快速验证技术能力：在高压环境下完成链上开发、合约编写、工具集成，补齐实战短板

• 搭建行业人脉：结识开发者、产品、设计师、投资人、协议方核心成员，积累Web3核心人脉

• 获得可见成果：产出可展示的Demo、GitHub仓库、路演视频，作为作品集与求职背书

• 争取激励与机会：获取奖金、测试网权益、 Grants、内推、孵化名额、全职offer

团队/项目视角

• 低成本验证产品可行性：在短周期内完成最小闭环，验证用户痛点与技术落地边界

• 快速获得专业反馈：从资深评审、生态导师处获得产品、技术、安全、商业化的直接建议

• 早期曝光与冷启动：面向行业社区、资方、用户展示项目，获取早期关注与潜在合作

• 团队磨合与分工验证：检验协作效率、角色匹配度、时间管理与风险应对能力

主办方/生态方视角

• 生态开发者招募：挖掘优质Builder，扩充公链/协议/应用的开发社区

• 技术场景落地：推动基础设施（公链、预言机、钱包、SDK）在真实场景的应用

• 早期项目筛选：发现高潜力初创项目，为投资、孵化、生态合作储备标的

• 行业影响力构建：聚集人才、输出案例、建立赛道标准与行业口碑

四、项目常见问题与反思要点

1\. 立项与规划类问题

• 问题：需求泛化、功能贪多、脱离赛道、未结合评审标准，导致时间严重不足

• 反思：必须做最小可行功能闭环，优先完成可演示的核心链路，砍掉非必要装饰性功能

• 优化：立项前对照评审维度（创新、技术、实用、完整、合规）做权重匹配，锁定1个核心痛点

2\. 团队与分工类问题

• 问题：角色缺失、沟通低效、任务边界模糊、开发进度不同步、后期无人负责路演

• 反思：明确前端/合约/产品/设计/路演专人负责，设置小时级里程碑，每日同步进度

• 优化：赛前1小时定分工清单、沟通频率、代码合并规则、紧急问题升级机制

3\. 技术与开发类问题

• 问题：合约安全漏洞、测试网环境异常、第三方SDK不稳定、跨模块联调Bug、文档缺失

• 反思：优先使用成熟工具链（Foundry/Hardhat/Ethers.js），避免现场啃全新语言/框架

• 优化：提前跑通基础demo、预留20%时间处理意外、只做现场可稳定复现的功能

4\. 路演与呈现类问题

• 问题：逻辑混乱、重点不清、演示卡顿、无法回答技术/产品问题、未突出赛道契合点

• 反思：路演严格遵循“痛点→方案→技术→价值→下一步”结构，Demo提前录制备用视频

• 优化：准备3分钟精简版、5分钟完整版，提前模拟问答，突出与赛事主题/协议的结合点

5\. 时间与风险管理问题

• 问题：前期构思过久、中期开发拖沓、后期仓促提交、缺少文档与演示材料

• 反思：以“提交物完整”为第一目标，而非功能完美，时间分配严格按阶段切割

• 优化：固定里程碑——立项10%、开发60%、联调15%、文档与路演15%

五、标准化复盘框架

1\. 基础信息

• 赛事名称/赛道/时长/团队角色/最终结果

• 核心功能与技术栈：合约语言、开发框架、链环境、第三方协议、前端工具

2\. 成果总结

• 完成模块：合约、前端、交互、数据、文档、演示视频

• 可量化产出：代码提交量、接口数、合约功能点、演示流程数、评审得分项

3\. 问题与根因

• 立项问题：需求边界、赛道匹配、评审理解偏差

• 协作问题：沟通、分工、进度同步、风险响应

• 技术问题：工具选型、环境、安全、联调、兼容性

• 呈现问题：路演结构、Demo稳定性、材料完整性

4\. 改进措施（可执行）

• 下次立项：先做最小闭环、对照评审标准倒推功能

• 团队协作：固定分工、小时里程碑、备用方案

• 技术准备：提前预跑基础Demo、锁定成熟工具、预留缓冲时间

• 路演准备：固定结构、双版本演示材料、问答题库

5\. 能力沉淀

• 新增技能：合约开发、链上交互、SDK集成、产品设计、路演表达

• 行业认知：赛道趋势、评审逻辑、生态合作方式、项目落地边界

• 人脉资源：导师、同期团队、主办方、生态方联系人清单

六、关键经验与通用建议

1\. 不追求完美Demo，追求可稳定演示的完整闭环，这是评审核心评分点

2\. 优先贴合赛事主题与协议方需求，更容易获得高分与资源倾斜

3\. 文档与提交物完整度同等重要，代码、说明、视频缺一不可

4\. 路演少讲技术细节，多讲用户价值与场景，评委更关注落地意义

5\. 赛后及时整理反馈并迭代Demo，是从参赛走向真实项目的关键一步

6\. 把黑客松产出全部归档，作为Web3投研/开发/运营岗位的核心作品集
<!-- DAILY_CHECKIN_2026-01-31_END -->

# 2026-01-30
<!-- DAILY_CHECKIN_2026-01-30_START -->









![截屏2026-01-30 12.32.29.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-01-30-1769747595586-__2026-01-30_12.32.29.png)
<!-- DAILY_CHECKIN_2026-01-30_END -->

# 2026-01-29
<!-- DAILY_CHECKIN_2026-01-29_START -->










![exported_image (1) 11.08.59.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-01-29-1769656184677-exported_image__1__11.08.59.png)
<!-- DAILY_CHECKIN_2026-01-29_END -->

# 2026-01-28
<!-- DAILY_CHECKIN_2026-01-28_START -->











![exported_image (1).png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-01-28-1769583525700-exported_image__1_.png)
<!-- DAILY_CHECKIN_2026-01-28_END -->

# 2026-01-27
<!-- DAILY_CHECKIN_2026-01-27_START -->












![截屏2026-01-27 11.59.08.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-01-27-1769486408130-__2026-01-27_11.59.08.png)
<!-- DAILY_CHECKIN_2026-01-27_END -->

# 2026-01-26
<!-- DAILY_CHECKIN_2026-01-26_START -->













x402笔记

![exported_image (1).png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-01-26-1769399723636-exported_image__1_.png)
<!-- DAILY_CHECKIN_2026-01-26_END -->

# 2026-01-25
<!-- DAILY_CHECKIN_2026-01-25_START -->














总结制作了区块链结构图

![截屏2026-01-25 09.40.47.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-01-25-1769305249395-__2026-01-25_09.40.47.png)
<!-- DAILY_CHECKIN_2026-01-25_END -->

# 2026-01-24
<!-- DAILY_CHECKIN_2026-01-24_START -->















共识机制原理

![exported_image (1) 10.04.50.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-01-24-1769220346123-exported_image__1__10.04.50.png)

，
<!-- DAILY_CHECKIN_2026-01-24_END -->

# 2026-01-23
<!-- DAILY_CHECKIN_2026-01-23_START -->
















![exported_image (1).png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-01-23-1769134854977-exported_image__1_.png)
<!-- DAILY_CHECKIN_2026-01-23_END -->

# 2026-01-22
<!-- DAILY_CHECKIN_2026-01-22_START -->

















![exported_image (1).png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-01-22-1769054237856-exported_image__1_.png)
<!-- DAILY_CHECKIN_2026-01-22_END -->

# 2026-01-21
<!-- DAILY_CHECKIN_2026-01-21_START -->


















![exported_image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-01-21-1768969127095-exported_image.png)
<!-- DAILY_CHECKIN_2026-01-21_END -->

# 2026-01-20
<!-- DAILY_CHECKIN_2026-01-20_START -->



















![exported_image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-01-20-1768889294082-exported_image.png)
<!-- DAILY_CHECKIN_2026-01-20_END -->

# 2026-01-19
<!-- DAILY_CHECKIN_2026-01-19_START -->




















![exported_image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-01-19-1768799164060-exported_image.png)
<!-- DAILY_CHECKIN_2026-01-19_END -->

# 2026-01-18
<!-- DAILY_CHECKIN_2026-01-18_START -->





















![IMG_5963.JPG](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-01-18-1768700429440-IMG_5963.JPG)
<!-- DAILY_CHECKIN_2026-01-18_END -->

# 2026-01-17
<!-- DAILY_CHECKIN_2026-01-17_START -->






















[https://www.notion.so/1-17-2eba0868b770804da936f64640ce78af?source=copy\_link](https://www.notion.so/1-17-2eba0868b770804da936f64640ce78af?source=copy_link)
<!-- DAILY_CHECKIN_2026-01-17_END -->

# 2026-01-16
<!-- DAILY_CHECKIN_2026-01-16_START -->























![AI 与 Web3 融合：Agentic AI 操作系统演进路径.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-01-16-1768555867565-AI___Web3____Agentic_AI_________.png)
<!-- DAILY_CHECKIN_2026-01-16_END -->

# 2026-01-15
<!-- DAILY_CHECKIN_2026-01-15_START -->
























![Web3 安全与风险趋势.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-01-15-1768447508747-Web3________.png)
<!-- DAILY_CHECKIN_2026-01-15_END -->

# 2026-01-14
<!-- DAILY_CHECKIN_2026-01-14_START -->

























![以太坊技术详解.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/Nahwordiss/images/2026-01-14-1768382894314-_______.png)
<!-- DAILY_CHECKIN_2026-01-14_END -->

# 2026-01-13
<!-- DAILY_CHECKIN_2026-01-13_START -->


























实习手册 区块链基本概念和以太坊概览

有疑问的几个概念及回答总结

**1\. 如何在 GitHub 上提交 Issue 或 Pull Request**

**提交 Issue：**

进入项目仓库，点击 "Issues" 标签

点击 "New Issue" 按钮

填写标题和描述（清楚说明问题或建议）

添加标签、指派人员（如有权限）

提交

**提交 Pull Request：**

Fork 目标仓库到自己账号

Clone 到本地，创建新分支

修改代码并 commit

Push 到自己的远程仓库

在 GitHub 上点击 "New Pull Request"

选择目标分支，填写说明，提交审核

**2.区块打包过程是什么，哈希函数的作用是什么**

交易被收集到内存池，矿工/验证者选择交易打包成区块

每个区块包含：上一个区块的哈希值、交易数据、时间戳、随机数

哈希函数（如 SHA-256）将区块数据转换为固定长度的唯一标识

任何数据改动都会导致哈希值完全改变，保证不可篡改性

**3.节点和区块链网络的关系：**

节点存储完整或部分区块链数据

节点通过 P2P 网络相互连接，同步区块和交易信息

全节点存储完整链数据，轻节点只存储部分数据

节点不是"端点"，而是平等的网络参与者

虽然单个节点可能有影响力，但整体仍是去中心化的，因为没有单点控制

**4.共识机制怎么运作：**

PoW（工作量证明）：矿工通过算力竞争打包权

PoS（权益证明）：根据持币量和质押时间选择验证者

确保网络中多数节点对账本状态达成一致

**5.不同类型的链**

**联盟链是什么，和公链有关系吗**

不是公链节点组成的，而是预选的组织/机构共同维护

半去中心化，只有授权节点可以参与共识

例如：Hyperledger Fabric，多家银行共同维护的金融网络

**私链和中心化有什么区别**

私链：使用区块链技术，但完全由单一组织控制

中心化系统：可能不使用区块链，使用传统数据库

区别：私链仍有不可篡改性、可追溯性等区块链特性，但访问权限受限

**怎么通过技术实现这两种链：**

联盟链：通过身份认证系统（CA证书）限制节点准入

私链：设置私有网络，不对外开放节点连接

两者都可以自定义共识机制（如 PBFT、Raft）以提高效率

**8\. 语义网 Web3.0 vs Web3的区别**

**语义网（Web3.0）：**

Tim Berners-Lee 提出的概念

强调机器可理解的网络，数据互联互通

技术：RDF、OWL、知识图谱、SPARQL

**Web3：**

基于区块链的去中心化网络

强调用户拥有数据所有权，去中介化

技术：区块链、加密货币、NFT、去中心化应用（DApp）

**区别：**

Web3.0 关注"智能化"，Web3 关注"去中心化"

两个概念有交集但侧重点不同

**9\. 公链效率问题和 Layer2**

**公链为什么会有效率低下的情况：**

**比特币**：每秒只能处理约 7 笔交易（TPS），交易确认需要 10 分钟，时间长。

**以太坊**：主网约 15-30 TPS，高峰期 Gas 费用飙升

对比：Visa 可处理 24,000+ TPS

**Layer2是什么：**

在主链（Layer1）之上构建的扩展方案

在链下处理交易，定期将结果提交到主链

例子：

**Lightning Network**（比特币）：支付通道

**Polygon、Optimism、Arbitrum**（以太坊）：Rollup 技术

好处：提高速度、降低费用，同时继承主链安全性

**共识层和执行层的区别：**

共识层是以太坊 2.0 架构中的概念

**执行层**：处理交易和智能合约

**共识层**：验证者达成共识，确定区块顺序

分层设计让系统更模块化、可扩展

**如何在网络体系中建立实感：**

尝试在测试网（如 Sepolia）发起交易，对比主网和 Layer2 的速度和费用

运行一个轻节点，观察区块同步过程

使用 MetaMask 等钱包体验不同网络的切换
<!-- DAILY_CHECKIN_2026-01-13_END -->

# 2026-01-12
<!-- DAILY_CHECKIN_2026-01-12_START -->



























📅 **日期**：2026年1月12日

📍 **事项**：Web3实习计划（冬季）第二期开营仪式

**记录一下开营仪式重点内容**

终于通过了筛选！从2700人中选出500人，感觉压力和期待并存。接下来四周要好好学习，争取能在年后找到实习机会。

### **关于助教**

今天了解到每个学员都可以选一个助教，相当于班主任的角色。

**重点记一下：**

在学习平台的"助教选择"功能里选

每个助教只有50个名额！要赶紧选，不然就满了

选完之后会显示"待审核"，等助教同意

助教不只是辅导学业，平台使用或其他问题都可以问

这期的助教80%都是上期学员，感觉会比较了解我们的情况

**特殊情况：**

如果是Permission List模式的同学，没有单独助教，但会有学习搭子群，群里也有助教老师。

_待办：尽快去平台选助教！_

### **四周学习安排**

**每周主题**

**Week 1**：基础认知和技能学习（这周）

**Week 2**：深度学习和技能进阶

**Week 3**：实习黑客松（运营组织活动，技术做demo）

**Week 4**：就业指导和岗位对接

**日常时间表**

**每天固定安排：**

下午5:00-6:00：线上共学时间（助教会在线答疑）

晚上：主题分享会

**每周固定活动：**

周一下午2点：以太坊中文周会，会讲区块链快讯和研报

周五晚上：例会，分享这周的学习成果

时间安排还挺满的，但都是下午和晚上，白天可以做其他事情。

### **打卡机制（重要！）**

从今天（1月12日）开始每天都要打卡！

**怎么打卡：**

在学习平台点"残酷共学打卡"按钮

必须提交学习笔记才算完整打卡

**规则：**

⚠️ 一周内连续超过2次没打卡 → 直接失去打卡资格

⭐ 每次打卡 = 40分

要注意不能断，连续两次就危险了。每天都要提醒自己记笔记和打卡。

### **学分排行榜**

有8个榜单，分得很细：

**按赛道分：**

运营赛道：周榜 + 总榜

入门技术赛道：周榜 + 总榜

深度技术赛道：周榜 + 总榜

**整体：**

整体周榜

整体总榜

周榜每周会清零重新算，总榜是一直累积的。

排名高的可以拿周边礼包、硬件钱包之类的奖励。虽然奖励是其次，但能激励自己好好学习。

### **推文模板**

思考了一个发推文的模板，可以分享到社交平台：

> 很荣幸成为Web3实习计划（冬季）第二期的500名学员之一！从2700人中脱颖而出，感谢 @LxDAO\_Official 和 @EthPandaDAO 提供的免费学习机会。

> 📚 接下来的四周将会：

> ✅ 系统学习Web3核心知识（去中心化、智能合约、安全防护等）

> ✅ 参加每日线上共学和主题分享会

> ✅ 参与休闲黑客松实践项目

> ✅ 获得就业指导和岗位对接机会

> 💪 选择了\[运营/入门技术/深度技术\]赛道，准备好认真打卡学习，争取在排行榜上有好成绩！

> 🔥 特别期待能在年后找到心仪的实习机会，也希望能和大家一起成长进步！

> #Web3 #实习计划 #区块链学习 #以太坊

**发推文小技巧：**

可以配开营仪式的截图

@相关社区账号增加曝光

根据自己选的赛道调整内容

加上自己的学习目标和期待

### **接下来要做的事**

去平台选助教（马上做！）

今天就开始打卡（第一次打卡）

发推文分享这个学习机会

确认加入学习群

下午5点参加第一次线上共学

晚上参加分享会

想一下自己这四周的具体学习目标

**个人感想**

这个计划安排得很系统，从基础到进阶，还有黑客松实战和就业指导。感觉如果能坚持下来，收获会很大。

最大的挑战可能是每天打卡，因为要写学习笔记。不过这也是好事，逼着自己每天都要学点东西。

希望四周后能真的掌握Web3的核心知识，找到好的实习机会！💪
<!-- DAILY_CHECKIN_2026-01-12_END -->
<!-- Content_END -->
