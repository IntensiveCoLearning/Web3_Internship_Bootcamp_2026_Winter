...
## **Reactive Network 本體：產品定位**

| 模塊 | 貼文描述 | 我怎麼理解（白話） | 可能的應用方向 |
| --- | --- | --- | --- |
| 定位 | 與 EVM 兼容的 dApp 執行層 | 像一個能跑 Solidity 的執行環境，但把「觸發邏輯」做成更事件驅動 | 事件驅動型 dApp、跨鏈自動化 |
| 核心概念 | Reactive Contracts（睿應式合約）採用「控制反轉」 | 不是人 call 合約才動，而是事件流觸發它動 | 自動化策略、跨鏈監控與回應 |
| 監聽能力 | 監聽多鏈事件 log | 像「跨鏈事件監聽器 + 合約自動執行」 | 跨鏈風控、清算、條件成交 |
| 狀態變更 | 自主判斷何時把資料傳到目標鏈 | 合約自己決定「何時跨鏈寫入狀態」 | 條件式跨鏈消息/同步 |
| 性能 | 專有並行化 EVM，快速且低成本 | 執行更快、更便宜的 EVM 計算 | 高頻事件處理、成本敏感 dApp |
...
| 分類（可作為資料庫 Tab / 模組） | RootData 會呈現的內容範圍（按文件原文歸納） | 典型欄位例子（便於落地） | 性質 |
| --- | --- | --- | --- |
| 基本信息 | 介紹、Logo、生態、標籤、成立時間、社交媒體鏈接等；透明度評分也把「必要基本信息」列為重要構成（官方鏈接、項目介紹、GitHub 代碼庫、開發文檔、成立日期、總部位置、所屬賽道等） | 名稱、官網、Logo、簡介、賽道 / 類別、標籤、成立日期、總部地點、社群鏈接、GitHub、Docs | 事實資料（需可驗證） |
| 日曆事件 | 主網 / 測試網、產品進展、代幣銷售與上線、品牌重塑、激勵活動等；透明度評分將「重要日曆事件」列為評分維度 | Mainnet / Testnet、TGE、Listing、Product Milestone、Campaign、Rebrand | 事件資料（時間敏感） |
| 融資背景 | 具體融資事件與金額、投資方名單；透明度評分將「融資和投資披露」列為評分維度 | 輪次、金額、日期、投資方、領投 / 跟投、公告來源 | 事實資料（強依賴公告與媒體） |
| 團隊信息 | 主要團隊成員信息，以及新的入職與離職動態；透明度評分將「團隊成員」列為評分維度 | 核心成員、職位、加入 / 離開時間、個人頁（LinkedIn 等） | 事實資料（高變動） |
| 代幣經濟學與解鎖 | 透明度評分把「代幣經濟學和代幣解鎖計劃」作為評分維度（比重 22%） | 是否 TGE、代幣資訊、分配、解鎖曲線、合約地址（如適用） | 事實 + 結構化數據（高敏感） |
| 透明度評分（衍生指標） | 衡量項目在平台上的信息完整性與時效性；分 5 檔等級；並公開評分維度與權重（基本信息、融資、團隊、日曆、代幣等） | Transparency Score、等級（A–F）、缺口提示（哪些維度不完整） | 平台計算（依賴底層資料完整度） |
| RD 熱度指數（衍生指標） | 以平台搜尋量、點擊量、評論量等維度歸一化加權；統計週期過去 24 小時，每小時更新；屏蔽異常數據 | 24h 熱度、Top 排行、趨勢變化 | 平台計算（主要靠站內行為數據） |
| RD 增長指數（衍生指標） | 綜合團隊、融資、市值、TVL、影響力、熱度等；含基礎指標與近一年變化的動態調節；融資分數隨時間衰減；每日更新 | Growth Index、近一年增長拆解、主要驅動因子 | 平台計算（依賴多源底層資料） |
| X 影響力（衍生指標） | 依「Top 關注者」與普通粉絲加權；Top 關注者是 RootData 選定約 5,000 個有影響力帳號；提供 7 天影響力（7d）等觀測 | Official X 帳號、影響力指數、7d 影響力 | 平台計算（前提是官方帳號綁定正確） |
...
| 題目 | 20 秒版 | 60 秒版 | 90 秒版 | 證據點 |
| --- | --- | --- | --- | --- |
| Q1 自我介紹與為何投 RootData | 我有 2 年企業金融 B 端客戶經營，累計服務 70+ 企業客戶，熟悉資料核驗、跨部門對齊風控並推進落地；近期做 Web3 營運與活動增長，能把「認證與資訊維護」做成標準化交付並帶來可歸因成效。 | 我在企業金融做 B 端客戶經營 2 年，日常高頻處理資料更新與核驗，並協同風控、法遵等單位，把問題拆成可追蹤節點推進落地。也做過部門季度存款推廣，單季帶動存款約 +5 億新台幣、5+ 新存款戶。近期參與黑客松營運，推進 4 場核心活動平均線上 100+，並策劃 side event，把參與行為和註冊綁成閉環，帶來 50+ 註冊。這些能力可直接遷移到機構入駐、認證通過率提升與資訊維護交付。 | （在 60 秒版基礎上補一段）我看這個職位最關鍵是「增長 + 交付」：既要把機構拉進來，也要把認證與資訊維護做得又快又準，減少補件往返。我擅長用清單化、證據分級、狀態看板把流程跑順，並用活動或誘因把關鍵動作（認證、補齊高權重欄位、更新）做成可歸因的轉化路徑。 | 70+ 客戶；存款 +5 億；5+ 新戶；平均 100+；註冊 50+ |
| Q2 你如何證明增長是你帶來的（歸因） | 我用「專案識別 + 前後同窗對照」做歸因：存款推廣是專案型方案，能用方案識別拉出新增戶與淨增存款，並對照活動前同窗日均變化，把增量鎖在專案上。 | 存款推廣目標是拉升部門存款餘額。我先透過客戶訪談與日常經營確認關鍵需求是「存款優惠額度」而非單純利率，然後跨產品、風控、法遵推進方案落地。成效是單季存款約 +5 億新台幣、5+ 新存款戶。歸因上，因為是專案型方案，新增與淨增可按方案識別與名單匯出，並用活動前同窗對照日均變化做復盤。 | （在 60 秒版基礎上補一段）若面試官追「自然增長怎麼排除」，我會補：用同類客群做對照、固定窗口（活動前後各 7 天或同長度）比較日均，並把名單與系統匯出對齊，確保增量不是季節性波動。 | 單季 +5 億；5+ 新戶；方案識別；前後同窗 |
| Q3 職位理解：三項核心工作是什麼 | 這職位可拆三段：機構入駐增長、認證與資訊維護交付、活動促活。最卡多在「申請到通過」，要用高權重清單與模板化流程縮短通過時長。 | 我用漏斗看：線索→申請→通過→資訊完成度→留存更新。你要做的事是：把機構推進申請；用清單與證據源降低補件往返、提升一次性補齊率與通過率；再用活動或誘因讓機構持續更新，拉動留存與活躍。 | （在 60 秒版基礎上補一段）我會把交付做成 SOP：客群分層、欄位分級（高權重先通過、低權重後補）、狀態看板與 SLA（截止時間、退補件規則、升級機制），讓流程可規模化。 | 指標：申請數、通過率、平均通過時長、一次性補齊率、更新頻率 |
| Q4 漏斗拆解與每段指標 | 四段漏斗：線索→申請（申請數/轉化率）、申請→通過（通過率/時長/一次性補齊率）、通過→完成度（高權重完成率）、完成度→留存（回訪率/更新頻率）。 | 我會用看板固定週追：申請數、申請轉化率；通過率、平均通過時長、一次性補齊率；高權重欄位完成率與資訊完整度；30 天回訪率與更新頻次。每段都有 owner 與 SLA。 | （在 60 秒版基礎上補一段）用這些指標的目的，是把「效率」與「品質」同時控住：只拉通過率不控品質會埋雷；只控品質不控時長會卡產能。 | 指標全套清單 |
| Q5 你認為最卡的一段、Top 2 原因、2 個 SOP、1 個目標指標 | 我選「申請→通過」最卡：原因是需求不清與補件往返。SOP 是高權重欄位模板 + 狀態看板與截止時間；主攻一次性補齊率與平均通過時長。 | Top 2 卡點通常是：機構不知道哪些欄位最重要、補件輪次多導致時長拉長。SOP 1：按客群提供高權重欄位清單與證據源示例；SOP 2：高權重先通過、低權重後補的節點設計，搭配狀態欄位與截止時間。主攻把平均通過時長壓下來、一次性補齊率拉上去。 | （在 60 秒版基礎上補一段）我會把「退補件原因 Top 5」做成模板化回覆，讓每次退件都可沉澱；並建立升級機制：超時、反覆缺口、或高價值機構走加速通道。 | 目標：一次性補齊率↑、通過時長↓ |
| Q6 客群分層與高權重欄位 | 我先分 VC / 交易所 / 項目方。以項目方為例，高權重先抓團隊/融資/可驗證產品與入口，先通過再補齊長尾。 | 分層後每層有自己的「高權重欄位包」。項目方優先：團隊與關聯實體、融資摘要、過往產品與可驗證連結、官方入口與一致性資訊。狀態欄位含已通知、待補件、已收件、已驗證、退補件、已通過、待更新。 | （在 60 秒版基礎上補一段）關鍵是把客戶思考成本降到最低：一頁式高權重清單 + 證據示例 + 截止時間，一次講清楚，減少來回。 | 客群：VC / 交易所 / 項目方；狀態欄位 |
| Q7 證據分級（Level 1/2/3） | Level 1 公開可交叉驗證；Level 2 文件影本；Level 3 資金往來證據。用分級平衡速度與可靠性。 | 我會把證據分級用在融資等敏感欄位：Level 1 先收雙方公告或多來源交叉；若有爭議或高風險再升級到 Level 2 文件；必要時用 Level 3 資金證據把關。 | （在 60 秒版基礎上補一段）同時把規則寫清楚：哪些欄位可用 Level 1 通過、哪些必須 Level 2，讓審核與客戶溝通一致，避免標準漂移。 | 分級：公告 / 文件 / 資金 |
| Q8 企業客戶棘手案例（STAR） | 我遇過市場急跌導致押品不足，目標是在合規前提下爭取補押或展延，三天內處理十餘戶，清算降至 0，補件 100%。 | 背景：股市急跌，押品不足恐觸發清算。目標：展延或補押，保護客戶權益。行動：先拉風險名單與缺口清單，再與風控、法遵收斂合規方案上呈核准，之後用標準化補件清單與截止時間推進客戶補押/還款並留痕核驗。結果：十餘戶、3 天內結案、清算 0 件、補件 100%。 | （在 60 秒版基礎上補一段）我把流程拆成節點：方案研擬與同步告知、核准啟動、客戶補件與時點管理、文件核驗留痕、結案追蹤，並沉澱成 SOP 以便下次黑天鵝直接套用。 | 十餘戶；3 天；清算 0；補件 100% |
| Q9 你怎麼做進度控管（表格欄位） | 我用看板控：客戶、狀態、缺口清單、截止時間、驗證清單、補退件、風險標記。 | （同 20 秒版）再加：Owner、最後更新時間、下一步行動，確保協作與節奏一致。 | （同 60 秒版）補一句：每個缺口都對應證據來源與驗證結果，讓「品質」可追溯。 | 欄位清單 |
| Q10 兩週增長實驗怎麼做（結合 RootData） | 我選提升通過率：干預是高權重清單模板；用時間切分準實驗，成功門檻通過率 +10% 或通過時長 -20%。 | 假設通過率低多源於需求不清與補件往返，我會上線高權重清單模板與缺口回覆格式，先用同一類客群做時間切分：一週舊流程、一週新模板，控住審核標準與來源一致。觀察通過率與平均通過時長，成功門檻設通過率 +10% 或時長 -20%。 | （在 60 秒版基礎上補一段）若達標就擴大覆蓋並沉澱 Playbook：退補件原因 Top 5、模板話術、SLA 與升級規則，讓成效可複製。 | 門檻：+10% / -20%；方法：時間切分 |
| Q11 反問 3 題 | 我會問：認證流程與 SLA 現況、漏斗最卡環節、過去最成功活動的成功指標與目前增長/活躍優先級。 | （同 20 秒版）補：如果我兩週做一個小實驗，你們希望我先改善哪個指標。 | （同 60 秒版）補：資訊維護最常見問題 Top 3 是否已有 SOP，若沒有我可優先沉澱。 | 反問聚焦 SLA / 漏斗 / 成功案例 |
...
| 類型 | Ethereum Layer 2 |
| 主網上線 | 2024-12-18 |
| 核心定位 | 合規友好（Compliance-Friendly）、機構級應用、TradFi 進入 Web3 的橋樑 |
| 技術棧 | OP Stack |
| 兼容性 | EVM 兼容；Solidity 合約可無縫遷移 |
| 性能口徑 | 測試網曾達 400 TPS（屬測試網表述） |
| 生態代幣 | $HSK（總量 10 億；回購銷毀機制） |

## **核心概念拆解**

| 問題 | 答案 |
| --- | --- |
| 是什麼？ | 由 HashKey Group 推出的以太坊 L2 公鏈 |
| 為誰服務？ | 機構／合規資產發行方／TradFi 進場團隊 |
| 跟一般 L2 的差異？ | 不走「極客去中心化敘事」，主打合規與機構級應用落地 |
| 核心護城河？ | 合規背景 + 面向 RWA／監管資產的設計取向 |

## **技術架構與開發者視角**

| 模組 | 描述 | 對開發者意味著什麼 | 可驗證的點 |
| --- | --- | --- | --- |
| 底層架構 | 基於 OP Stack 構建（與 Optimism、Base 同源） | 工具鏈與工程習慣可參考 OP Stack 生態 | RPC 穩定性、出塊時間、Explorer 體驗 |
| 兼容性 | EVM 完全兼容 | 合約可直接移植，無需重學語言 | 同一份合約在以太坊測試網與此鏈部署對比 |
| 性能目標 | 高吞吐、低手續費；測試網 400 TPS | 適合高頻、低額、機構流程型交易 | 實測：同筆交易成本、確認時間、失敗率 |

## **合規能力與機構需求**

| 機構常見門檻 | HashKey Chain 的取向（你筆記） | 可能對應的產品設計 |
| --- | --- | --- |
| KYC／AML 要求 | 基礎設施層支持 KYC/AML 驗證 | 白名單地址、權限分級、合規資產交易門檻 |
| 監管背書偏好 | 合規友好、香港 Web3 樞紐敘事 | 面向家辦、券商、資產管理方的合規發行與交易 |
| 可審計與可控性 | 更適合證券型代幣、債券代幣化 | 受限轉讓（transfer restriction）、合規借貸、鏈上外匯 |

## **$HSK 代幣經濟**

| 維度 | 內容 | 關鍵資訊 |
| --- | --- | --- |
| 角色 | HashKey 生態核心平台幣 | 是否同時用於多產品（交易所、鏈上、治理） |
| Gas | 鏈上交易手續費支付代幣 | Gas 定價、基礎費率、尖峰期成本 |
| 交易所權益 | 在 HashKey Exchange/Global 享手續費折扣與 Launchpad 優先權 | 具體折扣檔位、門檻、是否動態調整 |
| 治理 | 參與提案與投票 | 治理範圍：鏈參數、生態基金、合作案？ |
| 通縮機制 | 總量 10 億；集團利潤回購銷毀 | 回購頻率、銷毀地址、透明度與報表節奏 |

## **應用場景映射**

| 場景 | 為何適合 | 可能長出來的產品形態 |
| --- | --- | --- |
| RWA 代幣化 | 需要監管背書、合規敘事 | STO、債券代幣、基金份額、收益憑證 |
| 合規 DeFi | 機構需要 KYC/AML、權限控管 | 合規借貸、白名單 AMM、受限抵押品池 |
| TradFi × Web3 基礎設施 | HashKey 在香港持牌背景 + 政策落地期待 | 機構託管、清算結算、合規資產發行工具鏈 |
...
一張地圖把常見需求分成八類， 10 個基礎標準其實各司其職：  
可替代代幣 FT、NFT、多資產、介面宣告、轉帳回呼、元資料、錯誤碼、合規轉讓。

可替代代幣 FT 的底座：ERC-20  
ERC-20 是 DeFi 世界的普通話：  
balanceOf 查餘額  
transfer 直接轉帳  
approve + allowance + transferFrom 走授權代扣

DeFi 協議常用 transferFrom 入金，因為協議需要從使用者地址把資產拉進合約，授權就是給協議的「可扣款額度」。很多交互失敗也都圍繞 allowance 展開，例如額度不足、授權沒做、授權被覆蓋。

ERC-20 的常見坑  
最典型的是授權競態：從舊額度改成新額度時，若對方先花掉舊額度，可能造成意外支出。實務上常見做法是先把額度設為 0，再設新值，或使用更安全的授權模式。讀程式碼時看到 approve，第一反應是想到「授權生命週期」有沒有被處理好。

NFT 的底座：ERC-721  
ERC-721 的核心是 tokenId -> owner。它的授權有兩種：  
approve 授權某個 tokenId  
setApprovalForAll 授權一個 operator 管所有 tokenId

NFT 轉給合約地址時，如果對方合約不會處理 NFT，資產可能卡死。safeTransferFrom 會要求接收方實作接收介面，否則交易回滾，避免 NFT 進黑洞。

多資產與省 Gas：ERC-1155  
ERC-1155 解決的是「一個合約管理很多種資產」的問題，用 id 區分資產類型。它的強項在 batch：  
balanceOfBatch 一次查多種資產餘額  
safeBatchTransferFrom 一次轉多種資產

這在遊戲道具、會員權益、活動憑證、空投禮包裡非常常見。可以看到同一合約裡，某些 id 像 FT，某些 id 像 NFT，本質都是同一套餘額模型。

介面宣告：ERC-165  
ERC-165 是「自我介紹」。它提供 supportsInterface(bytes4)，讓外部合約或前端能問：是不是 ERC-721？是不是 ERC-1155？  
很多 NFT 市場與工具會先用 ERC-165 探測介面，再決定用哪套邏輯交互。讀程式碼時看到 supportsInterface，要意識到它是相容性的關鍵入口。

防誤轉黑洞的一條路：ERC-223  
ERC-20 的 transfer 不會管接收方是不是合約，也不管對方會不會處理。ERC-223 想解決「代幣誤轉到合約地址導致永久卡死」，做法是當接收方是合約時，必須走回呼 tokenReceived，否則直接 revert。

可以把它理解為強制收款方宣告「我會處理這筆入金」。它屬於早期改良路線之一，現實裡不如 ERC-20 普及，但讀到它時可以聯想到「防黑洞」這個設計目標。

付款體驗更務實的路線：ERC-1363  
在做產品時，會更常需要的是「轉帳後自動觸發業務邏輯」，例如付費即開通、入金即執行策略、充值即記帳。ERC-1363 就是把這個語義標準化：  
transferAndCall 轉帳後呼叫接收方 onTransferReceived  
approveAndCall 授權後呼叫花費方 onApprovalReceived

它讓「付款 + 執行」能在一筆交易內完成，減少使用者多點一次確認的摩擦。看到某個代幣支援 ERC-1363，就能聯想到它適合做訂閱、支付、商城、自動化金庫這類場景。

關於 ERC-827，務實的結論  
ERC-827 試圖把「授權或轉帳時順便呼叫第三方合約」做成擴展介面，看起來很強，但長期處於草案討論且被反覆指出安全風險。

在真正的專案裡，如果看到類似「轉帳時攜帶執行」的需求，優先考慮更成熟的交互設計與標準組合，而不是直接押注一個爭議較大的擴展。

讓 FT 也有展示層：ERC-1046  
NFT 有 tokenURI，ERC-20 通常只有 name/symbol/decimals。ERC-1046 把 tokenURI 引入可替代代幣，目標是讓代幣也能掛更豐富的元資料，利於錢包展示、資產列表、資料產品統一建模。

統一錯誤碼：ERC-6093  
很多合約失敗時只給一段字串，前端難以穩定解析，也更費 gas。ERC-6093 用標準化的 c

ustom errors，統一了 ERC-20、ERC-721、ERC-1155 常見失敗原因，例如餘額不足、授權不足、無效接收方等。

如果做的是客戶端、錢包或資料服務，這會顯著提升可觀測性。能把失敗原因精準映射成可讀提示，而不是讓使用者對著一串 revert 猜。

RWA 與合規轉讓：ERC-3643（T-REX）  
當資產走向現實世界，問題不再是「能不能轉」，而是「誰能持有、能轉給誰、出了事怎麼凍結或追回」。ERC-3643 把身份、合規規則、轉讓限制納入標準化框架，常見能力包括轉讓前校驗、角色權限、凍結、強制轉移、恢復機制等。

如果在檢視 RWA 專案，重點不該只盯著 transfer，而是盯「合規判斷在哪裡發生」「不合規會怎麼被拒絕」「管理員能做哪些強制動作」。這些決定了資產的監管可行性與風險邊界。

總結 :  
把這 10 個 ERC 串起來，就能從介面推斷產品形態，從事件推斷資金流，從授權模型推斷風險點。
...
-   Web3 实习手册[「行业前辈访谈录」](https://web3intern.xyz/zh/logic-frontend-journey/)
    

## **入行起步路線：預熱 → 小閉環 → 社群 → 開源**

| 步驟 | 具體動作 | 交付物（可放 GitHub / 履歷） |
| --- | --- | --- |
| 預熱 | 學錢包簽名、EVM 基礎 | 簡短筆記、操作紀錄、常見錯誤清單 |
| 小專案 | Mint NFT、DEX 數據可視化 | 可部署的 Demo、README、影片 / 截圖 |
| 社群 | 參與 DAO 共學 / 小型 Hackathon，先 Member 再 Contributor | 會議紀要、任務認領、週報 / PR 連結 |
| 升級 | 對開源錢包或交易所提 PR | 1–3 個高品質 PR（含測試、文件） |
...
-   [Solidity by Example | 0.8.26](https://solidity-by-example.org/) **Applications 部分**
