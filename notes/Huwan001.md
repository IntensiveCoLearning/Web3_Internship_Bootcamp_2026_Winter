...
以太坊生態的不能做solana的 (區別  
應用級的Dapp選技術賽道的就很吃虧  
選題不要太大，**小的切入點** 創新+前端展示+商業價值  
**很小的一个技术上面的创新**，或者是你**把东西做了一些集成**，然后以一个比较好的一个前端的呈现方式展示出来，就是已经很好了，一定要能够自圆其说，逻辑链是要通的。

**應用級**的可以+補充一些商業 (可以有甚麼價值等等  
**技術級**的，講好技術就行 不需要講商業價值 (指演講部分) ，著墨在這個技術價值即可  
(商業價值能夠自圓其說很重要

## 選題如何能做完?核心功能如何捨

團隊 約3-4人 1人solo how 邪修:最小的完成核心內容 -> **保證項目的最小呈現**  
去掉鏈上過程，核心看想法、創新點  
將配置寫到代碼，寫到合約。 最重要:POC(概念驗證  
**Demo很重要**，評委不看代碼，而是看視頻 時間不夠就留下帳本  
功能寫不完就放個圖片，黑克松**允許瑕疵**但必須**要有進展**

## **分享參加比賽的經歷 (4技術 1PPT**

-   (技術成員多的情況)  
    聯盟鏈加分，要和評委解釋有什麼技術加分、突破點，評委會看合約寫得如何
    
-   (產品成員多的情況)  
    80%討論做甚麼平台、做成甚麼樣  
    \-> 先交出一個初步的，**_有比沒有好、做完比完美更重要_**  
    團隊核心人員在技術及產品呈現上的配比，會影響給評委呈現的構成
    

PPT展示+Demo，一句話說清楚幹了什麼，說清楚核心錢包、操作面板  
\- 頁面展示 如何交互  
\- 邏輯展示用了甚麼協議  
\- 視頻必備以防萬一(展示有bug的狀況

XX 產品能夠解決 XX 痛點、簽名? 錢包?  
XX 協議解決了 XX 產品的 XX 痛點

**_Readme必定要寫_**，代碼部一定會看，但Readme一定看!!  
列出贊助商協議、提出合約地址

PPT不要放過多內容，展示大綱+講述技術系項  
PPT列出 核心功能、idea  
\-> PPT 是 show me  
\-> 演講 是 tell me
...
如何呈現技術閉環  
代碼亮出，說明解決了甚麼問題痛點，降低了XX，節省多少時間等等

## 土豆同學分享

线上＋时间长的黑客松，在沟通上一来一回很浪费时间（作息不一样、慢回等等），所以想了一下方法：
...
之前黑客松，透过这方法，只开过一次线上会议哈哈（初始讨论），很顺畅
...
硬男老師推薦 [https://bibigpt.co/zh-TW](https://bibigpt.co/zh-TW)
...
**Hardhat** 是一個用於**編譯**、**測試**、**部署**和**調試**以太坊軟件的開發環境。  
它可以幫助開發人員管理和自動化構建智能合約和 dApp 過程中固有的重複性任務。  
從最初的創建、測試、到交互和部署。
...
1.  TypeScript + Node Test Runner + Viem  
    較新
    
2.  TypeScript + Mocha + Ethers.js  
    較多人使用
...
-   **權限控制測試**： 未來會學到 `require(msg.sender == owner)`。如果到時候用 Account A 部署，卻切換到 Account B 點擊橘色按鈕（修改資料），交易就會**報錯失敗**。這就是切換帳戶最重要的用途——測試合約是否有正確保護資料 (這邊存著，目前看不懂，無法實際操作測試)
    

increment 翻譯為"遞增"  
在目前代碼中的作用就是把 `myNumber` 目前的值 **+1**  
**發起一筆交易** 指的是說 「對區塊鏈狀態發出的修改請求」，不一定是指點對點送錢

-   **舉例：** 想像區塊鏈是一個公共的大帳本。
    
-   **讀取（Blue buttons）**：你只是湊過去「看」一眼帳本，不用寫字，所以不用錢，也不叫交易。
    
-   **寫入（Orange buttons）**：你要拿筆在帳本上改數字（比如執行 `increment`）。因為這會改變帳本的內容，你必須**簽名（用你的帳戶）並付小費（Gas 費）給幫你記帳的人。這整個「簽名、付錢、改帳本」的過程，就叫做發起一筆交易**。
...
2.  在 Solidity Compiler 圖示中點擊Compile (將寫的 Solidity 代碼轉換為 EVM（以太坊虛擬機）能執行的**字節碼 (Bytecode)**。)
    
3.  編譯成功會出現綠色勾勾，之後部署的才會是的「新版本」
    
4.  部署 (Deploy) — 「上鏈發布」
    
5.  點擊左側「以太坊圖誌」的圖示 (Deploy & Run Transactions)，確認選好合約名稱後點擊 **Deploy**。  
    **用意：** 在區塊鏈（模擬環境）上開闢一個新的空間（地址），把你的代碼存進去。  
    **重要：** 因為區塊鏈不可竄改，每次改代碼都必須點一次 Deploy 產生一個**新地址**。
    

交互 (Interact) — 「**執行邏輯**」

| 按鈕顏色 | 類型 | 對應你的代碼 | 用意（你的行為） | 是否花錢 (Gas) | | 橘色 | Write (寫入) | store / increment | 發起交易。你要求區塊鏈「改帳本」，把數字變掉。 | 是 | | 藍色 | Read (讀取) | retrieve / myNumber | 查詢狀態。你只是「看」一眼帳本，不改內容。 | 否 |

## 想要成功看到數字 +1，你的標準動作順序應該是

1.  **點擊** `store` **(輸入 100)**：
    
    -   **用意**：給帳本一個初始值。
        
    -   **後台**：區塊鏈收到請求，把這個合約地址對應的空間存入 `100`。
        
2.  **點擊** `increment` **(不需輸入)**：
    
    -   **用意**：觸發你新寫的 `+1` 邏輯。
        
    -   **後台**：EVM 讀取目前的 `100`，計算出 `101`，然後**覆蓋掉**舊的 `100`。
        
3.  **點擊** `retrieve` **或** `myNumber`：
    
    -   **用意**：**刷新顯示**。
        
    -   **後台**：這一步最關鍵！因為網頁不會自動更新，你必須點它，Remix 才會去鏈上把最新的數值「抓」回來顯示給你看。
