...
有产出结果：我用vibe coding做出一款小工具，功能是：

**核心二：结果比过程重要**

看重结果：我设计出了XX产品；解决了XX痛点；每周帮用户节省XX小时；创新点在于XXX；Github上收获了15个star 缺点在于XX，打算下一步如何去改进？

关于vibe coding 的几点误区：

1.  技术背景先别从计组/计网/操作系统/数据结构四大件入手，这些东西偏向数学和底层逻辑，对于想参加黑客松或者做项目展示不需要做这些；
    
2.  想做一个产品应该从哪里开始：前端是用户能够看到的、能点击到的。最简单，最能带来成就感；数据库就是记录数据，存账号密码、帖子内容；后端就是中间的规则，现在逐渐标准化；
    
3.  新手只需要去专攻前端：**好反馈、好描述**(可以直接画图给AI甚至不需要写复杂的提示词)；
    
4.  关于UI设计（一小时入门）：所有大厂的UI都是趋同的（Design Token）： 胶囊按钮（Capsule Button）、圆角矩形（Rounded Corner）、大标题+小标题、下拉菜单 **核心思维：标准化 > 自己创作**
    
5.  写出“完美提示词”？：**对话工程(Dialogue Engineering)**：你的目标不是写咒语，而是**懂得什么是迭代**。像是和同事合作一样，边做边改，分步执行；
    
    ![image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/jacksoncheng-87/images/2026-02-06-1770381559110-image.png)
6.  项目自我感觉太简单: 核心是展示**自己的思考：**
    

-   **设计 ：**为什么要做出这个简单的东西出来？有没有痛点？有没有爽点？
    
-   **交付 ：**这个简单的东西真的上线了吗？有没有人用？
    
-   **ROI ：**既然简单，是不是用极低地成本解决了问题？
    

放在黑客松上面就是：

1) 赛道理解：这条赛道是什么，解决了谁的问题，用户现在用什么替代方案，主要用户分成哪几类（按“商业成本”分，不按功能去堆叠）；
...
关于精准提示词通用模板（了解就好 实战中跟ai慢慢聊）：

-   背景信息（使用场景、目标受众、资源约束）；
    
-   核心需求（功能A（成功标准）、功能B（成功标准））；
    
-   明确约束（技术栈、风格、篇幅、时间）；
    
-   质量标准（将“专业/简洁”转化为可验证指标）；
    
-   排除项：我不要的功能（Negative Prompt）；
    
-   自我验证：要求AI在输出前确定理解；
    

总结：我们不是在“写”提示词，而是在用Reward Signal（奖励信号）和Constraints(约束)来“训练”这次对话。
...
这不仅仅是一个技术周期的更替,而是一次认知层面的变革。AI的影响远超过去的任何技术创新,它改变的不只是编码方式,而是整个行业的运作模式。 代码本身正在变得廉价,以前通过代码质量来评判开发者水平的方式已经不再适用。

**对开发工作流程的影响:**

-   传统流程:手写代码 → 调试 → 测试 → 上线
    
-   AI时代流程:编写提示词 → AI生成代码 → 审查和修改 → 集成到系统 → AI辅助测试
    

**对团队结构的影响:**

AI可能成为初中级程序员的"天花板"。以前需要一两个资深开发带领多个初中级程序员的团队结构,现在可能变成一个资深开发配合AI工具就能完成同样的工作量。

### 关键问题:专业开发者与AI用户的区别

需要深入思考的核心问题:非计算机专业人士使用AI编码,与专业技术开发者使用AI编码,两者之间的本质区别是什么?

**实际案例:链上数据抓取功能**

凯恩老师分享了使用Claude Opus 4.5开发ERC20代币持有者统计功能的经历,展示了AI的多个问题:

-   **区块扫描方向错误**:AI从最新区块往前扫描,而不是从合约创建区块开始
    
-   **缺乏数据持久化**:没有记录已扫描的区块信息,导致每次都需要重新扫描
    
-   **批量处理概念缺失**:不知道RPC节点对每次扫描区块数量有限制
    
-   **更新算法低效**:采用实时更新而非批量更新的方式
    
-   **缺乏优化意识**:不知道使用Multicall等高效方法进行批量查询
    

这个例子说明,即使对于看似简单的功能,没有底层知识和架构思维的支撑,AI生成的代码可能存在多个严重问题。

### 2.开发者必须保持的核心能力

**基础知识和原理理解**

-   编程语言的基础知识必须扎实,才能在AI产生幻觉时及时判断和纠正
    
-   Web3领域需要深入理解密码学、区块链等底层技术,以及不同链的架构差异
    

**业务理解深度**

这是AI的重要短板。技术实现在AI看来可能没问题,但业务逻辑和安全性可能存在严重漏洞。设计之初就需要有深厚的业务认知和经验来避免这些问题。

**顶级源码学习**

不论是否使用AI,都强烈建议学习顶级开源项目的源码,理解其运行原理、实现细节、缺陷和改进方法。这会让你知道什么是好的代码,从而写出更好的代码。 造轮子虽然在工程上不推荐,但对个人能力提升非常重要,能让你清楚了解自己与顶级水平的差距。

**Web3特有优势**

Web3领域协议更新频繁(每周、每月都有新版本),而AI训练数据更新周期是半年到一年。开发者的学习速度可以超越AI,这是一个重要优势。
...
-   **系统复杂性理解不足**:对复杂系统的理解存在问题,可能产生幻觉式的方案
    
-   **知识库更新滞后**:训练数据以半年到一年为周期更新,对新协议支持不足
    
-   **上下文限制**:多轮对话后可能遗忘前面的需求和限制
    
-   **缺乏全局视角**:没有抽象封装概念,可维护性较差
...
自然语言对话生成代码的方式不够专业。计算机是严谨的科学,需要用更专业的方法。

**推荐:专业提示词工程**

类比视频生成领域,专业人士会使用专业摄影术语(镜头角度、视角、光影等)来精确控制生成内容。编码也应该使用技术术语和结构化的提示词:
...
不同的AI工具有各自的标准符号,需要深入学习这些专业用法,而不是简单的对话式编码。
...
AI不能为你的代码负责,最终决策者和责任人是你自己。特别是业务相关的代码,必须经过严格审查。

-   **确保代码可理解性**
    

AI生成的所有代码都应该是你能看懂的、自己也能实现的。如果AI写的代码你读不懂,这是非常危险的,特别是在Web3领域可能造成重大经济损失。

-   **避免过度依赖**
    

长期使用AI可能形成路径依赖,导致不会手写代码、懒得思考、动手能力变差。这在某种程度上是一种退化。 AI应该是提高效率的工具,而不是替代思考的黑盒。

### 扩展:超级个体和创业视角

-   从创业和个人项目角度看,AI降低了技术门槛,缩短了从想法到产品的路径。非程序员或经验不足的程序员也可以快速实现自己的idea,专注于寻找PMF、市场验证、MVP开发等。
    
-   这个逻辑与职场开发完全不同:只要代码能跑起来、没有明显问题就可以,重点是快速验证商业想法。这对有想法的人来说是一个非常好的时代。
    
-   关于vibe coding相关：新人可以通过code review的方式，可以学到自己练手学不到的细节，看别人写的好的code，很建议去配上ai去学，ai会点出很多细节，可以通过能学到很多安全、优化的方式
...
-   DevRel 是一个工程部门，不是单一岗位名称，属于工程师或营销团队
    
-   主要目的是连接产品、工程师和外部开发者，作为三者的中间层
    
-   以生态增长为核心，通过更短的介入路径响应市场需求
    
-   与研发团队独立，研发团队构建产品底层，DevRel 帮助开发者理解和使用产品
    

### 2.传统 Web2 DevRel 的十大职责

-   技术布道师：对外输出内容，阐述产品功能和痛点
    
-   社区经理：在 Discord 等平台与社区沟通，解决问题
    
-   技术写作：负责文档的编写和构建
    
-   开发者体验工程师：开发 SDK 和工具包
    
-   解决方案架构师和生态伙伴工程师：下派到合作公司协助技术架构
    
-   生态增长负责人：将开发者项目转化为面向 VC 的产品
    
-   项目经理：调配任务和人员，协调部门联动
    
-   Developer Marketing：做 SEO、投放和广告
    
-   Hackathon Leader：负责黑客松的 KPI 指标和项目管理
    
-   Data Analyst：分析 KPI 指标，如文档转化率、新项目数量
    
-   Web3 项目中这些角色常常由一人承担多个职责
    

### 3.Web3 DevRel 的四大核心工作

-   **倾听与反馈**：收集开发者意见，如文档 bug、模板问题等
    
-   **赋能与教育**：举办 workshop、宣讲技术内容、构建文档帮助开发者
    
-   **生态与连接**：与合作伙伴举办活动，构建开发者网络，类似 Solana 的 Superteam 计划
    
-   **影响与增长**：通过大型活动吸引开发者，创造新项目投入生态
    

### 5.DevRel 最小技术栈要求

-   **Quick Start**：提供能快速运行的模板，最小时间内展示核心产品功能
    
-   **示例仓库**：包含网络配置、测试 RPC、部署脚本等初始化准备，支持多个技术栈（如 Hardhat、Foundry）
    
-   **FAQ**：整理常见错误和问题，根据用户反馈更新
    
-   **架构与安全**：提供安全的最佳实践，避免直接使用私钥等不安全做法
    
-   **迁移与集成**：编写从竞品迁移的指南，如从以太坊迁移到其他链
    
-   **SDK**：为复杂产品提供 SDK 简化调用
    

### 6.KPI 指标和数据分析

-   关键指标包括：黑客松参与数量、GitHub 新仓库数量、活跃开发者数量
    
-   文档点击次数、使用率、Quick Start 完成率
    
-   通过指标优化开发者体验，反思部门工作是否合理有效
    
-   判断当前方向是否有意义，无效转化则调整方向
    

### 7.开发者社区建设

-   早期需要做基础文档和教育，寻找感兴趣的人和组织
    
-   与大使合作产出技术内容，提供 grant 或孵化产品
    
-   举办活动、更新新闻、传递重要进展
    
-   奖励社区贡献者，整理成名单提交，选拔区域管理人
    

### 8.黑客松和 Workshop

-   **黑客松类型**：一天、7天、一个月等不同时长，有明确赛道要求和产出对比
    
-   **黑客松目的**：吸引圈外 Web2 开发者了解产品，转化为核心成员
    
-   **Hackhouse vs 黑客松**：Hackhouse 无强制赛道要求，大家一起聊天和 build 好玩的项目
    
-   **Workshop**：通过简短视频展示产品功能和使用方法，可在黑客松、直播、Twitter Spaces 等场合使用
    

### 9.必备硬技能

-   **技术理解力**：至少知道 SDK 如何使用，能跑通示例模板
    
-   **Debug 能力**：能现场解决常见错误，处理 SDK 更新导致的问题
    
-   **底层原理理解**：理解共识、执行、存储等底层技术，能回答技术高手的提问
    
-   **写作能力**：把复杂抽象的技术变成简单易懂的内容，"讲人话"
    
-   **逻辑表达**：设身处地考虑新手视角，可使用 AI 模拟新手审视文档
    
-   **PPT 能力**：根据 PPT 展开核心点，包含重要内容
    

### 10.必备软技能

-   **同理心和沟通**：理解用户真正想表达的问题，不纠结用户的错误表述
    
-   精准触达用户核心需求，不以高高在上的态度对话
    
-   遇到问题首先审查自身，如 SDK 是否有问题
    
-   **跨部门协作**：联合多个部门协作执行，不应只用自己部门资源
    
-   **项目管理**：合理调配多个任务，根据优先级排序
    

### 11.Monad 项目介绍

-   从零开始设计但保留以太坊接口的 Layer1 公链，完全 EVM 兼容
    
-   特点：更快出块速度、单槽最终性（BFT 共识）、支持 128KB 合约代码
    
-   拥有 180+ 验证者，与以太坊 RPC 完全兼容
    
-   支持 Hardhat、Foundry、Viem、Wagmi、Ethers.js 等工具
...
1.  突出你跟想投的岗位之间的联系、重合面，技术：要具体到产品功能（即插即用）；
    
2.  好的简历模板（视觉上只应该有黑白）：
    
    ![image](https://hackmd.io/_uploads/BJ_FY3T8Zg.png)
3.  常见雷区：除黑白以外第三种颜色、选择难以让人阅读下去的字体（黑体）、想一套模板投所有岗位；
...
-   项目经验（学生会、社团里单独扛项目、或者自己做项目，有项目管理和推进能力）；
    
-   技术：side project；公共项目的参与维护；积极的github提交记录；对口的实习经历；
    
-   对资金、热点、情绪的感知能力（比如什么火就快速开发了一个side project获得好评；
...
1.  web3 面试的大致流程：比互联网短很多，短的可能就是founder一个电话，长的也不过3-4轮，tip：大公司需要口语英语好，也非常看重跨语言跨时区交流能力；
    
2.  面试准备该怎么做：试着找找你要面的公司现在有什么问题，以及是你的话会如何解决solid projects results；团队面和HR面的区别：团队更看重你是否即插即用，HR看重一种能否融入团队的综合素质，比如自驱力、学习力、快速适应能力等等；有没有人给你背书非常重要，如果你是团队成员引荐的人基本保送；
    

三、一些建议:

1.  新人如何选择第一份工作:视野起点: VC > 交易所 > 媒体 > 其他任何
...
2.技术成长：安全 > layer1/2 > 交易所 > 节点商/矿池 > 其他任何;

-   谨慎去小社区 = 什么都干 + 可能拿不到工资（看老板)
...
-   你是什么国家 什么城市 什么学校 ➡️ 能收到什么样的 offer ➡️ 再考虑要不要来币圈;
    
-   城市 > 公司(更进一步，所在小组) > 行业 > 具体做什么;
    
-   就目前而言的币圈的公司更适合你收不到其他行业好offer的情况下再考虑;
...
-   非技术（运营/商务）：跟互联网/游戏/电商一致
    
-   技术：多一个智能合约语言 solidity / rust ，其他也基本一致
    

4.以“实习”形式进入币圈，和自己野生进入币圈的区别:
...
-   精而专：项目方（layer1、infra)
    
-   跟web2差不多: CEX
    

6.好处:远程的工作多 而且大多对于kpi要求不高:有概率直接发财这辈子都不用工作

7.坏处:加密经历其他地方不认（但是年轻可以弥补这条):可能什么也学不到，极少有面相应届生的稳定培养计划（如管培生（因为行业不稳定;链上收入是灰色的；社会身份也很灰色;没有秋招（因为行业不稳定)
...
**ERC-20 (同质化代币)** ：以太坊上互换性资产（如货币、积分）的基础标准，通过余额账本管理代币，但存在必须先授权（Approve）再调用合约（TransferFrom）的繁琐两步操作流程。

**ERC-721 (非同质化代币 NFT)** ：针对独一无二资产（如艺术品、域名）的标准，核心是以唯一的 tokenid 区分每个资产，并支持通过tokenURI 链接到外部的元数据（图片、属性）。

**ERC-1155 (多代币标准)** ：一种高效的混合标准，允许在一个合约中同时管理多种同质化和非同质化代币，且支持通过批量转账功能极大降低向多人发送多种资产时的 Gas 费用。

**ERC-1046 (扩展元数据)** ：为了让 ERC-20 代币也能拥有类似 NFT 的丰富媒体展示能力，允许为代币添加图标、描述等元数据链接，改善钱包前端的展示体验。

* * *

### 2\. 功能增强与安全 (开发规范)

**ERC-165 (接口检测)** ：智能合约的“自我声明”标准，允许合约向外界证明自己实现了哪些特定的接口，是其他标准（如 ERC-721）进行互操作性检查的基础依赖。

**ERC-6093 (标准错误代码)** ：现代 Solidity 开发的错误处理规范，它用自定义错误类型（如`InsufficientBalance`）取代了传统的字符串报错（Revert String），既节省了部署和运行 Gas，又方便开发者精准定位错误原因。

**ERC-2612 (Permit 签名授权)** ： ERC-20 的扩展，允许用户通过链下签名的方式进行授权，从而无需持有 ETH 支付 Gas，可由第三方中继者代付 Gas 完成交易，极大地优化了用户体验。

**ERC-1363 (可支付代币)** ： ERC-20 的扩展，实现了“转账即调用”的功能，允许在转账代币的同时自动触发接收方合约的逻辑钩子，适用于订阅支付或一键购买场景。

**ERC-223 (防误转标准)** ：为了解决 ERC-20 代币误转入合约导致丢失的问题，它强制在转账时调用接收方的回调函数，如果接收方不支持该回调则交易失败，从而保护资产安全。

* * *

### 3\. DeFi 核心设施 (金融逻辑)

**ERC-4626 (代币化金库)** ：DeFi 收益聚合器的通用接口标准，统一了资产存入、份额铸造、资产提取和份额赎回的逻辑，解决了不同理财协议接口不一致导致的集成难题。

**ERC-3525 (半同质化代币 SFT)** ：一种带有层级结构的代币标准，引入了“插槽（Slot）”概念，拥有相同插槽属性的代币（即使 ID 不同）可以像 ERC-20 一样进行数学合并或拆分，非常适合表达票据、债券或复杂的金融合同。

**ERC-3009 (签名转账)** ：一种比 ERC-2612 更灵活的签名转账标准，常用于 USDC 等稳定币，允许用户通过签名授权第三方进行转账操作，支持在无 Gas 情况下进行资金结算。

**ERC-1271 (合约签名验证)** ：为了解决智能合约没有私钥无法签名的问题，它规定了一种验证逻辑，让多签钱包或智能钱包可以像普通用户一样对消息进行有效签名验证，是智能钱包登录 DApp 的关键。

* * *

### 4\. 账户与 NFT 进阶 (交互体验)

**ERC-4337 (账户抽象 AA)** ：无需修改以太坊底层协议即可实现智能合约钱包的标准，引入了 UserOperation 伪交易对象和打包器网络，实现了社交恢复、Gas 代付和批量交易等高级功能。

**ERC-6551 (Token Bound Accounts)** ：一种为每一个 NFT 创建专属智能合约账户的标准，使得 NFT 本身可以像用户一样拥有以太坊地址、持有资产（代币或其他 NFT）并与应用交互。

**ERC-4907 (租赁 NFT)** ：一种将 NFT 的所有权和使用权进行分离的标准，通过增加“用户（User）”角色和有效期设置，允许拥有者在不转移所有权的情况下将 NFT 租借给他人使用。

**ERC-2981 (NFT 版税)** ：一种链上版税信息的查询标准，允许 NFT 创建者声明无论资产在哪里出售，应获得多少比例的版税，旨在促进跨交易市场的版税支付互通性。

* * *

### 5\. RWA 与合规 (现实资产)

**ERC-3643 (合规代币)** ： RWA（现实世界资产）领域的主流标准，采用许可制代币逻辑，强制在每次转账前查询链上身份注册表，确保只有通过 KYC/AML 验证的合规用户才能持有或交易代币。

**ERC-1400 (证券型代币)** ：一套专为证券发行设计的模块化标准库，支持部分可互换性、强制转移（法律执行）、分红发放以及复杂的文档管理，满足证券法的监管需求。

* * *

### 6\. AI + Web3 (前沿探索)

**ERC-7007 (AIGC 验证)** ：一种结合零知识证明（zkML）的标准，用于在链上验证 AI 生成内容的真实性，确保 NFT 确实是由特定的 AI 模型在特定的输入下生成的。

**ERC-7575 (推理证明)** ：一种用于验证 AI 模型推理过程的标准，确保链下 AI 模型的计算结果没有被中间人篡改，为去中心化 AI 计算网络提供信任基础
...
Octant的资助逻辑建立在“收益即公共项目”的理念之上，通过循环的“纪元（Epoch）”运作。

资金来源：Golem Foundation 质押了100,000 ETH，其产生的质押收益是 Octant 资助池的主要来源。

纪元周期：每90天为一个纪元。在每个纪元结束时，系统根据用户参与度分配奖励。

奖励分配结构（Epoch 3后版本）：

70% 质押收益：分配给总奖励预算，其中一半用于个人奖励（User Rewards），另一半用于配捐奖励（Matched Rewards）。

25% 质押收益：分配给 Golem 基金会，用于运营、开发和市场推广。

5%质押收益：注入Octant社区基金，由社区治理委员会管理。

配捐机制：如果用户选择放弃个人奖金将其捐赠形成给选定的公共物品项目，Octant将利用匹配奖励池对捐赠进行放大，杠杆效应。

2\. 用户体验与参与流程

Octant的设计强调低基础设施和非托管资产，确保用户在支持公共物品的同时保留资产密钥。

GLM锁仓：用户需在Octant App中锁定至少100 GLM（Golem原创代币）才能获得个人奖励资格。

非托管参与：锁仓操作是非托管的，用户随时可以解锁并提走GLM。

时间加权算法：奖励计算采用时间加权任务。锁定的GLM越多、时间越长，获得的奖励和投票权重增益。

决策窗口：纪元结束后有周末的“决策窗口”，用户选择：

领取奖励：将 ETH 奖励捐赠至个人钱包。

捐赠奖励：将全部或部分奖励捐赠项目列表中的公共项目项目。

3.安全治理与生态影响

Octant正在从中心化决策向更广泛的社区治理演进。

治理委员会：目前由6人组成的临时治理委员会监督Octant社区基金运作，负责项目筛选和营销策略。

项目筛选：由 Golem Foundation 根据社区反馈设定资格标准，资助对象涵盖Protocol Guild、Gitcoin 和 Tor Project 等知名项目。

V2演进：Octant V2推出了Dragon Vaults等机构级工具，允许其他组织创建自己的“收益捐赠池”，将资助图案化和基础设施化。

4\. 研究结论

Octant成功展示了如何通过存量资产项目的利息（收益率）除本金（本金）来资助公共。这种“永续资助引擎”有效解决了Web3项目长期依赖一次性补助款的困境，同时也为GLM代币赋予了明确的治理效果。
...
链上数据基础类型： 1.交易数据；2.区块数据；3.合约数据；

基础知识：

-   **链上交易**是区块链状态改变的主要方式，每笔交易的基本生命周期包括创建、广播、打包、确认；
    
-   **区块**是区块链的基本组成单位，是纪录交易和数据的容器\*\*。\*\*在区块链中，一个区块就是一个数据包，包括一组经过验证的交易以及其他与区块链状态相关的信息。区块存储数据并链接到链上的前一个区块，形成连续、不可纂改的链条，保障数据完整性；
    

1.**交易数据**：

交易哈希 ；交易状态 ；区块 ；时间戳 ；收发双方地址 ；该笔交易转账的ETH数量 ；交易费 ；Gas费 ；

Nonce：交易发起者账号的交易计数器，用于防止重复交易

2.**区块数据**：

区块高度 ；区块哈希 ；时间戳 ；验证者/矿工地址 ；区块中的交易类型 ；Gas费

3.**智能合约数据**

合约地址 ；合约定义逻辑（函数）；合约定义事件 ；合约定义变量

## 02：常见数据指标

区分与传统的考查标准：

-   总锁仓量（TVL）：表示在某一协议或整个区块链生态系统中被锁定的加密资产的总价值，包括流动性地、抵押贷款和收益聚合中的总资产价值；
    
-   活跃地址数（Address）：指在一定时间段内至少执行过一次交易的地址数量。与Web2中的活跃用户（Active User）类似，链上的“活跃地址”表示的是有实际行为发生的用户地址；
    
-   交易数（Count）及交易量（Volume）：前者是在一段时间内链上发生的所有交易的数量，后者是指这些交易所涉及资产价值的总额；
    
-   交易费（Transaction Fee）/Gas 费用：是指用户在区块链网络中交易时支付给矿工或验证者用打包上链的费用，是用户交易的成本，通常又叫Gas费用
    
-   协议收入（Revenue）：
    

## 03:数据分析工作流：提取(extract)——>转换(transform)——>加载（load）

-   数据ETL：和Web2数据分析类似，一个完整的链上数据分析流程需要经过以下几个步骤： 数据抽取(Extract)→清洗转换(Transform) →加载(Load)到数据仓库→ 依据不 同业务线及观测指标分析数据→ 输出量化结果；
    
-   Web3数据产品生态：基于链上数据分析的ETL流程，区块链数据产品可以分为数据源(Data Source)、 数据开发工具(Data Dev) 和数据应用(Data App) 三类；
