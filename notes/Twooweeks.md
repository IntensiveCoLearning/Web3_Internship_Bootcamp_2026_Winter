...
-   BEACON 信标：管理验证者：它是质押系统的账本。它记录了谁质押了32 ETH，谁在偷懒（离线），谁在作恶（被罚没资产），以及谁有资格参加接下来的出块抽签；分配任务（随机性）： 它通过产生随机数，公平地决定哪个验证者在接下来的12秒内负责打包区块，以及哪些人组成委员会进行审查；维持共识： 它不处理具体的智能合约逻辑（那是执行层干的），它只负责同步各方的投票信号，确保全球成千上万个节点对哪一个区块是正确的达成统一意见
