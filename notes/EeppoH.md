...
静态类型，变量的类型在编译时就确定，不能随便换。

例子： uint256 age=18

age 永远是整数，不能变成字符串、地址等

为什么Solidity要静态类型。因为合约一旦上链不可修改，钱是真钱且必须在部署前就发现错误。
...
**RPC 不是区块链本身，也不是智能合约写的，通常是由某个节点运营方“控制”的。**

开发者**一般不写 RPC 协议本身**，而是**选择 / 配置 / 切换 RPC 提供者**。

RPC协议是区块链标准，有点像银行清算规则，RPC是web3中最容易被中心化的一层。 大量 DApp 用同一家 RPC，RPC 能：限流、记录请求、暂停服务

所以成熟 DApp 会：多 RPC 备份、允许用户自定义 RPC
...
**DApp 并不运行区块链节点，而是通过用户的钱包或自身的后端服务，借助 RPC 接口与区块链网络交互。**
