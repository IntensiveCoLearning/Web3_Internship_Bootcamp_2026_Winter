...
实习计划的最后一天了，发现我的底层知识还不够扎实，最后又又回顾了一下solidity基础，在cyfrin updraft重新开始过一遍基础语法，购入一本《精通以太坊》，准备重新把底层知识打牢。关于未来，可能会去学Rust或者Typescript，再补一点ai agent的系统知识。虽然我是主张干中学，但是总是会对基础不够扎实而带来自我怀疑，所以每次干一点项目，又会回去看基础知识，无法真正抉择自己的方向，可能这就是我的道路。祝大家越来越好，江湖再见👋
...
完成demo演示：[https://www.youtube.com/watch?v=k8vhekvICTI](https://www.youtube.com/watch?v=k8vhekvICTI)
...
Uniswap V2 是一个去中心化交易协议，建立在以太坊区块链上。它是 Uniswap V1 的重大升级版本，于 2020 年 5 月发布。
...
-   当用户用代币 A 兑换代币 B 时，代币 A 的储备增加，代币 B 的储备减少
    
-   乘积 k 保持不变（扣除手续费后）
    
-   这种机制自动调节价格：供应减少的代币价格上涨
...
用户用 1 ETH 兑换 USDC：
- 新 ETH 储备：11 ETH
- 新 USDC 储备：200,000 / 11 ≈ 18,182 USDC
- 用户获得：20,000 - 18,182 = 1,818 USDC（未计手续费）
```
...
Uniswap V2 采用工厂模式，主要包含三类合约：

### 3.1 UniswapV2Factory
...
-   `price0CumulativeLast` / `price1CumulativeLast` - 价格累积器
    
-   `kLast` - 最后的 k 值（用于协议手续费）
    

### 3.3 UniswapV2Router02
...
-   `MINIMUM_LIQUIDITY` (1000 wei) 被永久锁定，防止除零错误
...
### 4.3 无常损失 (Impermanent Loss)

当池子中代币的相对价格变化时，流动性提供者会遭受无常损失。
...
Uniswap V2 允许用户先接收代币，然后在同一交易中归还或支付：

1.  调用 `swap()` 并在 `data` 参数中传入回调数据
...
| 特性 | V1 | V2 |
| --- | --- | --- |
| 交易对 | ETH 为桥接 | ERC20/ERC20 直接交易 |
| 闪电兑换 | ❌ | ✅ |
| 价格预言机 | ❌ | ✅ TWAP |
| 协议手续费 | ❌ | ✅ 可选 |
| ERC20 实现 | 自定义 | 标准 ERC20 |
...
后续添加：
L = min(Δx * L_total / x, Δy * L_total / y)
...
### 12.1 基本交易（使用 ethers.js）

```javascript
const routerAddress = "0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D";
const routerABI = [...]; // Router02 ABI
...
✅ **简单高效** - 恒定乘积公式易于理解和实现  
✅ **去中心化** - 无需许可，任何人都可创建交易对  
✅ **抗审查** - 完全链上运行，无法被关闭  
✅ **可组合性** - 作为 DeFi 乐高积木，可与其他协议集成  
✅ **透明性** - 所有数据公开可查
...
-   **官方文档：** [https://docs.uniswap.org/protocol/V2/introduction](https://docs.uniswap.org/protocol/V2/introduction)
    
-   **白皮书：** [https://uniswap.org/whitepaper.pdf](https://uniswap.org/whitepaper.pdf)
    
-   **GitHub：** [https://github.com/Uniswap/v2-core](https://github.com/Uniswap/v2-core)
    
-   **审计报告：** 多家安全公司审计
    
-   **社区：** Discord, Twitter, 论坛
...
-   深入学习 Uniswap V3（集中流动性）
    
-   了解其他 AMM 协议（Curve, Balancer）
