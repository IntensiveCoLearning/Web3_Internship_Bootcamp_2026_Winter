...
DeFi 合约常调用外部合约（预言机、代币、其他协议），外部调用的 Gas 优化核心是「减少调用次数」和「选择高效调用方式」。

1\. 减少外部合约调用次数

外部调用的 Gas 消耗远高于内部计算，优先缓存外部合约的结果（如预言机价格），避免重复调用。
...
contract OracleCache {
    ...
    mapping(address => uint256) public lastPrice; // 缓存价格
    mapping(address => uint256) public lastPriceUpdate; // 缓存时间
    uint256 public constant PRICE_EXPIRY = 5 minutes; // 价格有效期5分钟

    // 获取价格：优先用缓存，仅过期时调用预言机
    function getAssetPrice(address asset) public returns (uint256) {
        uint256 updateTime = lastPriceUpdate[asset];
        if (block.timestamp - updateTime < PRICE_EXPIRY) {
            return lastPrice[asset]; // 用缓存，避免外部调用
        }
...
EVM 中每个存储槽（storage slot）占 32 字节，Solidity 会自动打包小类型变量到同一个槽，但需手动优化变量顺序（按字节数从大到小排列），避免浪费。

```
// 优化顺序：大类型在前，小类型打包到同一槽
// slot0: uint256 balance (32字节)
// slot1: bool isActive (1字节) + uint8 fee (1字节) + 空（30字节）
// 总计2个存储槽（节省1个槽的存储/读取成本）
uint256 public balance;
bool public isActive;
uint8 public fee;
```

2\. 优先使用 `mapping` 而非数组（按需读取）

DeFi 中常见的「用户地址 => 数据」场景，`mapping` 比动态数组更高效：

-   数组：读取用户数据需遍历（O (n)），Gas 随用户数增长；
    

`mapping`：直接通过键读取（O (1)），Gas 固定。**优化示例**（mapping 存储）：
...
状态变量（storage）的读写是 Gas 消耗大户（写入约 20000 Gas / 次，读取约 2100 Gas / 次），优先将重复读取的状态变量缓存到内存。

```
function withdraw(uint256 amount) public {
    uint256 userBalance = balances[msg.sender]; // 仅读取1次storage
    if (userBalance < amount) revert InsufficientBalance(msg.sender, userBalance, amount);
    
    balances[msg.sender] = userBalance - amount; // 仅写入1次storage
    payable(msg.sender).transfer(userBalance - amount); // 用内存变量
}
```
...
合理使用 `unchecked` 块（关闭溢出检查）

```
pragma solidity ^0.8.20;

contract GasOptimizationUnchecked {
    uint256[] public numbers;

    // 循环中使用 unchecked
    function sumNumbers() public view returns (uint256 total) {
        uint256 length = numbers.length; // 缓存长度（减少storage读取）
        for (uint256 i = 0; i < length; ) { // 去掉i++，移到unchecked内
            total += numbers[i];
            // 确定i不会溢出（length最大为2^256-1，i < length），用unchecked
            unchecked {
                i++;
            }
        }
...
contract GasOptimizationErrors {
    // 定义自定义错误（无参数/带参数）
    error ZeroAmount();
...
DeFi 合约常调用外部合约（如预言机、其他协议），需用 low-level call 捕获异常，避免连锁回滚：
...
contract DeFiExternalCall {
    ...
    // 从预言机获取价格：捕获异常 + 校验价格合理性
    function getAssetPrice(address asset) public returns (uint256 price) {
        // 用low-level call调用预言机，避免异常回滚
        (bool success, bytes memory data) = priceOracle.call(
            abi.encodeWithSignature("getPrice(address)", asset)
        );

        // 捕获调用失败异常
        if (!success || data.length == 0) {
            revert OracleCallFailed(priceOracle);
        }
