...
HD Wallet 的中文全称是 **分层确定性钱包** (Hierarchical Deterministic Wallet)。

助记词经一定流程（例如先变为一个种子数 seed，然后使用 HD 钱包标准

如 BIP-32，BIP-44 等），从这个种子派生（derive）出一个或多个私钥（Private

Key）。

HD 钱包（Hierarchical Deterministic Wallet）允许你用同一个助记词生成

多个私钥，每个私钥控制一个或多个区块链地址 /账户。
