...
-   **及时**：岗位 HC 仍然 open（不是转发几手的过期信息）
    
-   **真实**：岗位真实存在，不是“引流假岗”
    
-   **准确**：JD、薪资、地点、KPI、面试流程与实际一致
    

👉 你之后看到任何 Web3 招聘信息，都可以用这三条先做第一轮过滤。
...
4.  [**web3career.build**](http://web3career.build)
...
## 三、隐藏但非常重要的信息渠道（内推 & 早期机会）

很多 **好岗位不会上招聘网站**：

-   **LXDAO / ETHPanda 社区**（Telegram）
    
    -   Bounty、实习、内推、真实项目讨论
        
-   **项目官方 Discord / Telegram**
...
-   是否可在 LinkedIn / RootData / CoinGlass 查到
...
-   社区经理 / 市场 / BD / 产品运营 / 内容负责人
    

👉 本质能力：**沟通 + 组织 + 交付 + 对 Web3 文化的理解**
...
-   Hackathon（强烈推荐 LXDAO）
    
-   自己做项目（哪怕很小）
    
-   系统学 Solidity / Rust / 区块链原理
...
-   **技术栈**：C/C++（主业），Android / iOS / HarmonyOS
    
-   **关键经历**：
    
    -   研二自建以太坊节点
        
    -   Web2 大厂 + 区块链项目（国内 FT 项目 Top2）
        
    -   面试过国内 Top2 交易所专家岗
        
-   **当前规划**：逐步转向 Web3，计划赴 **香港** 发展
...
## 二、进入 Web3 的契机（Why Web3）

-   海外（新加坡）Web3 氛围更成熟 → 认知冲击
    
-   与同学结伴做链上研究 → 从“感兴趣”到“系统探索”
    
-   大厂区块链项目经历 → **验证赛道可行性**
    
-   不是“All in”，而是 **判断趋势后理性跨界**
    

👉 **Takeaway**：Web3 转型通常来自 _环境 + 项目 + 验证_ 的叠加，而不是一时热情

* * *

## 三、Web3 岗位信息从哪里来（How to get jobs）
...
> Web3 依然跑在 Web2 的硬件和系统之上

* * *

## 六、Web2 → Web3 的最大挑战（分角色）
...
-   **经历路径**：  
    校园黑客松 → LXDAO（运营 / PM）→ 前端开发 → Bybit
    
-   **关键词**：前端 × 业务理解 × 社区 × Web3 实战
...
**“小项目 → 社区 → 开源 → Hackathon → 职业岗位”**

-   从 **低风险、小闭环** 开始，而不是一上来冲大型 Hackathon
    
-   每一步都能形成真实产出（代码 / PR / 项目 / 人脉）
...
他们强调：Solidity 代码本身常常很“无聊”（就加减计数器），真正复杂的是它运行在链上、状态存在所有节点上、写操作要付 gas。
...
-   ⭐ 优点：合约一改，前端 UI 自动更新；最适合“点按钮试功能”的 tinkering。
...
-   ⭐ 优点：直接打开就能写/编译/部署/交互（适合快速试语法）。
    
-   缺点：功能/工程化有限；界面复杂，容易“迷路”。
    

### C. Hardhat（本节主讲）

-   ⭐ 优点：更底层、更工程化；适合写脚本/写测试；最终上生产更稳。
    
-   缺点：交互不直观，tinkering 主要靠 tests/scripts。
    

> 额外提到：Foundry（Rust 写的，更快）——偏命令行、偏硬核。

* * *

## 3) Solidity 合约结构最小集（Hello World 到 Counter）

### 合约的基本构成

-   `pragma solidity ^0.8.x;`：指定编译器版本范围  
    ⭐ 0.8+ 自动带 overflow/underflow 检查（以前要 SafeMath）
    
-   `contract X { ... }`：合约像“类”，部署后有自己的地址
    

### 状态变量（storage）

-   写在合约里、函数外的变量：**会存链上（storage）**，成本高。
    
-   变量默认值：`uint` 默认是 0，`address` 默认 0x0，`bool` 默认 false。
...
-   ⭐ **读**：`view`（读状态但不改）→ “免费/很便宜”（本质是读取，不需要上链）
    
-   ⭐ **纯计算**：`pure`（不读也不改状态）→ 完全确定性计算
    
-   ⭐ **写**：改变状态变量 → 是 transaction → 要挖矿/要 gas
...
-   `increment() public` 会改 state → 交易
    

* * *

## 5) 可见性（visibility）与一个重要误区

可见性：`public / private / internal / external`

-   ⭐ **误区纠正**：`private` 不等于“链上保密”。链上数据都可被读取。  
    `private` 只是限制“其他合约/继承访问”，不是隐藏数据。
    

* * *

## 6) Globals（全局变量）你最常用的三个

他们说“用得最多”的是：

-   ⭐ `msg.sender`：调用者地址
    
-   ⭐ `msg.value`：随交易带来的 ETH 数量（wei）
    
-   ⭐ `block.timestamp`：区块时间戳（注意可被矿工/验证者轻微影响）
    

还提到 `tx.origin`（⚠️ 不建议乱用，合约调用链里很坑）。

* * *

## 7) Require：Solidity 最关键的“规则锁”

-   ⭐ `require(condition, "error msg");`  
    不满足就 **revert（回滚）**：这一笔交易里所有状态修改都撤销（原子性）。
    

常见用途：权限、余额、供应量上限、价格校验等。

* * *

## 8) Modifiers：把重复 require 抽出来

当很多函数都要“only owner/boss”：
...
-   好处：减少重复代码、权限一眼可读、测试更集中。
    

* * *

## 9) Mapping：做 Token 的核心数据结构

-   ⭐ `mapping(address => uint256) public balances;`
    
-   关键限制：**mapping 不能遍历**（不能 for-loop 枚举所有 key）  
    所以像 `totalCreated` 这种统计量常常要额外用变量存着。
    

* * *

## 10) 手搓“简化版 ERC20”思路（本节的主线 demo）

他们从零做了一个“Sanford Token”：
...
-   ⭐ `require(totalCreated + qty <= totalSupply, "supply reached");`
...
-   后来补上：`create` 应该只有 boss 才能调用（`onlyBoss`）
    

### ③ send / transfer（转账）

-   ⭐ 显式 require：`balances[msg.sender] >= qty`
...
### ④ Hardhat tests（用测试“poke 合约”）

他们一直强调：Hardhat 里要靠 tests/scripts 才能交互。  
测试覆盖了：

-   create 能增加余额
    
-   超过 totalSupply 会 revert
    
-   transfer 余额变化正确
    
-   非 boss 调 create 会 revert
    

* * *

## 11) payable：收 ETH 的两种场景

### A. payable function（调用函数时带钱）

```
function buy() public payable { ... }
```

-   ⭐ 如果你对 non-payable 函数发送 value → 直接 revert
    
-   用 `require(msg.value == price, "wrong price");`
    

测试里用：

-   `token.connect(signer1).buy({ value: ethers.parseEther("0.01") })`（具体版本略有差异）
    

### B. receive()：直接给合约地址转账（不调用函数）

```
receive() external payable {}
```

-   ⭐ 没有这个，别人直接向合约地址转 ETH 会失败（revert）
    

* * *

## 12) 合约里“发送 ETH” 的 gotcha

他们强调：别用 `transfer` / `send`（gas stipend 等历史问题）。  
更稳的写法是：
...
并顺带提了：钱先进入合约地址，不会自动到 boss 手里，所以要写 withdraw。

* * *

## 13) Events：给前端看的“链上日志”

-   ⭐ event 是给前端/链下索引用的，**其他合约读不到**
    
-   用途：记录 buy/transfer 历史，让 UI 能显示“交易流水”。
...
`indexed` 让前端更容易按地址过滤事件。

* * *

## 14) 最实用的“你接下来怎么学”（他们给的路线）

1.  去 Solidity by Example 挑主题（primitives / mappings / structs / arrays / attacks）
    
2.  复制到 Remix/Scaffold/Hardhat
    
3.  编译部署 → poke
    
4.  ⭐ 写 tests（尤其是权限、边界条件、revert）
...
-   **一句话**：Scaffold-ETH 是一个“本地链 + 合约开发 + 前端自动生成/适配”的全套 dApp 模板
    ...
    -   组合栈：**Hardhat（本地链/编译部署） + Ethers.js（交互） + React（前端）**
        
-   **最强体验点**：
    
    -   你在合约里**增删变量/函数** → 保存 & 部署 → **前端界面自动出现/消失对应的字段和按钮**
        
    -   用它可以**边改 Solidity 边在 UI 里点按钮测试**，非常适合学习与快速迭代
...
1.  [**ethereum.org**](http://ethereum.org)：了解以太坊、钱包、ETH、DeFi、DAO、NFT、PoS 等整体概念
    
2.  **Remix**：浏览器内写合约，快速上手
    
3.  [**eth.build**](http://eth.build)：用互动小工具学“底层基础”
    
    -   key pair（私钥/公钥/地址）
        
    -   签名与恢复（sign / recover）
        
    -   hash、加密、分布式账本、拜占庭将军、PoW 等概念演示
        
4.  **Scaffold-ETH**：一旦你掌握基础，就进入本地全栈开发（合约 + 前端 + 部署）
    

* * *

## 3) 他用 Scaffold-ETH 演示了哪些 Solidity 核心点

### 3.1 基础数据类型（primitives & types）
...
-   重点：你把一个 `address` 写进合约，部署后，前端会多出一个可读字段（展示它）
    

### 3.2 访问控制：owner + require + msg.sender
...
-   测试方法：
    ...
    -   用 **incognito/新窗口**，模拟另一个地址调用，验证会失败（对抗式思维）
        

> takeaway：Solidity 合约本质就是“状态 + 规则（require）”。

### 3.3 把“单人合约”改成“多人可用”：payable + msg.value

-   移除 owner 限制，让任何人都能调用，但要付费：
    
    -   `payable`
        
    -   `require(msg.value >= 0.001 ether, "not enough");`
        
-   前端变化：出现一个“交易金额 value”的输入框
    

### 3.4 单位与精度：wei 与 10^18

-   EVM 不做浮点（没有小数），所以 ETH 金额用整数表示：
    
    -   `1 ETH = 10^18 wei`
        
-   你看到 UI 里 `0.001 ether` 会对应到一个很大的整数 wei 值
    

### 3.5 机制设计小例子：价格随调用次数上升（bonding curve 的影子）
...
-   每次成功调用后让价格变贵一点：
    ...
    -   `price = price * 1001 / 1000;`（约每次 +0.1%）
        
-   结果：前端价格不断上涨，合约余额不断累积
    

> takeaway：区块链“可编程”的强大之处在于，你只要加几行规则，就能改变整个经济机制。

* * *

## 4) “区块链长什么样”——他用 Web2 类比解释以太坊
...
-   类比 Web2：
    ...
    -   像一个**数据库**，但“冗余”是极端的：几乎每个节点都有一份状态 → 抗审查、抗宕机
        
-   关键特性：
    
    -   **censorship resistance**（抗审查）
        
    -   **trustless**（你信的是代码规则，而不是某个中心化机构）
        

* * *

## 5) 继续学 Solidity：mapping / transfer / token（从例子理解概念）

### 5.1 mapping：地址 → 数值（余额表）

-   `mapping(address => uint) public balance;`
    
-   本质：像 `balances[address]` 这样的哈希表
    

### 5.2 自己写一个最简 token transfer（理解 ERC20 的核心）
...
-   他提醒：示例里数值是 wei 级别的“整数”，别误以为是 ETH
    

> takeaway：你是在用最小实现理解“token 为什么能转账”，再去看 ERC20 标准会更顺。

* * *

## 6) 重要概念提醒：events / modifiers / inheritance / OpenZeppelin

-   **Modifiers**：把重复的 require 规则抽出来复用（比如 `onlyOwner`）
    
-   **Events**
    
    -   合约发事件给前端用（日志）
        
    -   事件不适合被其他合约当作链上逻辑依赖（“给人看/给前端用”，不是“给合约计算用”）
        
-   **Inheritance + OpenZeppelin**
    
    -   不要手写 Ownable/标准 token，直接继承 OpenZeppelin 的成熟实现
        
    -   `Ownable` + `onlyOwner` 是经典模式
...
-   在 Hardhat 配置里选择网络（testnet / mainnet）
...
-   前端里把 network 配置改成对应网络
    
-   `yarn build` 后部署静态站点（他提到 surge/s3 等思路）
    
-   最终：给别人一个 URL，就能用钱包访问你的 dApp
    

* * *

## 8) 他推荐的“学习任务线”（Ethereum Speedrun）

大意是：学完语言和基础概念后，用一系列挑战把关键坑都踩一遍：

-   NFT mint（并做小改动）
    
-   staking challenge（时间、payable、状态）
    
-   token vendor（重点：ERC20 `approve` 是两步交易）
    
-   commit-reveal 随机数、oracle、喂价
    
-   自己做 DEX、bonding curve、lending、leverage
    
-   签名、meta-tx、多签（final boss：signature-based multisig）
    
-   最后：读安全案例（reentrancy 等），看审计反馈
