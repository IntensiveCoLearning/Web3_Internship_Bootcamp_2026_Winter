...
## **什么是 EOA（Externally Owned Account）**
...
EOA 的一切行为：转账 ETH调用合约创建合约

都必须通过 **私钥签名交易**

总的来说，EOA（Externally Owned Account）是以太坊中由私钥直接控制的外部账户，本身不包含任何合约代码，能够作为交易的唯一主动发起者，通过对交易进行密码学签名来转移资产或调用合约，但由于其控制权完全依赖单一私钥、缺乏权限管理与恢复机制，实际协议设计中通常将其作为入口账户，并通过智能合约、多签钱包或账户抽象机制来增强安全性与可控性。
