...
| 特性 | Storage (状态变量) | Events (事件日志) | 嵌入式类比 |
| 存储位置 | State Trie (世界状态) | Transaction Receipt (收据) | RAM/Flash vs 串口打印流 |
| 成本 (Gas) | 极贵 (20,000 Gas) | 便宜 (375 Gas + 8 Gas/字节) | 写 Flash vs 发 UART |
| 可见性 | 合约内部可读 | 合约内部无法读取！ | 代码能读变量，但代码读不了自己的 printf 输出 |
| 用途 | 存储逻辑状态 | 通知外部世界 (前端/后端) | 变量存储 vs 调试日志/中断通知 |
...
3.  向下滚动，找到 **"Logs"** 字段。
    ...
    -   您会看到 `DataUpdated` 和 `HighTempAlert` 的详细内容。
...
-   **没有 indexed:** 如果您想找“ID 为 101 的所有历史记录”，前端脚本必须把 10,000 条日志全部下载下来，一条条遍历解析。效率极低。
    
-   **有 indexed:** 这里的 `deviceId` 被加入到了 **Bloom Filter (布隆过滤器)** 中。节点可以瞬间告诉您：“只给我 ID=101 的日志”，其他 9999 条直接忽略。
