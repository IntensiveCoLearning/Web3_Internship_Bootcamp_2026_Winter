---
timezone: UTC+8
---

# songke2025

**GitHub ID:** songke2025

**Telegram:** 

## Self-introduction

Web3 实习计划 2025 冬季实习生

## Notes

<!-- Content_START -->
# 2026-01-29
<!-- DAILY_CHECKIN_2026-01-29_START -->
### 学习内容：

1.  Web3 实习手册[「行业前辈访谈录」](https://web3intern.xyz/zh/logic-frontend-journey/)部分
    
2.  Web3 实习手册[「面试准备与行业岗位推荐」](https://web3intern.xyz/zh/job-platform/)部分
<!-- DAILY_CHECKIN_2026-01-29_END -->

# 2026-01-28
<!-- DAILY_CHECKIN_2026-01-28_START -->

### 继续看回放
<!-- DAILY_CHECKIN_2026-01-28_END -->

# 2026-01-26
<!-- DAILY_CHECKIN_2026-01-26_START -->


### 最近没跟上课程，在慢慢看直播回放
<!-- DAILY_CHECKIN_2026-01-26_END -->

# 2026-01-25
<!-- DAILY_CHECKIN_2026-01-25_START -->



### **继续查看没看直播的回放。**
<!-- DAILY_CHECKIN_2026-01-25_END -->

# 2026-01-24
<!-- DAILY_CHECKIN_2026-01-24_START -->




### 一：复习回顾相关章节，完成未完成课程

### 二：查看直播的回放。
<!-- DAILY_CHECKIN_2026-01-24_END -->

# 2026-01-22
<!-- DAILY_CHECKIN_2026-01-22_START -->





### 一：收看「Web3 实习计划（冬季）第一周例会」回放。

### 二：参与项目TGE空头活动。
<!-- DAILY_CHECKIN_2026-01-22_END -->

# 2026-01-21
<!-- DAILY_CHECKIN_2026-01-21_START -->






### 今天收看了「社区运营基础 & 活动策划与执行」回放

-   看了群友分析的社区运营基础&活动策划与执行的会议笔记。
    
-   继续了解社区运营的知识
<!-- DAILY_CHECKIN_2026-01-21_END -->

# 2026-01-20
<!-- DAILY_CHECKIN_2026-01-20_START -->







### 一：社区常用工具的了解及熟练应用 DC

-   **官方链接**
    
    -   主服务器：[https://discord.com/invite/binanceofficial](https://discord.com/invite/binanceofficial) （约 28.2 万成员，支持文本/语音/角色系统）
        
    -   子服务器（如区域社区）：[https://discord.com/invite/jDfbNjCG9F（约](https://discord.com/invite/jDfbNjCG9F（约) 2.9 万成员）
        
    
-   **定位与目标用户**
    

面向核心用户、重度用户与社区治理用户；适合深度讨论、教程、开发者交流。

-   **成员规模与结构**
    

约 28.2 万，主服务器结构化：多频道（如公告、交易讨论、教育、支持、事件），角色系统（新手/VIP/ moderator），支持语音室和 bot 整合。

-   **内容类型**
    
    -   深度讨论（技术分析、DeFi/NFT 策略）。
        
    -   实时新闻与 AMA。
        
    -   媒体分享（图表、视频、alpha 洞察）。
        
    -   专属频道（支持、项目合作、TGE 预告）。
        

-   **运营策略**
    
    -   **频道管理**：分层频道（公开/私有），角色分配提升活跃度。
        
    -   **事件主持**：定期 AMA、直播、竞赛。
        
    -   **Moderation**：多层管理员，防诈骗和低质内容。
        
    -   **频率**：实时互动，每月大事件（如 TGE airdrop）。**总结**
        

DC = 深度社区 + 活动中心 + 用户留存系统
<!-- DAILY_CHECKIN_2026-01-20_END -->

# 2026-01-19
<!-- DAILY_CHECKIN_2026-01-19_START -->








### 一：Web3 实习手册[「社区运营指南」](https://web3intern.xyz/zh/community-intern/)部分

-   社区运营职责
    
-   常见工具与平台使用
    
-   常见任务案例模板
    

### 二：社区常用工具的了解及熟练应用 TG

以币安交易所为例:

-   **官方链接**
    
    -   英文社区群：[https://t.me/binanceexchange](https://t.me/binanceexchange)
        
    -   公告频道：[https://t.me/binance\_announcements](https://t.me/binance_announcements)
        
    -   其他专题（如中文、公告、多语言）：[https://t.me/BinanceChinese](https://t.me/BinanceChinese) 等，总计 60+ 群组
        
-   **定位与目标用户**
    

面向更大众用户，消息快速触达；适合空投信息、公告、活动通知等。

-   **成员规模与结构**
    

总计数百万（分散在 60+ 群组），结构简单：频道（单向公告）+ 群聊（多语言讨论）。易于全球用户加入，支持即时消息。

-   **内容类型**
    
    -   实时公告（平台更新、上市、airdrop）。
        
    -   教育资源（交易策略、初学者指南）。
        
    -   互动讨论（市场信号分享、Q&A）。
        
    -   多语言支持（英文、中文等）。
        

-   **运营策略**
    
    -   **推送导向**：每日公告推送，结合多群组覆盖全球用户。
        
    -   **工具整合**：Bot 自动化公告和反馈收集。
        
    -   **Moderation**：严格指南（禁止 spam、诈骗、广告），社区经理监控并鼓励报告。
        
    -   **频率**：高频更新（每日新闻），每周任务/事件。
        

-   **总结**
    

TG = 流量入口 + 广播站 + 福利吸引
<!-- DAILY_CHECKIN_2026-01-19_END -->

# 2026-01-17
<!-- DAILY_CHECKIN_2026-01-17_START -->









### 一：复习回顾相关章节，完成未完成课程

### 二;行业探索：

-   继续打卡知名开发者 & Builder X 关注清单
    
-   了解一下相关数据平台。
<!-- DAILY_CHECKIN_2026-01-17_END -->

# 2026-01-16
<!-- DAILY_CHECKIN_2026-01-16_START -->










### 一：YouTube的Day4的课程

[**Day 5: Stuck Transactions, Gas Limits, Multisigs, L2s, Lending…**](https://www.youtube.com/watch?v=11QTT6BK5j0&list=PLJz1HruEnenAf80uOfDwBPqaliJkjKg69&index=5)

👩‍💻 Web2 到 Web3 🚀 - 第一周第五天：卡住的交易，Gas 限制，多重签名，L2s，借贷...

-   **Stuck Transactions（交易卡住）** 指交易已广播到网络但长时间处于 Pending，无法被打包确认，也阻塞了后续交易。
    
-   **Gas Limits** 指你为一笔交易允许消耗的最大计算量上限。在 Ethereum 上，Gas 用完 ≠ 成功，而是 Out of Gas → 回滚且已花的 Gas 不退。
    
-   **Multisigs（多签钱包）** 指一笔交易需要多个签名共同批准才能执行的账户/合约，常用于资金托管、DAO 金库、项目权限管理。
    
-   **L2s（Layer 2）** 是构建在 Ethereum 之上的扩容网络，把大量交易放到主链之外处理，只把结果/证明提交回主链，从而更快、更便宜。
    

### 二：打卡通用任务

在 My First NFT mint 第一个 NFT。
<!-- DAILY_CHECKIN_2026-01-16_END -->

# 2026-01-15
<!-- DAILY_CHECKIN_2026-01-15_START -->











### 一：YouTube的Day4的课程

1.  [**Day 4: NFTs!!! ERC20 vs ERC721, IPFS, Metadata**](https://www.youtube.com/watch?v=NOdrEpnoCiM&list=PLJz1HruEnenAf80uOfDwBPqaliJkjKg69&index=4)
    

👩‍💻 Web2 到 Web3 🚀 - 第一周第四天 - 🎫 NFTs!!! ERC20 与 ERC721，IPFS，元数据。

-   NFT：NFT (Non-Fungible Token) 是一种在区块链上记录“独一无二所有权”的数字资产。每个 NFT 都不可互换，用来证明某个数字或现实资产的归属。
    
-   ERC20：ERC-20 是 Ethereum 上最基础、最通用的 同质化代币标准，定义了代币如何转账、授权、查询余额等行为。
    
-   ERC721：ERC-721 是 Ethereum 上的 NFT（非同质化代币）标准，用于表示唯一、不可互换的资产（一枚 = 一个独立物品）。
    
-   IPFS：IPFS（InterPlanetary File System） 是一种 去中心化文件存储与分发协议，常用于 NFT 元数据、图片、视频 的长期保存与抗审查访问。
    
-   元数据：元数据（Metadata） 是 NFT 的“灵魂说明书”，定义了 这枚 NFT 是什么、长什么样、有什么属性。  
    在 Ethereum 的 ERC-721 / ERC-1155 体系里，合约只存一个 `tokenURI`，真正内容都在元数据里。
    

### 二：打卡通用任务

-   打开知名开发者 & Builder X 关注清单。
    

### 三：阅读群友分析的Ai及其概念分享会的笔记

-   AI[及其基础概1.AI](http://及其基础概1.AI)智能体的演进:从"聊天"到"打工"。
    
-   [AI](http://2.AI)智能体面临的核心挑战。
    
-   Web3提供的解决方案:协议栈。
    
-   Spoon0S框架介绍。
    
-   未来展望:机器经济的崛起。
    

-   复习了之前交易所AI产品调研
    

[https://hjpvwrbpwy3c.jp.larksuite.com/wiki/BcK5wGuhZiwtufkjSSFjLb3Qpzc](https://hjpvwrbpwy3c.jp.larksuite.com/wiki/BcK5wGuhZiwtufkjSSFjLb3Qpzc)
<!-- DAILY_CHECKIN_2026-01-15_END -->

# 2026-01-14
<!-- DAILY_CHECKIN_2026-01-14_START -->












### 一：安全与合规：

-   Web3 实习手册[「安全与合规」](https://web3intern.xyz/zh/security/)部分。
    
-   参加了Web3安全主题分析会。
    
-   参加了Web3合规主题分析会.
    

今天学习了Web3 的安全与合规，要注意的就是两件事：做到不被坑，能长期发展和符合法律法规。  
安全上，要尽量避免合约漏洞、钓鱼网站和私钥泄露，让自己资产不容易丢；合规上，是让项目在法律允许的范围内运行，减少被封、被关停的风险。我们在操作既要安全操作、又要合规，在Web3 行业才能更好地学习和发展。

### 二：观看YouTube的视频：

-   [**Day 3: ENS, DEX, Identity, Inventory, Sybil**](https://www.youtube.com/watch?v=wYSMNdIRoII&list=PLJz1HruEnenAf80uOfDwBPqaliJkjKg69&index=3)
    

使用钱包与去中心化应用进行交互，注册一个 ENS 地址，然后将一些 ETH 兑换成 DAI。  
展示了在 Web3 中，身份和库存如何从一个应用到另一个应用跟随。
<!-- DAILY_CHECKIN_2026-01-14_END -->

# 2026-01-13
<!-- DAILY_CHECKIN_2026-01-13_START -->













### 一：YouTube的Day2的课程

[**Day 2: Becoming a Power User - Wallets, Mnemonics, Keypairs**](https://www.youtube.com/watch?v=_GjPeRLCREA&list=PLJz1HruEnenAf80uOfDwBPqaliJkjKg69&index=2)

Web2 到 Web3 🚀 - 第一周成为高级用户 - 第二天：钱包、助记词、密钥。

### 一：今天阅读了《从零到一学习以太坊》的读前必看和第一章。

-   以太坊的起源
    

2013 年 11 月：Vitalik Buterin 发表《Ethereum 白皮书》（概念提出）。

2014 年：以太坊完成众筹（ICO）

2015 年 7 月 30 日：以太坊主网正式上线（Frontier 阶段）

-   第一章总结：
    

以太坊并不只是“另一种数字货币”，而是一种将区块链从单纯记账系统升级为可编程公共基础设施的尝试。它通过智能合约和去中心化网络，把原本依赖机构和平台的信任关系，转移到公开的代码和共识机制中，使规则可以自动执行、结果可被全网验证。以太坊的价值不只体现在 ETH 本身，而体现在它所支撑的 DeFi、NFT、DAO 等应用生态上，本质上是在探索一种无需中心中介、由代码协作运行的全新社会与经济形态

### 三：参与了Web3运营原理的分享会
<!-- DAILY_CHECKIN_2026-01-13_END -->

# 2026-01-12
<!-- DAILY_CHECKIN_2026-01-12_START -->














今天着重学习了Web3 实习手册[「入门导读」](https://web3intern.xyz/zh/blockchain-basic/)部分。

## 一：加深关于区块链的认识

区块链 = 不靠中心机构背书的“全球共享数据库”。

区块链（Blockchain） 是一种去中心化的分布式账本技术，通过密码学 + 共识机制，在无需可信中介的情况下，让多方对同一份数据达成一致并不可篡改。

-   核心构件：区块（Block）、节点（Node）、共识机制（Consensus）、密码学
    
-   关键特性：去中心化（无单点控制）、不可篡改(历史几乎无法篡改）、透明可验证(所有人可查）、抗审查（不依赖单一机构）、可编程（智能合约).
    
-   类型：公链（完全开放：DiFe/Web30、联盟链（多机构共管：金融/政务）、私链（单机构：内部审计）
    

## 二:以太坊的概览及其与比特币的差异

以太坊 ：是一个支持智能合约的去中心化区块链平台，使开发者能够构建DiFi、NFT、DAO 等去中心化应用。其核心创新是智能合约。

-   事件：The Merge （合并）是 Ethereum 历史上最重要的一次升级：把共识机制从 PoW（挖矿）切换到 PoS（质押），而不改变账户、合约、交易历史。
    
-   Ethereum与Bitcoin的差异：
    

Bitcoin：坚持 PoW（安全优先、简单）

Ethereum：转向 PoS（可扩展性、经济设计）

| 维度 | 比特币 | 以太坊 |
| --- | --- | --- |
| 核心定位 | 钱 | 平台 |
| 类比现实 | 数字黄金 | 去中心化云计算 |
| 开发者生态 | 很小 | 极大 |
| 可扩展性 | 低 | 高（L2） |

## 三：Web3的工作方式

-   了解Web3 常用工具及远程办公软件
    
-   工具安装了解
    

## 四：社区运营知识

### 1.社区运营职责：

-   日常内容与社确维护
    
-   内容发布与互动引导
    
-   内容发布与互动引导
    
-   对外合作与社区联动
    

### 2.常见工具与平台使用：

-   社交媒体渠道：Twitter (X)、微信公众号、Medium
    
-   社群沟通平台：Discord、Telegram、微信群
    
-   内容创作工具：Notion / Notion AI、 ChatGPT等
    
-   数据与行业调研工具：Etherscan、Dune Analytics、Token Terminal等
    

除了Web3 实习手册的入门导读，还看了YouTube的Day1的课程。
<!-- DAILY_CHECKIN_2026-01-12_END -->
<!-- Content_END -->
