...
-   发行代币会被视作非法集资。无论发行的东西叫做token、积分等任何名词，只要具备融资功能或可流通性，即构成非法金融行为。即便是开发者，也会被当作共谋追究法律责任。
    
-   某些web3的下注、随机收益、兑现的玩法会被认定为赌博，发起者被认定开设赌场罪。邀请返利、多级推广等模式则会被认定为传销。
    
-   协助发币与虚拟币之间的交易，造成绕过外汇监管的情况，可能构成非法经营外汇业务。而且，如果涉及资金来自犯罪活动，成为洗钱中的一环，可能会被牵涉入刑事案件。所以，入金、出金需要核实交易的资金来源。
    
-   涉及币币交易的合同会被认为无效，产生纠纷法院不予受理。
...
NFT当时是伴随元宇宙的概念火起来的。后来元宇宙凉了，NFT也凉了。所以近些年几乎没有什么爆火的NFT品牌出现。

法律层面，香港对待NFT的态度是，要么是一个金融产品，要么就是一个纯艺术品。如果分红，那就是金融产品，必须按照严格的法律规范申请牌照。中国对待NFT的态度是，保护持有者的权益，把NFT当作财产保护。

后来发布NFT时都会同时发一个TOKEN代币。
...
用REMIX可以测试合约编译、部署、调用函数，非常方便。所有流程在本地的区块链上运行。

![image.png](attachment:eb4b1c8c-28f4-403d-a85c-7fb40d0751fb:image.png)

不过，实际开发的时候，肯定还是得在真正的去中心化网络上测试。所以要使用ETH的测试网。

这两个是比较流行的测试网。看到Sepolia就知道它是测试网了，而不是另一个L1或者L2。
...
Skill竟然使用关键词或者正则表达式匹配触发，为什么不让模型自行决定触发？看了下Claude官方的Skills文档，并没有关键字匹配的功能，而是在描述里面让模型自动选择的，可能是SpoonOS自己加的功能？确实SpoonOS是这样实现的。
...
以太坊使用JSON-RPC 2.0协议作为RPC通信内容的格式。所有请求和响应都是JSON，通过HTTP或WebSocket传输。
...
![image.png](attachment:bd3e3a90-31cf-4fe6-83c8-585b3bd5d58e:image.png)

![image.png](attachment:132046a3-3239-4196-bac8-d4384414a66b:image.png)

可以使用Viem、Ethers.js、Web3.js等库调用RPC节点。

注意，RPC服务是收费的，所以要避免泄露API\_KEY。RPC服务本质上是中心化的服务，是区块链的全节点为其它想要开发Web3应用的人提供的第三方服务。因为直接通过区块链的协议查询余额是很麻烦的，域名并不直接保存在链上，很多东西需要用单独的数据库维护，并实时与链上数据同步。所以RPC服务就解决了这个问题。Web3开发者只需要调用服务就行了，不需要自己直接连接到区块链上。当然，这样做本质上又增加了中心化程度。看起来中心化和去中心化就是一个拉锯战，很难调和去中心化程度与便捷性。
