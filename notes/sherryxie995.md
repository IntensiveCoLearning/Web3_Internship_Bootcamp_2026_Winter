...
-   **前端 (Frontend)：** 依然是 React/Vue 等前端框架，但它们不通过用户名/密码登录，而是通过 `ethers.js` 或 `viem` 库与钱包（MetaMask）通信。
    
-   **节点提供商 (RPC Provider)：** 区块链数据很大，普通手机/电脑跑不动全节点。应用通常通过 **Infura** 或 **Alchemy** 这样的 RPC 节点来读取链上状态或发送交易。
    
-   **存储层：** 区块链存数据非常贵（存一张照片可能要几千美金）。因此，Web3 采用“链上存哈希，链下存数据”的方案。
    
    -   **IPFS (星际文件系统)：** 基于内容寻址（Content Addressing）的分布式存储。
...
作为密码学专业的同学，你会发现 Web3 实际上是**大规模应用密码学**的实验室。

-   **哈希函数 (Hashing)：** Web3 的灵魂。
    
    -   以太坊主要使用 **Keccak-256**（注意它和标准的 SHA-3 有细微区别）。
        
    -   **Merkle Tree (默克尔树)：** 区块链压缩数据的核心。它允许节点在不下载整个区块的情况下，仅凭一个“默克尔根”和一条“路径（Proof）”就能验证某笔交易是否存在。
        
-   **非对称加密与签名：**
    
    -   **ECC (椭圆曲线密码学)：** 比特币和以太坊都使用 **secp256k1** 曲线。
        
    -   **ECDSA (椭圆曲线数字签名算法)：** 你的每一笔转账其实都是一次数字签名过程。
        
    -   **助记词 (BIP-39)：** 它是通过哈希和校验位将随机数映射到单词表，本质上是私钥的可读化包装。
        

* * *

### 3\. 进阶黑科技：零知识证明 (ZK-Proofs)

如果你在本科阶段接触过 ZK，那么你会发现它在 2026 年的 Web3 已经是绝对的主角。

-   **核心逻辑：** 证明者（Prover）能在不泄露任何原始数据的情况下，向验证者（Verifier）证明某个命题是真的。
    
-   **Web3 的两大杀手级应用：**
    
    1.  **隐私：** 证明我有钱，但不用展示余额；证明我满 18 岁，但不用展示出生日期（如 zkLogin）。
        
    2.  **扩容 (ZK-Rollups)：** Layer 2 节点在链下处理一万笔交易，然后生成一个微小的**有效性证明 (Validity Proof)** 发给以太坊主网。主网只需要花极小的算力验证这个证明，就相当于确认了这一万笔交易的合法性。
...
这是 Web3 走向大规模应用的关键。传统的钱包（EOA）太难用，弄丢私钥就完蛋。

-   **概念：** 将钱包从一个“密钥对”变成一个**智能合约**。
    
-   **带来的改变：**
    
    -   **社交恢复：** 像找回邮箱密码一样，通过朋友投票找回钱包。
        
    -   **免 Gas 费：** 项目方可以代付 Gas 费。
        
    -   **多因子认证：** 像银行 App 一样，大额转账需要指纹+动态验证码。
...
以前的区块链（如以太坊 1.0）是单体架构：自己管共识、自己管执行、自己管数据。现在的趋势是**拆分**：

-   **执行层：** 负责跑代码（如 Arbitrum）。
    
-   **数据可用性层 (DA)：** 专门负责存原始交易数据（如 **Celestia**）。
...
如果说比特币是“数字黄金”，那么智能合约就是 Web3 的“自动售货机”。它是构建 DApp（去中心化应用）的基石。

-   **定义：** 一段部署在区块链上的代码，当满足特定条件时，它会自动执行，无需人工干预。
    
-   **特性：**
    
    -   **Code is Law（代码即法律）：** 规则一旦写入并部署，就不可篡改（大部分情况下）。
        
    -   **去信任化：** 你不需要信任对方，只需要信任代码逻辑。
        
-   **通俗类比：**
    
    > 传统合同：如果A迟到了，B需要找律师起诉，法官判决后，A才赔钱。
    > 
    > 智能合约：A和B把钱锁进代码里，代码连接了原子钟。如果时间到了A没打卡，代码自动把钱转给B。没人能阻止这个过程。
    

* * *

### 2\. 你的数字护照：加密钱包 (Crypto Wallet)

在 Web3 中，钱包不仅仅用来存钱，它是你的**身份 ID**和**登录器**。

-   **非托管钱包 (Non-Custodial)：** 如 MetaMask、Phantom。
    
    -   **核心理念：** "Not your keys, not your coins"（没有私钥，币就不是你的）。
        
    -   **公钥 (Public Key)：** 相当于你的银行卡号，可以公开给别人给你转账。
        
    -   **私钥 (Private Key) / 助记词 (Seed Phrase)：** 相当于你的卡号+密码+最高权限。**绝对不能给任何人**，一旦丢失，资产永久找不回。
        
-   **冷钱包 vs 热钱包：**
    
    -   **热钱包 (Hot Wallet)：** 连网的（如手机App、浏览器插件），方便但风险稍高。
        
    -   **冷钱包 (Cold Wallet)：** 不连网的硬件设备（如 Ledger），最安全，适合存大额资产。
        

* * *

### 3\. 扩容方案：Layer 1 与 Layer 2

新手常会抱怨：“以太坊转账手续费（Gas Fee）怎么这么贵？” 这就涉及到了区块链的分层架构。

-   **Layer 1 (L1) - 主网/地基：**
    
    -   **代表：** Ethereum (ETH), Solana (SOL), Bitcoin (BTC)。
        
    -   **功能：** 负责最终的安全性及结算。就像市中心的银行金库，极其安全但处理速度慢，甚至拥堵。
        
-   **Layer 2 (L2) - 扩容层：**
    
    -   **代表：** Arbitrum, Optimism, Base, Polygon (侧链)。
        
    -   **功能：** 为了解决 L1 拥堵而生。它们在链下打包处理几百笔交易，然后一次性把结果汇报给 L1。
        
    -   **类比：** L1 是结算总行，L2 是遍布各地的分行或支付宝。日常买咖啡（小额高频）走 L2，速度快、手续费极低；大额资产存储走 L1。
...
在 Web3 里，几乎所有价值传输都通过 Token（代币）进行。

| 类型 | 技术标准 (以太坊为例) | 特点 | 例子 |
| 同质化代币 | ERC-20 | 互相可替代。你手里的一枚比特币和我手里的一枚完全一样，可以分割（如0.01个）。 | USDT, UNI, SHIB |
| 非同质化代币 (NFT) | ERC-721 | 独一无二。就像艺术品或房产证，不可分割，每一枚都有独特编号。 | 无聊猿 (BAYC), 游戏道具, ENS域名 |

* * *

### 5\. 三大应用支柱 (Web3 能干什么？)

当你有了钱包和 Token，你可以参与以下活动：
...
-   **你可以做：**
    ...
    -   **DEX (去中心化交易所)：** 如 Uniswap。不需要注册账户，连接钱包就能把 ETH 换成 USDT。
        
    -   **Lending (借贷)：** 如 Aave。抵押你的比特币，借出美元，没有信用审查。
...
-   **你可以做：** 购买数字艺术品、在游戏中拥有真正属于你的装备（甚至可以跨游戏出售）。
...
-   **你可以做：** 持有某个项目的 Token，你就是“股东”。你可以发起提案（比如“我们要不要把手续费降低？”）并投票，代码会根据投票结果自动执行决策。
...
Web3 是“黑暗森林”，自由的代价是责任。请牢记：

1.  **助记词绝不截图、绝不联网：** 最安全的方式是用笔抄在纸上，锁在保险柜里。
    
2.  **警惕钓鱼网站：** 很多诈骗网站做得和 Uniswap 一模一样。操作前务必核对 URL。
    
3.  **不要乱点链接：** 此时此刻，你的钱包里可能突然多了一些来路不明的“空投币”。**千万不要去操作它或授权它**，这通常是诱导你授权钱包权限的陷阱，一旦授权，你的资产会被瞬间转空。
