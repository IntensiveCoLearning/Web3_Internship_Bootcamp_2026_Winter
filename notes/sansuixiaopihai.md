...
最近的工作有点忙，学习的时间比较少，今天主要也是听一些分享会和周会
...
练习和使用solidity完成demo，记录难点

\### payable 关键字：

在 Solidity 中，默认情况下，函数是\*\*拒绝\*\*接收以太币（ETH）的，

\- **没有 payable**：如果你尝试调用一个普通函数并附带 ETH，交易会直接\*\*报错（Revert）\*\*，钱会退回给用户，Gas 费会被消耗掉一部分。

\- **加上 payable**：这就告诉以太坊虚拟机（EVM），“这个函数允许在调用的同时接收 ETH”

当用户调用 payable 函数并发送了 ETH 后：

ETH 会自动进入合约的余额（address(this).balance 自动增加），

Solidity 会自动提供一个全局变量叫 **msg.value**系统会自动把用户转账的金额填入 msg.value

代码解释：address(this).balance\*\*（用来读取当前合约还有多少以太币余额）\*\*

\- **this**: 指代\*\*当前的合约实例\*\*（也就是 Crowdfunding 这个合约自己）。

\- **address(this)**: 把合约实例转换成\*\*地址类型\*\*。相当于获取\*\*这个合约在区块链上的钱包地址\*\*。

\- **.balance**: 这是 Solidity 中所有 address 类型都有的一个属性，用来读取该地址当下持有的 **以太币余额**（单位是 wei）。

把地址重新转回去

(bool success, ) = payable(msg.sender).call{value: amount}("");没有gas限制，通常回返回两个值，第一个值是成功还是失败，第二个是数据（通常忽略掉）

务必获取返回值 bool success 并 require(success)

**什么是rpc：**

**RPC（Remote Procedure Call，远程过程调用）** 是一种通信协议，正常的web2中请求数据用的一般都是RESTful API

RPC 在区块链开发中的应用，链接区块链的数据，但正常一般来说开发用的metamask里面有自带的默认的RPC，所以会对着个概念产生模糊，但自带的RPC人多的时候也会出现卡顿的情况

正常其他使用的场景：

\- **离线展示：** 当用户\*\*还没连接钱包\*\*时，你的 DApp 依然需要展示当前代币价格、总锁仓量（TVL）等数据。这时你的前端代码需要通过一个固定的 RPC 地址去抓取数据，而不是等用户点“连接钱包”。

\- **后端逻辑：** 如果你有后端脚本（比如监听合约自动发邮件、定时任务等），后端是没有钱包环境的，必须配置一个长期的 RPC URL。
...
找了好久终于找到问题了，之前一直以为是编码格式的问题，后面找到问题才发现是\*\*合约没有审核\*\*，没有进行源码验证所以看不到历史的记录

kite 里面的eos钱包和AA钱包的区别：

eos钱包就是自己的本地连接的钱包，AA钱包是eos钱包然后根据生成出来的钱包

使用场景就有点像只要进行一次签名，在未来的一段时间里面所有的交互就是AA从后台完成

这个kite的sdk还是没有弄好，\*\*明天还得多花时间\*\*，一直卡在这个aa钱包每次支付都需要签名授权
...
Solidity中`block`全局变量用于获取当前区块链的属性
...
block.gaslimit：gas限制

黑客松总结：

路演：10分钟，3-5分钟提问，12页ppt比较好，去掉章节目录，动画可能拖延时间，图片+文字最好，重要的是有限时间完成讲解、用很抓眼的图片吸引

1.预估 2.清晰结构 3.项目的亮点和差异

选题：选题不要太大，要小的切入点，创新+前端展示+商业价值

商业价值能够自圆其说很重要，去掉上链过程，核心看想法，创新点，保证项目的最小呈现

如何呈现技术闭环？

代码亮出，说解决什么问题，降低...，节省多少时间？

视频作用：第二手准备，如果代码运行失败，可以更好展示，可以剪辑，节省时间，更好展示
