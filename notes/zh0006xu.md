...
今天是 Web3 实习计划的最后一天，这个周末忙于赶行程，错过了昨天的打卡。现在在机场写下最后一篇打卡笔记，算是对这个月的学习做好收尾。

最大的感受是对主办方的感激：LXDAO 真良心！这个项目的质量远超我的预期。认真参与活动的这个月让我收获颇丰，对 Web3 的横向和纵向理解都加深了许多。与此同时，因为目前的种种限制，无法对社群做出什么贡献，让我觉得很惭愧。有朝一日我一定会回馈！

这个月的学习笔记汇总：bit.ly/49E72mP。还有更多内容要陆续补充完整。
<!-- DAILY_CHECKIN_2026-02-08_END -->

# 2026-02-06
<!-- DAILY_CHECKIN_2026-02-06_START -->

今日整理笔记，检查列表内剩余任务，安排周末时间完成
...
> ERC-7962 是一套“以 keyHash 而非 address 作为所有权标识”的代币标准。授权通过链下签名完成，执行由任意调用者提交上链（可代付 gas），从而实现更好的隐私隔离与 gasless 交互。

## 1\. 动机

现有 ERC-20 / ERC-721 的核心问题：

-   **所有权强绑定 address** → 余额、转移路径天然可追踪。
    
-   **授权依赖 msg.sender / allowance** → 用户必须自己付 gas，且 approve/allowance 是长期授权，风险与可追踪性都高。
...
-   把“谁拥有资产”从 address 抽象成 **keyHash（bytes32）**，
...
### 2.1 所有权模型：address → keyHash

-   余额 / NFT 归属不再是 `address => balance`，而是`bytes32 keyHash => balance / ownership`
    
-   `keyHash = hash(publicKey)`（而非地址）
...
-   同一用户可使用**多把 key（一次性 key）**，降低地址级关联
    

### 2.2 授权与执行分离

-   **授权**：持有 key 的用户在链下对结构化数据签名（EIP-712）
    
-   **执行**：任何人（relayer、平台、合约）都可把签名提交上链
...
-   每个 `keyHash` 维护独立 nonce
    
-   签名包含 `nonce + deadline`
    
-   合约验证 `pubKey → keyHash` 一致性
...
-   NFT 的 owner 是 `keyHash`
    
-   转移：`fromKeyHash → toKeyHash`
...
-   **不是强隐私**（公钥仍在 calldata 中）
    
-   钱包复杂度显著提高（key pool、找零、扫描）
    
-   DeFi 兼容性差（无 allowance，余额模型不同）
...
Q1：既然签名是链下生成、任何人都能提交，如何防止签名被“盗用 / 滥用”？

A1（GPT）：**签名是一次性、强约束、强绑定上下文的“可执行凭证”**，而不是通用授权。

**ERC-7962 不试图阻止“别人提交你的签名”，而是让“即使别人提交，也只能按你签的那一次来执行，且只能成功一次”。**
...
1.  强语义绑定：签名内容不可泛化——签名只对应一个确定的状态迁移
...
2.  单次有效：per-keyHash nonce
    
3.  时间约束：deadline
    

-   签名自带过期时间 — 签名在 mempool / relayer / 中转服务里泄露，**窗口期有限**
    
-   超时即不可用 — 防“长期埋雷式授权”
    

4.  身份绑定：pubKey → keyHash 校验
    

```
hash(pubKey) == fromKeyHash
```

-   提交者无法“换一个公钥来用同一签名”
    
-   签名只能为该 keyHash 所有的资产服务
    

### ERC-KeyHash20 的额外安全性

ERC-KeyHash20 还引入了更激进的一层防护：**强制** `leftKeyHash`**（找零 key）**
...
-   即使签名暴露，也不存在“剩余余额可被继续薅”的问题
    

👉 安全模型更接近 Bitcoin 的 UTXO，而不是账户余额。
...
今日回顾了「**就业简历指导 & 面经分享」**分享会内容，准备了一份面向 Web3 的简历。

睡觉前得知自己的 PolyLens 项目在黑客松预测赛道拿了亚军，刚好可以丰富简历内容。立春的馈赠 ：)
...
**是什么**  
定义了“余额、转账、授权”的统一接口，用来表示**同质化资产**。

**解决什么问题**  
让钱包、交易所、DeFi 协议都能用**同一种方式**操作代币。

**为什么重要**

-   DeFi 几乎全部建立在 ERC-20 之上
    
-   没有 ERC-20，就没有今天的 DeFi
...
**是什么**  
定义**不可替代代币**，每个 token 都有唯一 ID。

**解决什么问题**  
在链上表达“唯一性资产”（艺术品、域名、头像、收藏品）。

**为什么重要**

-   OpenSea / Blur / ENS 全部依赖它
...
### 3️⃣ ERC-1155｜多资产标准（FT + NFT）

> 更工程化、更省钱的资产标准

**是什么**  
一个合约里同时支持 **ERC-20 + ERC-721**。

**解决什么问题**  
ERC-20 / 721 合约碎片化、Gas 成本高的问题。

**为什么重要**

-   批量 mint / transfer，省 Gas
...
**是什么**  
定义 `supportsInterface()`，让合约能“自我声明支持哪些 ERC”。

**解决什么问题**  
合约之间如何**安全判断**对方是不是 ERC-721 / 1155？
...
### 5️⃣ ERC-2612｜Permit（免 approve 授权）

> 用签名替代 approve

**是什么**  
允许用**离线签名**完成 ERC-20 授权。

**解决什么问题**  
传统流程：  
`approve → 再交易`（2 笔交易 + 2 次 Gas）

**为什么重要**

-   用户体验显著提升
    
-   DeFi 基础设施标配（DAI、Uniswap）
    

* * *

### 6️⃣ ERC-4626｜Vault（金库）标准
...
### 7️⃣ ERC-4337｜账户抽象（AA）

> 钱包进入“智能合约时代”

**是什么**  
把钱包从 EOA（私钥账户）升级为**智能合约账户**。
...
### 8️⃣ ERC-6551｜Token Bound Accounts（NFT 钱包）
...
### 9️⃣ ERC-5192｜Soulbound Token（SBT）
...
### 🔟 ERC-3643（T-REX）｜合规代币标准

> Web3 与现实金融的接口

**是什么**  
内置 **KYC / 白名单 / 合规转账逻辑** 的代币标准。

**解决什么问题**  
传统 ERC-20 **不符合监管要求**，无法用于证券、债券。
...
| 层级 | ERC | 核心关键词 |
| --- | --- | --- |
| 基础 | 20 / 721 / 1155 | 资产怎么表示 |
| 交互 | 165 / 2612 | 合约如何协作 |
| DeFi | 4626 | 收益如何标准化 |
| 钱包 | 4337 | 钱包如何进化 |
| NFT | 6551 / 5192 | NFT 是否“有生命”，可转让 |
| RWA | 3643 | 如何合规 |

## \* 其他思考

感觉 ERC 的本质，和 Python 社区的 PEP 非常像**，但“约束力”和“落地方式”更强。**
...
-   PEP 是 Python 社区“如何把代码**写好**”
    
-   ERC 是 Web3 社区“什么代码才算**存在**”——像**「以太坊生态的可执行接口宪法」**
...
今天把 PolyLens UI 简单优化了以下，加入了日/夜间和跟随系统切换模式；优化了 Markets 页面的 token 价格显示。
...
今日完成黑客松提交，在参加同学demo分享的过程中完成了原计划的下一个阶段任务：网页的Insights页面。以下是项目相关文件链接：

-   [PolyLens Github](https://github.com/zh0006xu/PolyLens.git)
    
-   [Demo slides](https://docs.google.com/presentation/d/1Ys9aJu7nqEaQjvxk8SDrm95fGxA-0c17vQCnlIxzZqc/edit?usp=sharing)
...
今日完成黑客松项目目标的基本技术层面，最主要的工作是把前端从 streamlit 迁移到 React，页面美观度大幅提升。
...
今天继续做黑客松，用 streamlit 完成简易“情绪仪表盘”前端，展示：从 Polymarket 抓取到的市场，常用市场指标和动态数据，K线走势，捕捉“鲸鱼交易“
...
今天继续为黑客松做工作，完成Polymarket 链上市场与交易索引器，搭建了本地数据库。初步完成了基础设施建设
...
今天为黑客松做准备，选择了「市场预测」赛道。

首先通读了项目方提供的[资料](https://github.com/ogalias/OGBC-Intern-Project/tree/master)。主要了解Polymarket运作、架构与链上数据解码机制，完成2个脚本trade\_decoder.py和market\_decoder.py

-   trade\_decoder.py: 交易日志解析器，输入交易哈希（在 Polygon 链上），输出该交易中 Polymarket 订单撮合的详情;
    
-   market\_decoder.py: 给定链上获取的市场创建相关信息（如 `ConditionPreparation` 日志，或已知的 `conditionId`），提取并计算出该市场的核心链上参数，包括预言机、问题 ID、抵押品地址，以及 YES/NO 两种头寸的 TokenId
    

明天计划完成Polymarket 链上市场与交易索引器，并跟小组成员商讨黑客松详细安排。
...
# Vibe Coding & 黑客松经验分享 **\- 讲座笔记**

## 黑客松

主要内容：

介绍了经典黑客松流程、团队组成、项目选择、技术工具推荐、获奖技巧等。

![image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/zh0006xu/images/2026-01-27-1769512638245-image.png)![image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/zh0006xu/images/2026-01-27-1769512658356-image.png)

## Vibe Coding

主要内容：Vibe Coding的演化史、推荐哪个模型、使用技巧。

![image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Web3_Internship_Bootcamp_2026_Winter/main/assets/zh0006xu/images/2026-01-27-1769512785707-image.png)
