...
## 🏛️ 第一层：资产基石 (The Asset Layer)

这是 Web3 的“原子”，所有上层应用都建立在这些标准之上。

| 标准 | 别名/核心功能 | 一句话总结 (Key Takeaway) | 适用场景 |
| ERC-20 | 同质化代币 | DeFi 的血液，每一枚代币价值相等。 | 支付、治理代币、稳定币 |
| ERC-721 | NFT | 独一无二的数字资产，强调所有权。 | 艺术品、PFP、域名 |
| ERC-1155 | 多代币标准 | 一个合约管理多种资产 (FT + NFT)，省 Gas。 | 游戏道具（金币+装备打包） |
| ERC-3525 | SFT (半同质化) | 像 NFT 一样有 ID，像 ERC-20 一样可拆分计算。 | 票据、债券、会员卡积分 |

> **💡 学习重点：** 熟练掌握这三者的 `balanceOf`, `transfer`, `approve` 机制差异。理解 ERC-1155 的 `batchTransfer` 如何优化 Gas。

* * *

## ⚙️ 第二层：DeFi 引擎与效率 (DeFi & Efficiency)

这部分标准解决了“如何更安全、更便宜、更自动化地使用资产”的问题。

### 1\. 资金效率与流动性

-   **ERC-4626 (Tokenized Vaults):** **必学。** 它统一了 DeFi 的收益金库接口（不再需要为每个协议写适配器）。
    
    -   _核心：_ `deposit` / `withdraw` (资产) vs `mint` / `burn` (份额)。
        
-   **ERC-3156 (Flash Loans):** (补充) 闪电贷标准，虽然你未列出，但在 DeFi 进阶中很重要。
    

### 2\. 交互体验与 Gas 优化

-   **ERC-2612 (Permit):** **必学。** “签名即授权”。用户不需要先发一笔 `approve` 交易（省 Gas），直接在链下签名，合约拿着签名去划转代币。
    
-   **ERC-1363 (Payable Token):** 让转账和调用合约一步完成（`transferAndCall`），减少交易步骤。
    
-   **ERC-1271:** 允许智能合约像普通人一样“签名”验证。这对多签钱包（Gnosis Safe）和智能钱包至关重要。
    

* * *

## 🚀 第三层：大规模应用前沿 (Mass Adoption & UX)

这里是目前以太坊生态最活跃的开发领域，解决了“门槛太高”和“资产太死板”的问题。

### 1\. 账户抽象 (Account Abstraction)

-   **ERC-4337:** **核心中的核心。** 彻底改变钱包体验。它引入了 `UserOperation` 和 `Bundler`，实现了无私钥社交恢复、Gas 代付、批量交易。
    

### 2\. NFT 的进化 (NFT 2.0)

-   **ERC-6551 (Token Bound Accounts):** 让 NFT 拥有自己的钱包。
    
    -   _场景：_ 一个 RPG 游戏角色（NFT）可以拥有自己的装备（也是 NFT）和金币（ERC-20）。
        
-   **ERC-4907 (Rental):** 租赁标准。将 `owner` (所有者) 和 `user` (使用者) 分离。GameFi 租赁系统的基石。
    
-   **ERC-2981:** 统一版税标准，解决跨平台交易版税丢失的问题。
    

* * *

## ⚖️ 第四层：RWA 与合规 (Regulatory & RWA)

机构入场的关键。这部分标准的核心逻辑是：**“不是谁都能转账，必须符合规则”**。

-   **ERC-3643 (T-REX):** 目前最主流的合规代币标准。
    
    -   _机制：_ 在 `transfer` 函数中植入 `ONCHAINID` 验证，只有通过 KYC 的白名单用户才能接收代币。
        
-   **ERC-1400 系列:** 证券型代币的“瑞士军刀”，功能极其丰富（分层、强制转移、文档附加），但也相对复杂。
    

* * *

## 🤖 第五层：AI + Web3 (Emerging Narrative)

这是一个正在被定义的领域。核心在于解决 AI 的\*\*信任（Trust）**和**所有权（Ownership）\*\*问题。

-   **ERC-7007 (AIGC 验证):** 使用 zkML (零知识机器学习) 证明某个内容确实是由特定模型生成的。
    
-   **ERC-7016 (AI Agent NFT):** 赋予 AI 代理身份和资产控制权。
    
-   **ERC-7500/7641:** 将算力、模型权重、API 订阅权资产化，构建去中心化的 AI 算力市场。
    

* * *

## 🛠️ 第六层：治理与合约架构 (Governance & Upgrade)

-   **ERC-1967 & ERC-1822 (UUPS):** 代理升级模式的标准。理解“数据与逻辑分离”是写可升级合约的基础。
    
-   **ERC-2535 (Diamond):** 钻石模式。适合巨型合约，突破合约大小限制（Spoke-Hub 模式）。
<!-- DAILY_CHECKIN_2026-01-27_END -->

# 2026-01-26
<!-- DAILY_CHECKIN_2026-01-26_START -->

**1\. Solidity by Example | 0.8.26 Applications**

-   **重点应用场景：**
    
    -   **Ether Wallet:** 学习 `receive()` 与 `fallback()` 的区别，以及 `transfer` vs `call`（推荐使用 `call` 防止 Gas 限制问题）。
        
    -   **Multi Sig Wallet (多签钱包):** 核心逻辑是“提交(Submit) -> 批准(Approve) -> 执行(Execute)”。注意 `confirmations` 计数逻辑和权限控制。
        
    -   **Merkle Tree:** 白名单校验的高效方案（节省 Gas），利用 `keccak256` 验证 Proof。
        
    -   **Create2:** 预测合约地址（对于交易所充值地址或 Uniswap Pair 生成至关重要）。
        
-   **0.8.26 新特性关注：**
    
    -   `require(bool, Error)`: 允许在 `require` 中使用自定义 Error（Gas 更低），不再局限于 string。
        

**2\. Hardhat 3 核心笔记 (New Architecture)**

Hardhat 3 引入了 **EDR (Ethereum Development Runtime)**，底层由 Rust 重写，旨在解决构建速度慢的问题。

-   **架构变化：**
    
    -   **Native Solidity Tests:** 你可以直接用 Solidity 编写测试（类似 Foundry），不仅限于 TypeScript/Chai。
        
    -   **Performance:** 编译和测试速度大幅提升，接近 Foundry。
        
    -   **ESM First:** 默认支持 ECMAScript Modules (`import/export`)，配置文件更新为 `hardhat.config.ts` (ESM 风格)。
...
-   **常用命令速查：**
    ...
    ```
    # 初始化 Hardhat 3 项目
    npm init hardhat@latest
    
    # 启动本地节点 (现在底层是 Rust EDR，速度更快)
    npx hardhat node
    
    # 运行测试 (支持 .sol 测试文件)
    npx hardhat test
    
    # 部署脚本 (使用 Hardhat Ignition 模块化部署)
    npx hardhat ignition deploy ./ignition/modules/MyModule.ts
    ```
    

**3\. 拓展：Uniswap V2 核心代码解析**

Uniswap V2 是 DeFi 的“Hello World”，理解它就理解了 AMM。

-   **核心公式：** $x \\times y = k$ (Constant Product AMM)
    
-   **关键合约结构：**
    
    -   `UniswapV2Factory`: 负责创建交易对 (`createPair`)，使用 `create2` 保证地址确定性。
        
    -   `UniswapV2Pair`: 核心逻辑所在。
        
        -   `mint()`: 添加流动性，铸造 LP Token。
            
        -   `burn()`: 移除流动性，销毁 LP Token。
            
        -   `swap()`: 交易逻辑，包含价格预言机（TWAP）的累积价格更新。
            
    -   `UniswapV2Router`: 用户交互的前端合约，负责处理 ETH <-> WETH 转换和滑点检查。
        

* * *

### **📅 Phase 2: Foundry 全栈掌握 (1.28 - 1.29)**

Foundry 是目前最硬核的开发框架，全 Rust 编写，核心理念是\*\*“用 Solidity 写一切”\*\*（测试、脚本、部署）。

**1\. Foundry 工具箱组件**

-   **Forge (Build & Test):** 编译、测试的核心。
    
-   **Cast (Interact):** 命令行瑞士军刀，用于与节点交互（发交易、查数据）。
    
-   **Anvil (Local Node):** 极速本地节点（替代 Ganache/Hardhat Node）。
    
-   **Chisel (REPL):** Solidity 交互式命令行，快速验证简短代码片段。
    

**2\. 本地节点交互实战 (Cast Cheat Sheet)**

在本地 Anvil 节点启动后 (`anvil`)，使用 `cast` 进行交互：

Bash

```
# 1. 查询余额 (支持 ENS 或地址)
cast balance 0x... --rpc-url http://127.0.0.1:8545

# 2. 调用 View 函数 (读取数据)
cast call 0xContractAddress "balanceOf(address)(uint256)" 0xUserAddress

# 3. 发送交易 (写入数据, 比如调用 transfer)
# --private-key 为 Anvil 默认私钥之一
cast send 0xContractAddress "transfer(address,uint256)" 0xRecipient 1000000000000000000 \
  --private-key 0xac09... 

# 4. 类型转换工具 (非常实用)
cast --to-dec 0x1a      # Hex 转 Dec
cast --to-wei 1 ether   # Ether 转 Wei
cast calldata "transfer(address,uint256)" 0x... 100 # 生成 raw calldata
```

**3\. Foundry 测试流 (Forge)**

-   **Cheatcodes (作弊码):** Foundry 的灵魂。
    
    -   `vm.prank(address)`: 伪造下一个调用的发送者（模拟用户）。
        
    -   `vm.deal(address, uint256)`: 给某个地址设置 ETH 余额。
        
    -   `vm.warp(uint256)`: 修改区块时间（测试时间锁 TimeLock 必备）。
        
    -   `vm.expectRevert()`: 预期下一行代码会回滚（测试异常情况）。
        

**4\. 拓展：OpenZeppelin 代码结构**

-   **Wizard:** 推荐使用 OpenZeppelin Wizard 快速生成代码骨架。
    
-   **核心库路径:**
    
    -   `@openzeppelin/contracts/token/ERC20/`: 标准代币实现。
        
    -   `@openzeppelin/contracts/access/Ownable.sol`: 简单的权限控制。
        
    -   `@openzeppelin/contracts/proxy/`: 代理合约（Transparent, UUPS），用于合约升级。
        
    -   **注意:** 在 Foundry 中使用 OZ 库需要配置 `remappings.txt`，将 `@openzeppelin/` 映射到 `lib/openzeppelin-contracts/`。
